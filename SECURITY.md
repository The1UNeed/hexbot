...
This document describes Hexbot's trust model, names the one
...
security vulnerabilities. **Hexbot does not operate a bug
bounty program.**
...
- Environment details (`hexbot core --version`, commit SHA, OS, Python
  version).
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Hexbot access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Hexbot's documentation
...
Hexbot supports two OS-level isolation postures. They address
...
Hexbot supports this in two ways:

- **Hexbot's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Hexbot's in-process heuristics
...
Hexbot filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Hexbot
itself. Bugs in Hexbot's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Hexbot does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Hexbot's own documentation, or reasonable
  operator expectations would predict — including cases where
  Hexbot has documented a stance about how its output should
...
- Hexbot includes supply-chain guards for MCP server
