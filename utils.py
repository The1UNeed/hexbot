...
def atomic_replace(tmp_path: Union[str, Path], target: Union[str, Path]) -> str:
    ...
    ``~/.hexbot/`` to a git-tracked profile package or dotfiles repo
    (GitHub #16743).
...
def warn_if_credential_file_broadly_readable(
    ...
    *,
    ...
    Secret-bearing files that users create by hand (or that older Hexbot
...
def atomic_roundtrip_yaml_update(
    ...
    # ``hexbot core config set``'s ``_set_nested`` (#91607: /model + TUI
