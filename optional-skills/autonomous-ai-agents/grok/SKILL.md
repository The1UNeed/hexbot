...
# Grok Build CLI — Hexbot Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Hexbot terminal. Grok
...
1. Prepare stable input files first with Hexbot tools (`read_file`,
...
2. **Don't conflate Hexbot's xAI auth with the `grok` CLI's auth.** Hexbot's
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Hexbot's xAI auth carries
   over.
