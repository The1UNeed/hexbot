...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from Hexbot.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For Hexbot-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `hermes-agent` (Hexbot subagents via `delegate_task`).
