---
name: honcho
description: Configure and troubleshoot Honcho memory for Hexbot.
version: 2.0.0
author: Hermes Agent
license: MIT
...
# Honcho Memory for Hexbot

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Hexbot profile its own peer identity while sharing a unified view of the user.
...
```bash
hexbot core memory setup honcho
...
```bash
hexbot core honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Hexbot profile's AI peer
...
Honcho models conversations as interactions between **peers**. Hexbot creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Hexbot instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Hexbot run |
| `global` | Single session across all directories |

Manual override: `hexbot core honcho map my-project-name`
...
Each Hexbot profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
hexbot core profile create coder --clone
...
```bash
hexbot core honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Hexbot when Honcho memory is active.
...
Run `hexbot core honcho setup`. Ensure `memory.provider: honcho` is in `~/.hexbot/config.yaml`.

### Memory not persisting across sessions
Check `hexbot core honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `hexbot core profile create <name> --clone`. For existing profiles: `hexbot core honcho sync`.
...
| Command | Description |
|---------|-------------|
| `hexbot core honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `hexbot core honcho status` | Show resolved config, connection test, peer info for active profile |
| `hexbot core honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `hexbot core honcho disable` | Disable Honcho for the active profile |
| `hexbot core honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `hexbot core honcho peers` | Show peer identities across all profiles |
| `hexbot core honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `hexbot core honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `hexbot core honcho sessions` | List known directory-to-session-name mappings |
| `hexbot core honcho map <name>` | Map current working directory to a Honcho session name |
| `hexbot core honcho identity` | Seed AI peer identity or show both peer representations |
| `hexbot core honcho sync` | Create host blocks for all Hexbot profiles that don't have one yet |
| `hexbot core honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Hexbot + Honcho |
| `hexbot core memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `hexbot core memory status` | Show active memory provider and config |
| `hexbot core memory off` | Disable external memory provider |
