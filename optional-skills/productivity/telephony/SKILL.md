...
- save provider credentials into `${HERMES_HOME:-~/.hexbot}/.env`
...
It does **not** turn Hexbot into a real-time inbound phone gateway. Inbound SMS is handled by polling the Twilio REST API. That is enough for many workflows, including notifications and some one-time-code retrieval, without adding core webhook infrastructure.
...
4. Treat third-party phone numbers as sensitive operational data:
   - do not save them to Hexbot memory
...
### 1) "I want Hexbot to own a real phone number"
Use **Twilio**.
...
- pairs well with Hexbot `text_to_speech` plus a public file host or tunnel

## Files and persistent state

The skill persists telephony state in two places:

### `${HERMES_HOME:-~/.hexbot}/.env`
...
```bash
SCRIPT="$(find ~/.hexbot/skills -path '*/telephony/scripts/telephony.py' -print -quit)"
```
...
```bash
hexbot core skills search telephony
hexbot core skills install official/productivity/telephony
```
...
Then save credentials into Hexbot:
...
3. Buy it and save it into `${HERMES_HOME:-~/.hexbot}/.env` + state:
```bash
python "$SCRIPT" twilio-buy "+17025551234" --save-env
```
...
```bash
python "$SCRIPT" twilio-call "+15551230000" --message "Hello! This is Hexbot calling with your status update." --voice Polly.Joanna
```

### E. Call with a prerecorded / custom voice message

This is the main path for reusing Hexbot's existing `text_to_speech` support.

Use this when:
- you want the call to use Hexbot's configured TTS voice rather than Twilio `<Say>`
...
Recommended Hexbot TTS -> Twilio Play workflow:

1. Generate the audio with Hexbot `text_to_speech`.
2. Make the resulting MP3 publicly reachable.
3. Place the Twilio call with `--audio-url`.

Example agent flow:
- Ask Hexbot to create the message audio with `text_to_speech`
...
Important note:
- Hexbot TTS is great for prerecorded outbound messages
...
- Hexbot STT/TTS alone is not being used here as a full duplex phone conversation engine; that would require a much heavier streaming/webhook integration than this skill is trying to introduce
...
7. Summarize the outcome without persisting third-party numbers to Hexbot memory.
...
- Do not store arbitrary third-party phone numbers in Hexbot memory.
...
3. persist that number to `${HERMES_HOME:-~/.hexbot}/.env`
