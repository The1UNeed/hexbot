...
The design is **Hexbot-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
hexbot core skills install official/security/unbroker
```

Then start a new Hexbot session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Hexbot session:
...
- **PII handling.** Dossiers live under the Hexbot home directory (`0600`, optionally
