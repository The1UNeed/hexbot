...
- `tmux` available for stable authenticated sessions during Hexbot terminal calls (desktop app flow only)
...
### Service Account (recommended for Hexbot)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${HERMES_HOME:-~/.hexbot}/.env` (the skill will prompt for this on first load).
...
## Hexbot Execution Pattern (desktop app flow)

Hexbot terminal commands are non-interactive by default and can lose auth context between calls.
