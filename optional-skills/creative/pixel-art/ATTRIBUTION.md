...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (Hexbot uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — Hexbot has `image_generate`
...
- Expansion with palettes + video: Hexbot contributors
