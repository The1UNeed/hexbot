...
## 5. Hexbot Configuration

### Config location

`$HERMES_HOME/config.yaml` (defaults to `~/.hexbot/config.yaml` when `HERMES_HOME` is unset)
...
Restart the Hexbot session for changes to take effect. The MCP connection is
