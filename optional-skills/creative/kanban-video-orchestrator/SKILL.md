...
short to a music video to an ASCII loop — in a Hexbot Kanban pipeline that
...
3. **Generates** a setup script that creates Hexbot profiles, project workspace, and the initial kanban task
...
For mapping role → which Hexbot skills + toolsets it loads, see
...
3. Creates each Hexbot profile via `hexbot core profile create --clone`
...
7. Fires the initial `hexbot core kanban create` task assigned to the director
...
```bash
hexbot core kanban watch --tenant <project-tenant>     # live events
hexbot core kanban list  --tenant <project-tenant>     # board snapshot
hexbot core dashboard                                   # visual board UI
```
...
6. **The director never executes.** Even with the full `kanban + terminal +
   ...
   a `hexbot core kanban create` call to a specialist profile. The kanban
...
8. **Verify API keys BEFORE firing.** External APIs (TTS, image-gen,
   image-to-video) need keys in `${HERMES_HOME:-~/.hexbot}/.env` or the user's secret store.
