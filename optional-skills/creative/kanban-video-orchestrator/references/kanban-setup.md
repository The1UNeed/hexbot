...
2. **Create profiles** — `hexbot core profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.hexbot/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `hexbot core kanban create` assigned to the director
...
```bash
hexbot core profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.hexbot/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.hexbot/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Hexbot
...
Each profile gets a `SOUL.md` at `~/.hexbot/profiles/<name>/SOUL.md` that
...
```bash
hexbot core kanban create "Direct production of <video title>" \
    --assignee director \
...
the Hexbot `.env` (`${HERMES_HOME:-$HOME/.hexbot}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _hermes_env="${HERMES_HOME:-$HOME/.hexbot}/.env"
