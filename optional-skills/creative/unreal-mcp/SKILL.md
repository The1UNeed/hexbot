...
Companion skill for the `unreal-engine` entry in the Hexbot MCP catalog. The
...
Two halves, in this order: the editor side must be up before Hexbot connects.
...
### One-time, Hexbot side

    hexbot core mcp install unreal-engine
...
`~/.hexbot/config.yaml` under `mcp_servers.unreal-engine` to match.

Do NOT use `ModelContextProtocol.GenerateClientConfig` for Hexbot — that
writes `.mcp.json`-style files for Claude Code/Cursor/etc. Hexbot connects
...
2. Start the Hexbot session. Tools register as `mcp_unreal_engine_*`. If
...
three meta-tools, and every real tool is reached through them. Through Hexbot
they appear as:
...
then happens at `hexbot core mcp install`/`configure` time instead. Tool-search
...
3. **NEVER issue overlapping calls.** Do not batch multiple
   `mcp_unreal_engine_*` calls in one turn — Hexbot runs batched calls
...
- **Start order matters.** Editor + server up first, then the Hexbot
...
- **Timeouts on long operations.** Hexbot's per-call default is 120 s; asset
  imports, big level saves, and renders can exceed it. Raise
  `mcp_servers.unreal-engine.timeout` in `~/.hexbot/config.yaml` for
