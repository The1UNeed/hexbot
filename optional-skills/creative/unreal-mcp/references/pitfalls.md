...
### 1. Start order: editor first, Hexbot second

Hexbot probes MCP servers at session start. If the editor (and its server)
...
`LogModelContextProtocol` with the address), then open a NEW Hexbot session.
...
restart the editor, restart the Hexbot session.
...
server fails to bind (Output Log) or Hexbot's probe times out. Fix: change
Server Port Number in Editor Preferences > Model Context Protocol AND the
`url` in `~/.hexbot/config.yaml` (`mcp_servers.unreal-engine`), then restart
...
### 5. GenerateClientConfig is not for Hexbot

`ModelContextProtocol.GenerateClientConfig` writes config files for Claude
Code/Cursor/VSCode/Gemini/Codex into the project root. Hexbot's connection
lives in `~/.hexbot/config.yaml` via `hexbot core mcp install unreal-engine`.
Running GenerateClientConfig neither helps nor harms Hexbot — just don't
mistake it for the Hexbot setup step.
...
explicitly warns against overlapping calls. Hexbot executes same-turn tool
...
### 9. Timeouts: Hexbot gives up before Unreal does

Hexbot's default per-call timeout is 120 s. Asset imports, first-shader
...
`<Project>/Saved/Screenshots/...`). If Hexbot runs elsewhere (SSH backend,
