...
import upstream classes into Hexbot itself.
...
```bash
mkdir -p ~/.hexbot/cache/darwinian-evolver && cd ~/.hexbot/cache/darwinian-evolver
...
```bash
cd ~/.hexbot/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.hexbot/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.hexbot/skills/research/darwinian-evolver
DE_DIR=~/.hexbot/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Hexbot core.
   Custom driver scripts under `~/.hexbot/skills/...` are user-side and fine.
