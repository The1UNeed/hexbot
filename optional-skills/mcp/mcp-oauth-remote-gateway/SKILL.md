...
# MCP OAuth on a Remote Hexbot Gateway

## Overview

Hexbot's built-in MCP OAuth client runs a one-shot HTTP listener on `127.0.0.1:<port>`
inside the Hexbot process and registers that loopback address as the OAuth
`redirect_uri`. That works perfectly for a local CLI on the user's own machine.
It breaks completely when Hexbot runs as a remote gateway (container, VPS,
messaging bot), because the user's browser resolves `127.0.0.1` to the user's own
laptop, not the remote container — so the authorization code never reaches Hexbot.

This skill does the OAuth dance by hand and writes the resulting tokens into the
exact files Hexbot's token storage expects, so a subsequent `/reload-mcp` finds
...
2. Hexbot is running as a **remote gateway** (container, VPS, Docker, managed service) — NOT a local CLI on the user's laptop.
...
Do NOT use this for:
- **Local CLI Hexbot** — just set `auth: oauth` in `mcp_servers.<name>` and `/reload-mcp`. The built-in flow opens a browser and captures the callback on localhost. Works perfectly.
...
laptop and fails to connect. The callback never reaches the Hexbot process, the
...
already cover the deployment. When Hexbot detects a remote session it prints two
...
Both require an interactive terminal to the Hexbot host. The rest of this skill
is for when there is NO interactive TTY — Hexbot running purely as a messaging
gateway/bot where `/reload-mcp` triggers the flow with nobody at a prompt.

## Preferred Front Door: the Hexbot Dashboard (try this BEFORE manual token surgery)

A remote Hexbot gateway often also runs the **dashboard** web UI as a SEPARATE
process (e.g. `hexbot core dashboard --host 0.0.0.0 --port <port>`; check with
`ps aux | grep 'hexbot core dashboard'`). It exposes a connector/MCP console —
...
Hexbot's `HermesTokenStorage` would have written, so on `/reload-mcp` Hexbot finds
cached tokens and skips the browser flow entirely.
...
uses these same env vars, so if Hexbot's own auto-detect says "headless", the
...
```json
{
  "client_name": "Hexbot (manual OAuth)",
...
### 8. Write tokens in Hexbot's exact schema
...
UA** — Cloudflare will 403 you even though Hexbot (which uses httpx) will succeed.
`scripts/diagnose-oauth-mcp.py` automates this smoke test.

### 11. Tell the user to run `/reload-mcp`

On reload, Hexbot sees `auth: oauth`, calls `HermesTokenStorage.get_tokens()`,
...
1. **Do not assume "headless" means "OAuth impossible."** The built-in flow works fine for local CLI; the issue is strictly remote deployments where the user's browser and the Hexbot process are on different machines. Check the execution environment before claiming OAuth isn't an option.
...
11. **Client info file is NOT optional.** Hexbot needs `<server>.client.json` to know the `client_id` for refresh grants. Skipping it means the first refresh fails and the user has to re-auth — writing both files is the whole point of this skill.
...
- **Don't use `mcp-remote` as a fallback.** It runs an npx subprocess whose OAuth callback server ALSO sits on the remote container's localhost — same problem. `mcp-remote` only helps when the MCP client doesn't speak remote HTTP at all (Hexbot does natively).
...
- **Don't claim Hexbot doesn't support a feature without reading the source.** Grep the source tree before making capability claims.
...
- `native-mcp` — general guide to configuring MCP in Hexbot. Authoritative config reference lives there.
- `mcporter` — the external CLI bridge, for ad-hoc MCP calls outside of Hexbot's config.
