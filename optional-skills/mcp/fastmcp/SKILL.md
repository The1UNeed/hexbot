...
- smoke-test a server with the FastMCP CLI before wiring it into Hexbot or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Hexbot. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.hexbot/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.hexbot/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Hexbot integration, either:

- configure the server in `~/.hexbot/config.yaml` using the `native-mcp` skill, or
...
### Hexbot cannot see the deployed server

The server-building part may be correct while the Hexbot config is not. Load the `native-mcp` skill and configure the server in `~/.hexbot/config.yaml`, then restart Hexbot.
