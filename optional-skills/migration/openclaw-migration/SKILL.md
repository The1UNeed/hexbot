...
description: Import an OpenClaw setup (memories, skills) into Hexbot.
version: 1.0.0
...
metadata:
  hermes:
    tags: [Migration, OpenClaw, Hexbot, Memory, Persona, Import]
    related_skills: [hermes-agent]
---

# OpenClaw -> Hexbot Migration

Use this skill when a user wants to move their OpenClaw setup into Hexbot with minimal manual cleanup.
...
```bash
hexbot core claw migrate              # Full interactive migration
hexbot core claw migrate --dry-run    # Preview what would be migrated
hexbot core claw migrate --preset user-data   # Migrate without secrets
hexbot core claw migrate --overwrite  # Overwrite existing conflicts
hexbot core claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `hexbot core setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hermes.py` to:

- import `SOUL.md` into the Hexbot home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Hexbot memory entries
- merge OpenClaw command approval patterns into Hexbot `command_allowlist`
- migrate Hexbot-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Hexbot working-directory configuration
- copy OpenClaw skills into `~/.hexbot/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Hexbot workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hexbot/tts/`
- archive non-secret docs that do not have a direct Hexbot destination
...
- `~/.hexbot/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py`

Do not guess a shorter path like `~/.hexbot/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.hexbot/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Hexbot-compatible secrets, currently:
...
Hexbot CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Hexbot. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Hexbot workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Hexbot skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python ~/.hexbot/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py
```
...
```bash
python ~/.hexbot/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --preset user-data
```

Execute a user-data migration:

```bash
python ~/.hexbot/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python ~/.hexbot/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python ~/.hexbot/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Hexbot unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Hexbot targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Hexbot destination. Unsupported auth blobs must still be reported as skipped.
...
- Hexbot persona state imported
- Hexbot memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.hexbot/skills/openclaw-imports/`
