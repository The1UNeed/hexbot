...
Let a Hexbot agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where Hexbot runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `hexbot core meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `hexbot core meet` CLI |
...
| `cli.py` | `hexbot core meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/cli.py` | v3: `hexbot core meet node {run,list,approve,remove,status,ping}` |
| `SKILL.md` | agent usage guide |

## Local quick start

```bash
hexbot core plugins enable google_meet
hexbot core meet install                                      # pip + Chromium
hexbot core meet setup                                        # preflight
hexbot core meet auth                                         # optional
hexbot core meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
hexbot core meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.hexbot/.env
hexbot core meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
hexbot core meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
hexbot core meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.hexbot/.env
hexbot core meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, Hexbot will **not** switch your system audio input automatically — the
...
hexbot core plugins enable google_meet
hexbot core meet node run --display-name my-mac --host 0.0.0.0 --port 18789
# prints the bearer token on first run; copy it
```

On the gateway:
```bash
hexbot core meet node approve my-mac ws://<mac-ip>:18789 <token>
hexbot core meet node ping my-mac
