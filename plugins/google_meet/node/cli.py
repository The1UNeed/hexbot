"""`hexbot core meet node ...` subcommand tree.

Wired into the existing ``hexbot core meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``hexbot core meet node`` argparse object — typically
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``hexbot core meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             hexbot core meet node approve <name> ws://<host>:{args.port} {token}")
        try:
