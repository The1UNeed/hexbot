...
hexbot core meet install                 # pip deps + Chromium (transcribe only)
hexbot core meet install --realtime      # + pulseaudio-utils / brew blackhole+ffmpeg
hexbot core meet auth                    # optional; skips guest-lobby wait
hexbot core meet setup                   # preflight checks
```

`hexbot core meet install --realtime` prompts before running `sudo apt-get` (Linux)
...
#   Then set OPENAI_API_KEY or HERMES_MEET_REALTIME_KEY in ~/.hexbot/.env
```
...
pip install playwright websockets && python -m playwright install chromium
hexbot core plugins enable google_meet
hexbot core meet node run --display-name my-mac    # persistent server
# copy the printed token

# on the gateway:
hexbot core meet node approve my-mac ws://<mac-ip>:18789 <token>
hexbot core meet node ping my-mac                   # confirm reachable
```

Run `hexbot core meet setup` to preflight local prereqs.

## Flow

1. **Join** — call `meet_join(url=..., mode=..., node=...)`. Returns immediately.
2. **Announce yourself** — no auto-consent. Say (in whatever channel the user is watching): "A Hexbot agent bot is in this call taking notes."
...
- Guest mode sits in the lobby until a host admits. Warn the user; `hexbot core meet auth` avoids this.
...
- Remote nodes use bearer-token auth; tokens are generated on the node (32 hex chars, persisted in `$HERMES_HOME/workspace/meetings/node_token.json`) and must be copied to the gateway via `hexbot core meet node approve`.
