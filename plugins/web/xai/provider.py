...
``{title, url, description, position}`` rows every other Hexbot web
provider produces.
...
prefers Hexbot-managed xAI Grok OAuth (via ``hexbot core auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.hexbot/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``hexbot core tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `hexbot core auth` to sign in with "
