...
A standards-compliant OpenID Connect Relying Party for the ``hexbot core dashboard``
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          ...
          # credential — prefer the env var / ~/.hexbot/.env over config.yaml.
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Hexbot does not call
...
def register(ctx) -> None:
    ...
    # canonical home is the env var / ~/.hexbot/.env; config.yaml is supported
