...
# ``model_tools`` import time, so ``hexbot core status``, ``hexbot core chat``, etc.
...
#     fresh "Hexbot is thinking…" card during the small window between
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # they don't sit in the chat forever as "Hexbot is thinking…".
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Hexbot is thinking…' marker message.
        ...
        "Hexbot is thinking…" stuck in chat forever, plus one card that
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Hexbot is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Hexbot is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Hexbot is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``hexbot core setup``.
    ...
    prompt for env vars, persist them to ``~/.hexbot/.env`` so the next
    ...
    print()
    print_success("Google Chat configuration saved to ~/.hexbot/.env")
    print_info("Restart the gateway: hexbot core gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``hexbot core cron`` running as a
    separate process from ``hexbot core gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Hexbot plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="Run `hexbot core setup` to install Google Chat support.",
        ...
        # the "Hexbot is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Hexbot is thinking…' marker message "
