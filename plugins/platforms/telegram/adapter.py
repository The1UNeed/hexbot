...
class TelegramAdapter(BasePlatformAdapter):
    ...
    def _thread_kwargs_for_send(
        ...
        metadata. Hexbot-created private-chat topic lanes are marked with
        ...
        queued follow-ups after a gateway restart) prefer the Hexbot topic's
        ``message_thread_id`` so they stay in the active topic lane (#87051);
        ``direct_messages_topic_id`` is only used when no topic thread
        resolves, since the native DM-topic id does not match the Hexbot
        ...
        if metadata and metadata.get("telegram_dm_topic_reply_fallback"):
            ...
            if reply_to_message_id is None:
                ...
                # active topic lane: prefer the Hexbot topic thread id when it
    ...
    # older PTB/clients. Streaming edits stay on Hexbot's existing MarkdownV2
    ...
    def _has_telegram_desktop_details_math_crash_shape(self, content: str) -> bool:
        ...
        Hexbot must skip rich delivery up front and use the legacy MarkdownV2
    ...
    async def _handle_polling_conflict(self, error: Exception) -> None:
        ...
        # has been killed (e.g. during `hexbot core update` or `--replace` handoffs)
        ...
        message = (
            ...
            "To recover: ensure no other Hexbot or OpenClaw instance is running "
            "with this token, then restart the gateway with 'hexbot core gateway restart'."
    ...
    async def _run_post_connect_housekeeping(self) -> None:
        ...
        try:
            ...
            try:
                ...
                # payload size limit (~4KB total).  Hexbot defaults to 60 to
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # readable by the Hexbot process (shared mount, same machine, etc.).
    ...
    def _build_provider_keyboard(self, providers: list, page: int = 0) -> tuple:
        ...
        CLI ``hexbot core model`` picker via the shared ``group_providers`` fold,
    ...
    async def _handle_inline_query(
        ...
        Hexbot default), so most skill commands can never appear in the
    ...
    # Scripts live in ~/.hexbot/scripts/gmail-triage/. `arg` from the callback
    ...
    def _explicit_bot_mentions_exclude_self(self, message: Message) -> bool:
        ...
        Telegram groups can contain several Hexbot bot profiles. A message like
    ...
    def _is_own_message(self, message: Message) -> bool:
        ...
        counted as incoming unread messages in the Hexbot inbox.
        """
        if not self._bot:
            return False
    ...
    def _should_process_message(self, message: Message, *, is_command: bool = False) -> bool:
        ...
        # unread in the Hexbot inbox (#52363).
        #
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="Run `hexbot core setup` to install Telegram support.",
