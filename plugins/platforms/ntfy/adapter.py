"""ntfy platform adapter (Hexbot plugin).
...
``plugins/platforms/ntfy/``. The Hexbot plugin loader scans the
...
async def _standalone_send(
    ...
    *,
    ...
    ``hexbot core cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Hexbot dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `hexbot core gateway status` without
