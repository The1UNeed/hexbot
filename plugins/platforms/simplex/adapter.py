"""SimpleX Chat platform adapter (Hexbot plugin).
...
This adapter ships as a Hexbot platform plugin under
``plugins/platforms/simplex/``. The Hexbot plugin loader scans the
...
discoverable and ``hexbot core setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Hexbot — no
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``hexbot core setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.hexbot/.env`` via
    ``hermes_cli.config``.
    """
    print()
