"""
IRC Platform Adapter for Hexbot.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the Hexbot agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :Hexbot")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :Hexbot shutting down")
...
def check_requirements() -> bool:
    ...
    # hexbot core setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `hexbot core gateway setup` flow for the IRC platform.
    ...
    print_info("Connect Hexbot to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #hexbot — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.hexbot/.env")
...
async def _standalone_send(
    ...
    *,
    ...
    try:
        ...
        await _raw(f"USER {standalone_nick} 0 * :Hexbot (cron)")
