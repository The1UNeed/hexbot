"""
LINE Messaging API platform adapter for Hexbot.
...
to Hexbot. It deliberately ports the *strongest* idea from each into
...
def is_connected(config) -> bool:
    """Surface in ``hexbot core status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``hexbot core status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``hexbot core setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.hexbot/.env`` via ``hermes_cli.config``.
    """
    print()
    ...
    try:
        ...
        print("hermes_cli.config not available; set LINE_* vars manually in ~/.hexbot/.env")
        return
