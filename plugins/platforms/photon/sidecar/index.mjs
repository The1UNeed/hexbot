// Hexbot — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Hexbot): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Hexbot -> gRPC): `/send` drives `space.send(...)`; `/typing`
...
//                          default off — toggle with `hexbot core photon telemetry`)
  ...
  streamRestartTimer = setTimeout(() => {
    ...
    console.error(
      `photon-sidecar: upstream stream degraded for ${degradedForMs}ms; ` +
        "exiting so Hexbot can restart the Photon adapter"
    );
    process.exit(75);
...
// instead of a cryptic module-resolution error during import. Apply Hexbot's
...
try {
  ...
  console.error(
    "photon-sidecar: spectrum mixed attachment patch failed. " +
      "Run `hexbot core photon install-sidecar` or " +
...
try {
  ...
  console.error(
    "photon-sidecar: spectrum-ts is not installed. Run " +
      "`hexbot core photon install-sidecar`. Original error: " +
      (e && e.stack ? e.stack : String(e))
  );
  process.exit(3);
}
...
function inboundStreamErrorMessage(e) {
  ...
  // iMessage internal error. Local Hexbot allowlists cannot cause or fix this:
  ...
  if (
    ...
    out +=
      ...
      "this is upstream of Hexbot, so inbound iMessages may not be delivered " +
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Hexbot supplied them so a known-good
