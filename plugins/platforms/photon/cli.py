"""
``hexbot core photon ...`` CLI subcommands — registered by the plugin via
...
there is no standalone ``login`` verb (matching how every other Hexbot
...
# wiring imports this module for `hexbot core --help`).
...
# when called later (gateway start, hexbot core status). Cleared on success.
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire up `hexbot core photon ...` subcommands."""
...
def _run_device_login(args: argparse.Namespace) -> int:
    ...
    no standalone ``hexbot core photon login`` command; Photon onboards
...
def _cmd_setup(args: argparse.Namespace) -> int:
    ...
    # 2. Find or create the default project.
    ...
    # 3. Provision Spectrum credentials (runtime -> ~/.hexbot/.env,
    ...
    print("  Start the gateway:  hexbot core gateway start")
    return 0
...
def _cmd_status(_args: argparse.Namespace) -> int:
    ...
    print(f"  sidecar deps        : {'✓ installed' if sidecar_installed else '✗ run `hexbot core photon install-sidecar`'}")
    print(f"  telemetry           : {'on' if _telemetry_enabled() else 'off'} (`hexbot core photon telemetry on|off`)")
    return 0
...
def _telemetry_enabled() -> bool:
    """Read PHOTON_TELEMETRY from the env / ~/.hexbot/.env.
...
def _cmd_telemetry(args: argparse.Namespace) -> int:
    ...
    if state is None:
        ...
        print("  Toggle with `hexbot core photon telemetry on` / `hexbot core photon telemetry off`.")
        return 0
    try:
        ...
        return 1
    print(f"✓ Spectrum telemetry turned {state} (PHOTON_TELEMETRY in ~/.hexbot/.env)")
    print("  Restart the gateway for the sidecar to pick it up:  hexbot core gateway restart")
    return 0
...
#
# `hexbot core gateway setup` discovers platforms via the registry and calls each
...
# project + user + sidecar flow as ``hexbot core photon setup`` with interactive
# defaults (phone is prompted when stdin is a TTY).

def gateway_setup() -> None:
    """Run Photon first-time setup from the `hexbot core gateway setup` wizard."""
