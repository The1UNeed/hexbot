...
Credential storage mirrors every other Hexbot channel:

    * runtime SDK creds  -> ``~/.hexbot/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.hexbot/auth.json`` under
...
# registers Hexbot as its own client_id.
...
# Default name of the project Hexbot provisions for the operator.
DEFAULT_PROJECT_NAME = "Hermes Agent"  # matched by name on re-setup; keep for existing installs
...
def _auth_json_path() -> Path:
    """Resolve ``~/.hexbot/auth.json`` honouring the active Hexbot profile."""
    try:
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.hexbot/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.hexbot/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.hexbot/.env`` (canonical runtime store).
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `hexbot core photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `hexbot core photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `hexbot core photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `hexbot core photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `hexbot core photon setup`)"
