# A2A — Agent-to-Agent protocol for Hexbot
...
peer (another Hexbot, LangChain, CrewAI, Google ADK, OpenClaw, …). Stdlib only —
no `a2a-sdk` dependency.

## Enable

```bash
hexbot core gateway setup      # pick A2A, or:
```
...
When the `a2a` platform is enabled, Hexbot serves a v1.0 Agent Card at
...
- Every exchange is logged to `~/.hexbot/a2a_audit.jsonl`.
- Conversations persist to `~/.hexbot/a2a_conversations/` — they survive context
