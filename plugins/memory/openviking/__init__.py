...
def _derive_openviking_user_text(content: Any) -> str:
    """Strip Hexbot slash-skill scaffolding before sending content to OpenViking.
...
def _normalize_openviking_url(url: str) -> str:
    ...
    # could attach Hexbot to an unrelated deployment and forward credentials to
    ...
    try:
        ...
        raise _OpenVikingEndpointError(
            "OpenViking endpoint safety validation failed; Hexbot refused the connection."
        ) from exc
...
def _start_local_openviking_server(endpoint: str) -> tuple[str, str]:
    ...
    if _local_openviking_port_is_open(host, port):
        ...
        return (
            ...
            f"Port {host}:{port} is occupied by {listener}. Hexbot did not start "
    ...
    try:
        ...
        # The Hexbot Desktop backend can include the Hexbot venv's
        # site-packages in PYTHONPATH. If inherited, openviking-server would
        # import aiohttp and friends from the Hexbot venv instead of its own
        # (its venv's site-packages are shadowed because PYTHONPATH precedes
        # them) —
        # and on Windows the loaded DLLs then lock the Hexbot venv,
        # aborting `hexbot core update` with access-denied on .pyd files.
...
def _runtime_openviking_timeout_message(endpoint: str) -> str:
    return (
        ...
        "OpenViking memory is temporarily unavailable; Hexbot will retry on a later access or when "
        "the config changes."
    )
...
def _print_openviking_ready(message: str, path: Optional[Path] = None) -> None:
    ...
    print("  Start a new Hexbot session to activate.\n")
...
def _run_create_profile_setup(
    *,
    ...
    save_choice = select(
        ...
        [
            ("Keep in Hexbot only", "write values only to Hexbot .env"),
    ...
    _print_openviking_ready("Connection saved to Hexbot .env.")
    return True
...
class OpenVikingMemoryProvider(MemoryProvider):
            ...
            try:
                ...
                if health_state == "unreachable":
                    ...
                    _emit_runtime_warning(
                        ...
                        "Hexbot will retry on a later access or when the config changes.",
                        warning_callback,
                    )
                    self._client = None
            except ImportError:
    ...
    def _ensure_client(self) -> Optional["_VikingClient"]:
        ...
        ``~/.hexbot/.env`` after startup never reach the live client and tools
    ...
    def _ensure_client_locked(self) -> Optional["_VikingClient"]:
        ...
        if health_state == "responded":
            logger.warning(
                "%s OpenViking memory is temporarily unavailable; Hexbot will retry on a "
    ...
    def _extract_current_turn_messages(
        ...
        """Slice the completed turn out of Hexbot's full canonical transcript."""
        if not messages:
            return []
    ...
    def _messages_to_openviking_batch(
        ...
        *,
        ...
        """Convert Hexbot canonical messages into OpenViking batch payloads."""
    ...
    def _tool_remember(self, args: dict) -> str:
        ...
        recovery_note = (
            ...
            "Hexbot. Otherwise, do not resubmit automatically; report the uncertain "
            "state to the user."
        )
        ...
        # otherwise alter the live Hexbot conversation session.
        try:
