...
```bash
hexbot core memory setup honcho   # configure Honcho directly (works on a fresh install)
hexbot core memory setup          # generic picker, choose Honcho from the list
```
...
Or manually:
```bash
hexbot core config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.hexbot/.env
```

> `hexbot core honcho setup` also works, but only **after** Honcho is the active
> memory provider — the `honcho` subcommand is registered for the active
> provider only. On a fresh install, use `hexbot core memory setup honcho`.
...
cycle (not per pass). Select a fast, inexpensive model under `hexbot core model`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$HERMES_HOME/honcho.json` | Profile-local (isolated Hexbot instances) |
| 2 | `~/.hexbot/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active Hexbot profile: `hermes` (default) or `hermes_<profile>`.
...
> **Deprecated:** `pinPeerName` is a legacy alias for `pinUserPeer`, still read for back-compat (`pinUserPeer` wins where both are set). `hexbot core honcho setup` migrates it onto `pinUserPeer` on touch and never writes it.
...
| 4 | `per-session` strategy | Hexbot session ID (`20260415_a3f2b1`) |
...
- **`per-directory`** — basename of `$PWD`. Running `hexbot core` in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — Hexbot session ID (timestamp + hex). Every `hexbot core` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple Hexbot profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Host key is derived from the active Hexbot profile: `hexbot core` (default) or `hermes_<profile>` (e.g. `hexbot core -p coder` -> host key `hermes_coder`). Older `hermes.<profile>` host blocks are still read for compatibility and are migrated when the CLI writes profile-scoped Honcho config.
...
|---------|-------------|
| `hexbot core memory setup honcho` | Configure Honcho directly — works on a fresh install |
| `hexbot core honcho setup` | Interactive setup wizard (only registered once Honcho is the active provider; redirects to `hexbot core memory setup`) |
| `hexbot core honcho status` | Show resolved config for active profile |
| `hexbot core honcho enable` / `disable` | Toggle Honcho for active profile |
| `hexbot core honcho mode <mode>` | Change recall or observation mode |
| `hexbot core honcho peer --user <name>` | Update user peer name |
| `hexbot core honcho peer --ai <name>` | Update AI peer name |
| `hexbot core honcho tokens --context <N>` | Set context token budget |
| `hexbot core honcho tokens --dialectic <N>` | Set dialectic max chars |
| `hexbot core honcho map <name>` | Map current directory to a session name |
| `hexbot core honcho sync` | Create host blocks for all Hexbot profiles |
