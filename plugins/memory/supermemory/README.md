...
```bash
hexbot core memory setup    # select "supermemory"
```

Or manually:

```bash
hexbot core config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.hexbot/.env
```
...
Before running `hexbot core memory setup`, add the local endpoint to
...
Then run `hexbot core memory setup` and enter the local server's API key. Configuring
...
`https://api.supermemory.ai`. Hexbot resolves it once and uses the same endpoint
...
it groups Hexbot-written memories into a dedicated "Hexbot" Space in the
...
Use `{identity}` in the `container_tag` to scope memories per Hexbot profile:
