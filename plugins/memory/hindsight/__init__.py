...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so Hexbot can degrade gracefully instead of repeatedly trying to start
    ...
    healthy and ``hexbot core memory status`` would stay green while the daemon
...
def _local_runtime_hint(reason: str | None) -> str:
    ...
    selected local_embedded without going through ``hexbot core memory setup`` — a
    ...
    if "no module named" in text and ("hindsight'" in text or 'hindsight"' in text
        ...
        return (
            ...
            f"{sys.executable} hindsight-all — or run 'hexbot core memory setup'. "
...
def _event_timestamp() -> str:
    """Return the configured Hexbot event time with an explicit UTC offset."""
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this Hexbot config."""
...
def _load_simple_env(path) -> dict[str, str]:
    ...
    values: dict[str, str] = {}
    # utf-8-sig, not plain utf-8: this is also used on the Hexbot .env during
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active Hexbot profile name (from agent_identity)
      {workspace} — Hexbot workspace name (from agent_workspace)
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between Hexbot and the User"
    ...
    def _offer_starter_template(self, mode: str, provider_config: dict, env_writes: dict) -> None:
        """Offer to seed the bank with a Hexbot starter template (best-effort)."""
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between Hexbot and the User"},
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        self._retain_context = self._config.get("retain_context", "conversation between Hexbot and the User")
        ...
        # Hexbot emits a "👁️ Hindsight — recalled N memories" status line so the
        ...
        if self._mode == "local_embedded":
            ...
            if hasattr(os, "geteuid") and os.geteuid() == 0:
                msg = (
                    ...
                    "memory daemon. Run Hexbot as a non-root user, or switch "
                    "to cloud / local_external mode via 'hexbot core memory setup'."
                )
                logger.warning(msg)
                # Surface to the terminal too — a daemon that never starts
                # would otherwise fail silently and the user would only see
                # Hexbot get sluggish. (issue #13125)
                try:
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When Hexbot created/used that client on the shared async loop,
