...
```bash
hexbot core memory setup    # select "hindsight"
```
...
```bash
hexbot core config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.hexbot/.env
```
...
Hexbot spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.hexbot/logs/hindsight-embed.log`
...
Config file: `~/.hexbot/hindsight/config.json`
...
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `hermes-{profile}` isolates memory per active Hexbot profile. Empty placeholders collapse cleanly (e.g. `hermes-{user}` with no user becomes `hexbot core`). |
...
| `retain_context` | `conversation between Hexbot and the User` | Context label for retained memories |
...
The LLM API key is stored in `~/.hexbot/.env` as `HINDSIGHT_LLM_API_KEY`.
