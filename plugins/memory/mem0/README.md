...
```bash
hexbot core memory setup    # select "mem0"
```

Or manually:
```bash
hexbot core config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.hexbot/.env
```

## Config

Behavioral settings live in `$HERMES_HOME/mem0.json` (set them via `hexbot core memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.hexbot/.env`.
...
2. Point the plugin at it — via the setup wizard:
   ```bash
   hexbot core memory setup    # select "mem0" → "Self-hosted server"
   # Or non-interactive:
   hexbot core memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
   ```
   or via env vars:
   ```bash
   echo "MEM0_HOST=http://localhost:8888" >> ~/.hexbot/.env
   echo "MEM0_API_KEY=your-admin-api-key" >> ~/.hexbot/.env
   ```
   ...
   ```
3. Start a fresh Hexbot session and call `mem0_search` — it connects to your server.
...
```bash
hexbot core memory setup mem0 --mode oss \
...
```bash
hexbot core memory setup mem0 --mode oss --oss-llm-key sk-...
```
...
```json
{
  ...
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.hexbot/mem0_qdrant"}}
  }
}
```
...
```bash
hexbot core memory setup mem0 --mode platform --api-key sk-...
```

### Dry Run (preview without writing)

```bash
hexbot core memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```
