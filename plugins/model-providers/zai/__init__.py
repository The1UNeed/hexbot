...
request omits ``thinking``.  Hexbot's ``reasoning_config = {"enabled": False}``
...
:meth:`ZaiProfile.build_api_kwargs_extras` translates the Hexbot reasoning
...
(per Z.AI / BigModel docs).  Hexbot's richer effort scale is collapsed onto
...
def _glm_5_2_reasoning_effort(
    ...
    """Map Hexbot reasoning effort onto GLM's native vocabulary.
