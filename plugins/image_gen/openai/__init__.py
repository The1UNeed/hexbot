...
three virtual model IDs so the ``hexbot core tools`` model picker and the
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "OPENAI_API_KEY not set. Run `hexbot core tools` → Image "
                    "Generation → OpenAI to configure, or `hexbot core setup` "
                    "to add the key."
                ),
