...
models show up in ``hexbot core tools`` automatically. No model ids are
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key:
            return error_response(
                error=(
                    "DEEPINFRA_API_KEY not set. Run `hexbot core tools` → Image "
                    "Generation → DeepInfra to configure, or `hexbot core setup` "
                    "to add the key."
                ),
