...
# Hexbot uses 3 abstract aspect ratios. Map to Krea's enum (which is wider).
        ...
        **kwargs: Any,
        ...
        if managed is not None:
            ...
            if not auth_token:
                return error_response(
                    error=(
                        "KREA_API_KEY not set. Run `hexbot core tools` → Image "
            ...
            if managed is not None and 400 <= status < 500:
                hint = (
                    ...
                    if status == 429
                        ...
                        "`hexbot core tools` → Image Generation."
                    )
                )
                return error_response(
                    error=(
