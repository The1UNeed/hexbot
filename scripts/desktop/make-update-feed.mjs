...
const version = JSON.parse(await readFile(join(repositoryRoot, 'apps/desktop/package.json'), 'utf8')).version
const releaseDate = new Date().toISOString()

async function sha512(file) {
  const hash = createHash('sha512')
  await new Promise((done, fail) => createReadStream(file).on('data', chunk => hash.update(chunk)).on('end', done).on('error', fail))
...
function manifest(entries, primary) {
  ...
  for (const e of entries) lines.push(`  - url: ${e.url}`, `    sha512: ${e.sha512}`, `    size: ${e.size}`)
  lines.push(`path: ${primary.url}`, `sha512: ${primary.sha512}`, `releaseDate: '${releaseDate}'`, '')
  return lines.join('\n')
}
...
for (const arch of ['arm64', 'x64']) {
  ...
  if (!zip) { console.log(`mac/${arch}: no zip artifact, skipped`); continue }
...
if (linuxManifest) {
  ...
  for (const artifact of artifacts) {
    ...
    if (!(await stat(source).catch(() => undefined))?.isFile()) throw new Error(`latest-linux.yml references missing artifact: ${artifact}`)
    await cp(source, join(destination, basename(artifact)))
  }
  await cp(linuxYml, join(destination, 'latest-linux.yml'))
  console.log('Prepared linux/x64'); prepared++
} else {
