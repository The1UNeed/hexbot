...
- **Two arms, one variable.** `baseline` and `fixes` runs differ ONLY by
  ...
  Hexbot home, same model, same tasks, same reps.
...
1. Create a dedicated Hexbot home with credentials for the models under test:
   ...
   integration at it; no Hexbot observability plugin needs to be enabled.
