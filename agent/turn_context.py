...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``hexbot core logs``.
    ...
    if (
        ...
        # Hexbot only initiates compaction in "hermes" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping Hexbot preflight compression for codex app-server "
                "(mode=%s); Hexbot will not start thread compaction here.",
    ...
    if agent._memory_manager:
        ...
        # to surface it. Rendered by Hexbot (via _emit_status), so it always
