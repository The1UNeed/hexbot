"""OpenAI-shape bridge shared by Hexbot's ACP clients.

An ACP agent (``copilot --acp``, and the ACP CLIs that reach Hexbot as
...
the agent's *own* tool notifications. Hexbot's agentic surface — ``memory``,
...
* :func:`completion_to_stream_chunks` — re-shape a one-shot ACP response as
  ...
  is inherently one-shot from Hexbot's perspective).
...
of its own (Copilot) forwards everything Hexbot offers; a CLI that is an
autonomous agent with its own read/edit/execute tools must forward only Hexbot's
agent-level tools, because re-offering the overlapping ones makes Hexbot re-run
...
class StreamChunks(list):
    ...
    Hexbot reads provider-level extras off the object returned by
...
def tool_specs_from_openai_tools(
    ...
    *,
    ...
    name is in it survive — that is how a client forwards just Hexbot's
