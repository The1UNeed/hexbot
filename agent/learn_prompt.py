...
that instructs the live agent to:
  ...
  1. Gather the sources the user named, using the tools it already has
     ...
     material).
  2. Author a skill via ``skill_manage`` that follows the Hexbot
     skill-authoring standards (description <=60 chars, the modern section
     order, Hexbot-tool framing, no invented commands). Small sources get one
...
Follow the Hexbot skill-authoring standards exactly. These are the same
...
- version: 0.1.0
- author: always the literal value `Hexbot`. NEVER fill it from the host
  ...
  a privacy leak the user never opted into; the skill names itself as Hexbot.
...
4. "## How to Run" — the canonical invocation, framed through Hexbot tools.
...
Hexbot-tool framing (this is what makes it a skill, not shell docs):
- Frame running scripts as "invoke through the `terminal` tool".
- Reference Hexbot tools by name in backticks: `terminal`, `read_file`,
