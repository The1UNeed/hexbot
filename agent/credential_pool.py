...
# un-throttled log fires on *every* selection. On Windows several Hexbot
...
# readiness handshake ("Timed out connecting to Hexbot backend after
...
def custom_provider_pool_key_candidates(
    ...
    ``hexbot core auth add <key>`` stores new-style ``providers.<key>`` credentials
...
class CredentialPool:
    ...
    def _sync_anthropic_entry_from_pool_store(
        ...
        (``~/.hexbot/auth.json`` / profile equivalent), so it works for
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``hexbot core model`` / ``hexbot core auth`` which
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Hexbot process
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        ``hexbot core`` invocation that defaults to the active provider
        (e.g. setup wizard, ``hexbot core auth status``) would land on
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # sync→POST→write-back sequence below must run atomically across Hexbot
        ...
        # Previously "anthropic" was excluded from this tuple: two Hexbot
        ...
        if self.provider in ("openai-codex", "xai-oauth", "anthropic"):
            ...
            with _auth_store_lock(
                ...
                if self.provider == "anthropic" and synced.source == "claude_code":
                    ...
                    # (or macOS Keychain) that every Hexbot profile's pool
    ...
    def _claude_code_credentials_lock(self):
        ...
        serializes every profile (and every Hexbot process) that might
    ...
    def _refresh_entry_impl(
        ...
        try:
            if self.provider == "anthropic":
                ...
                # _seed_from_singletons() reads ~/.hexbot/.anthropic_oauth.json
                ...
                # a second authority for the same refresh-token family.
                    ...
                    try:
                        ...
                        return self._fail_closed_unpersisted_rotation(
                            entry, wexc, store="~/.hexbot/.anthropic_oauth.json"
                        )
            elif self.provider == "openai-codex":
                # Adopt fresher tokens from auth.json before spending the
                # refresh_token — single-use tokens consumed by another Hexbot
            ...
            if self.provider == "xai-oauth":
                ...
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/nous — another Hexbot process
    ...
    def _available_entries(
        ...
        *pending_refresh* tuples so the caller can execute them outside the
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Hexbot profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `hexbot core model` / `hexbot core auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `hexbot core model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``hexbot core auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `hexbot core auth add %s`",
                            _label,
    ...
    def add_entry(self, entry: PooledCredential) -> PooledCredential:
        with self._lock:
            ...
            if borrowed_ids:
                # ``hexbot core -p <profile> auth add <single-use provider>``: the
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `hexbot core auth remove <provider> <N>` is stable across all source types.
    try:
    ...
    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Hexbot PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `hexbot core setup` ("Claude
        ...
        # that masquerade.  Prefer ~/.hexbot/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `hexbot core setup` writes.
        _env_file = load_env()
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `hexbot core auth add nous --label <name>`).  Fall back to the
        ...
        try:
            ...
            # (hexbot core auth remove copilot gh_cli) must not pay either on
        ...
        # Hexbot auth store or env vars, so resolve them here.
        ...
        # MiniMax OAuth tokens live in ~/.hexbot/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `hexbot core auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `hexbot core auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Hexbot auth store.  Without this gate the removal is instantly
        ...
        # Hexbot owns its own Codex auth state — we do NOT auto-import from
        ...
        # via `hexbot core auth openai-codex`.
        ...
        # When the user logs in via ``hexbot core model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``hexbot core auth list`` reflects the logged-in state and so the pool
...
# Prefer ~/.hexbot/.env over os.environ — the user's config file is the
# authoritative source for Hexbot credentials. Stale env vars from parent
...
def _warn_env_ingestion_once(provider: str, env_var: str) -> None:
    ...
    logger.warning(
        ...
        "hexbot core auth remove %s <n> to suppress.",
        env_var,
        provider,
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Honour user suppression — `hexbot core auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.hexbot/.env.
    ...
    if provider == "openrouter":
        # Prefer ~/.hexbot/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `hexbot core auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Hexbot
