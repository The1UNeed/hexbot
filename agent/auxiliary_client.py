...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.hexbot/auth.json active provider)
    ...
    try:
        ...
        if not _WARNED_KEEPALIVE_IMPORT_SKEW:
            ...
            logger.warning(
                ...
                "back to the SDK default HTTP client. Run `hexbot core update` (or "
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Hexbot owns auxiliary retry + provider/model fallback policy (the
    ...
    # Hexbot sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Hexbot control the budget; explicit callers can still
    # override via kwargs.
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Hexbot triggers summarization.  Higher values delay
...
class _CodexCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        resp_kwargs: Dict[str, Any] = {
            # Strip the Hexbot-side ``-900k`` large-context picker suffix —
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        ...
        #   - ``_``-prefixed keys: private Hexbot plumbing (_reasoning_config
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate ~/.hexbot/auth.json for an active Nous provider.
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``hexbot core auth
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from Hexbot auth store.
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `hexbot core setup` or `hexbot core model` and enter a valid http(s) base URL."
        ) from exc
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``hexbot core config aux reset``)."""
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Hexbot's built-in discovery
...
def _resolve_auto_route(
    ...
    #    scenario where a user switches providers via `hexbot core model` but the
    #    old OPENAI_BASE_URL lingers in ~/.hexbot/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: hexbot core model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.hexbot/.env",
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    # See _create_openai_client: disable SDK-internal retries so Hexbot owns
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: hexbot core auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: hexbot core model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: hexbot core model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: hexbot core doctor for "
                "diagnostics)"
            )
            return None, None
...
def resolve_compression_fast_lane(
    ...
    *,
    ...
    route is the one Hexbot will call. A requested model covers a compressor
...
def _build_call_kwargs(
    ...
    # Anthropic Messages adapters translate Hexbot reasoning into native
...
def _is_managed_local_endpoint(base_url: Optional[str]) -> bool:
    """True when *base_url* targets the llama-server this Hexbot manages."""
    if not base_url:
        return False
...
def _call_llm_impl(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Hexbot reasoning config for direct model calls
