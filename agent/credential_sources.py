"""Unified removal contract for every credential source Hexbot reads from.

Hexbot seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.hexbot/.env
    claude_code   — ~/.claude/.credentials.json
    hermes_pkce   — ~/.hexbot/.anthropic_oauth.json
    ...
    manual        — user ran `hexbot core auth add`
...
unify here is **removal**:
    ...
    ``hexbot core auth remove <provider> <N>`` must make the pool entry stay gone.
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.hexbot/.env  → clear it
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to Hexbot.",
            f"  The pool entry is now suppressed — Hexbot will ignore "
            f"{env_var} until you run `hexbot core auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Hexbot stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `hexbot core auth add anthropic` to re-enable if needed.",
    ])


def _remove_hermes_pkce(provider: str, removed) -> RemovalResult:
    """~/.hexbot/.anthropic_oauth.json is ours — delete it outright."""
    ...
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Hexbot Anthropic OAuth credentials")
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    `hexbot core auth add nous` (or any other path that writes providers.nous)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `hexbot core auth add nous` to re-engage, which is the
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``hexbot core auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `hexbot core model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the Hexbot auth store is not enough — _seed_from_singletons() would
    ...
    via ``hexbot core auth add openai-codex``), but in both cases the re-seed
    ...
    result.hints.extend([
        ...
        "Run `hexbot core auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `hexbot core auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Hexbot stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `hexbot core auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        ...
        description="~/.hexbot/.anthropic_oauth.json",
    ))
    register(RemovalStep(
