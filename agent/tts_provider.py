...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Hexbot with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.hexbot/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Hexbot
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``hexbot core tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``hexbot core setup`` for
