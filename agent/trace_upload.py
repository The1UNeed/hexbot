"""Upload a Hexbot session transcript to Hugging Face as an agent trace.

Hexbot stores sessions in its own SQLite store (``hermes_state.SessionDB``),
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Hexbot's secret redactor (``force=True``) unless the
...
# ---------------------------------------------------------------------------
# Conversion: Hexbot OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Hexbot's shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Hexbot conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.hexbot/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `hexbot core trace upload`)."
)
