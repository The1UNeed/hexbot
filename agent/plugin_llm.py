...
Plugins built on Hexbot often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Hexbot
...
embedded slugs, no shorthands. This mirrors Hexbot's main config
...
handles every provider, fallback chain, and per-task override Hexbot
supports.
"""
