...
# cli.py) or `HERMES_REDACT_SECRETS=false` in ~/.hexbot/.env. An opt-out
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across Hexbot — used by
    ``hexbot core config``, ``hexbot core status``, ``hexbot core dump``, and anywhere
