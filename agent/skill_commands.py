...
#
# When a user invokes a /skill (or /bundle), Hexbot expands the turn into a
...
def _resolve_skill_commands_home() -> str:
    """Return the effective Hexbot home the skill scan should be scoped to.
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.hexbot/skills/ and return a mapping of /command -> skill info.
        ...
        for scan_dir in dirs_to_scan:
            ...
            for skill_md in _iter:
                ...
                try:
                    ...
                    # with a core Hexbot slash command (name or alias). The
                    ...
                    if resolve_command(cmd_name) is not None:
                        logger.warning(
                            ...
                            "collides with a core Hexbot command; skipping "
...
def get_skill_commands() -> Dict[str, Dict[str, Any]]:
    ...
    active profile's Hexbot home changes (e.g. Desktop switching profiles
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.hexbot/skills/`` and any ``skills.external_dirs`` so the
...
def build_preloaded_skills_prompt(
    ...
    bundle-invocation gate (#59156). Without this, ``hexbot core -s <skill>`` or
