...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: str) -> None:
    ...
    mechanism as the ~/.hexbot/prefill.json incident, 20/20 blocked with
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Hexbot tool use: "
    ...
    return (
        ...
        f"context, but Hexbot needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Hexbot config, set `model.ollama_num_ctx: 65536` "
    ...
    if (provider or "").strip().lower() == "anthropic":
        ...
        if unverified:
            lines = [
                ...
                "Retry with a fresh credential state: `hexbot core auth reset anthropic`. Until "
...
def _stored_prompt_matches_runtime(agent, prompt: str) -> bool:
    ...
    def host_info_value(label: str) -> str:
        ...
        occurrence, so only Hexbot's own emitted block can satisfy the read.
        """
        prefix = f"{label}:"
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `hexbot core fallback add`."
)
...
def run_conversation(
    ...
    # Adopt any ~/.hexbot/.env credential/base-url edits made since the last
    ...
    # all run inside Codex). Default Hexbot path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # cache prefix.  The system prompt is reserved for Hexbot internals.
        #
        # Hexbot invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Hexbot tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
            ...
            try:
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Hexbot/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if (
                    ...
                    print(f"{agent.log_prefix}     • Re-authenticate: hexbot core auth add nous")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `hexbot core doctor` for credential-chain diagnostics, or")
                    ...
                    from hermes_constants import display_hermes_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Hexbot-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: hexbot core config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: hexbot core config set ANTHROPIC_API_KEY \"\"")
                ...
                # and omits the field, and Hexbot's local compression takes
                ...
                if getattr(api_error, "status_code", None) == 404:
                    ...
                    if _suggestion:
                        ...
                        agent._buffer_vprint(
                            f"      Did you mean '{_suggestion}'?  Re-pick it with `hexbot core model`."
                        )
                ...
                # providers (DeepSeek, Kimi, MiMo, Hexbot).  A 429 can
                ...
                # which Hexbot's system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Hexbot's system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`hexbot",
                        force=True,
                    )
                    agent._vprint(
                        f"{agent.log_prefix}      core setup` → GitHub Copilot), or pick any other provider.",
                        force=True,
                    )
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # Hexbot may add API-only content not present in persisted
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                ...
                                agent._vprint(f"{agent.log_prefix}      2. Then run `hexbot core auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `hexbot core model`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   💡 Nous Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: hexbot core portal", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: hexbot core setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        hexbot core fallback add   (interactive picker — same as `hexbot core model`)",
                            force=True,
                        )
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.hexbot/config.yaml` to extend the per-call "
                            f"timeout. (Hexbot's built-in floor is 600s for "
        ...
        try:
            ...
            # with the iterations Hexbot never saw. Appended before this turn's
            ...
            if assistant_message.tool_calls:
                ...
                try:
                    ...
                    # terminates Hexbot mid-turn, resume logic still sees the
            ...
            if sys.is_finalizing() or _is_interpreter_shutdown_error(e):
                ...
                final_response = (
                    ...
                    "resumed with: hexbot core --resume <session-id>"
                )
