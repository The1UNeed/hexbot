...
# output items.  Hexbot injects xAI's native ``web_search`` for the xAI
...
# server-assigned base64 blobs that can run 400+ chars, while Hexbot-minted
...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    explicitly relies on Hexbot threading encrypted reasoning back across
    ...
    ``convert_messages``). Dropping the checkpoint costs nothing: Hexbot's
                ...
                if isinstance(codex_reasoning, list):
                    for ri in codex_reasoning:
                        if isinstance(ri, dict) and ri.get("encrypted_content"):
                            ...
                            # it is a Hexbot-side metadata key and not part
