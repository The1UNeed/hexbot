...
Data resolution order:
  ...
  2. Disk cache (~/.hexbot/models_dev_cache.json — any age; stale data is
...
# ---------------------------------------------------------------------------
# Provider ID mapping: Hexbot ↔ models.dev
# ---------------------------------------------------------------------------

# Hexbot provider names → models.dev provider IDs
PROVIDER_TO_MODELS_DEV: Dict[str, str] = {
    ...
    # under the "meta" provider id; Hexbot's provider is "meta-ai" (and the
...
# Reverse mapping: models.dev id → Hexbot ids (built lazily; many-to-one,
...
def _models_dev_to_hermes_ids(mdev_id: str) -> List[str]:
    """Return the Hexbot provider ids that map to *mdev_id* (may be [])."""
...
def fetch_models_dev(
    ...
    When ``force_refresh=True`` (used by ``hexbot core config refresh``, the
...
def _provider_override_section(provider: str) -> Optional[Dict[str, Any]]:
    ...
    Accepts either the Hexbot provider id or the models.dev provider id as
    ...
    # Reverse: caller passed a models.dev id, config keyed by Hexbot id.
...
def _get_provider_models(
    ...
    """Resolve a Hexbot provider ID to its models dict from models.dev.
...
def list_provider_models(
    ...
    picker (``hexbot core model``), which is an interactive user-facing flow
...
# Gemma models whose TPM quotas are too small for normal Hexbot agent traffic.
...
def get_provider_info(
    ...
    Accepts either a Hexbot provider ID (e.g. "kilocode") or a models.dev
    ...
    ``allow_network=False``.
    """
    # Resolve Hexbot ID → models.dev ID
...
def get_model_info(
    ...
    Accepts Hexbot or models.dev provider ID.  Tries exact match then
