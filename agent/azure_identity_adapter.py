...
* No persisted JWT. ``azure-identity`` caches in-process and (where
  available) in the OS keychain or ``~/.IdentityService``. Hexbot does
...
class EntraIdentityConfig:
    ...
    Captures the Hexbot-managed Entra knobs we need outside Azure SDK
...
def _build_default_credential(config: EntraIdentityConfig) -> Any:
    ...
    Only Hexbot-selected knobs are passed as kwargs. Everything else
    ...
    ``~/.hexbot/.env`` or the deployment environment.
    """
...
def build_credential(config: EntraIdentityConfig) -> Any:
    ...
    Hexbot processes use exactly one Entra config at a time (the
...
def has_azure_identity_credentials(scope: Optional[str] = None,
    ...
    error — never raises. Use for ``hexbot core doctor`` /
    ``hexbot core auth status`` / wizard preflight.
...
def describe_active_credential(config: Optional[EntraIdentityConfig] = None,
    ...
    Designed for ``hexbot core doctor`` and the wizard preflight — never
    ...
    def _inject_bearer(request: "httpx.Request") -> None:
        try:
            ...
            logger.warning(
                ...
                "Run `hexbot core doctor` or `az login` to recover.",
                exc,
            )
