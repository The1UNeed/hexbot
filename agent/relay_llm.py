"""Core NeMo Relay adapters for physical Hexbot provider attempts."""
...
def _relay_operation_name(provider_name: str, metadata: dict[str, Any] | None) -> str:
    """Return Relay's canonical operation name when Hexbot knows the API mode."""
...
def execute_current(
    ...
    *,
    ...
    """Run a provider attempt under the inherited Hexbot turn when present."""
...
def stream_current(
    ...
    *,
    ...
    """Run a provider stream under the inherited Hexbot turn when present.
...
class ManagedLlmStream(Iterator[Any]):
    """Drive Relay's async stream from Hexbot's provider worker thread."""
    ...
    def __init__(
        ...
        *,
        ...
        def run_callback(callback: Callable[..., Any], *args: Any) -> Any:
            ...
            def guarded() -> Any:
                # Hexbot-side callbacks run while the native pipeline drives
...
class AnthropicStreamAccumulator:
    ...
    def response(self, base: Any = None) -> Any:
        """Return the attribute-shaped response consumed by Hexbot."""
    ...
    *,
    ...
    with turn.finalize_lock:
        ...
        try:
            ...
            logger.warning(
                "Hexbot Relay logical LLM finalization failed",
                exc_info=True,
            )
            return
        with turn.logical_llm_lock:
