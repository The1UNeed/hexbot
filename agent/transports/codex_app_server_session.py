...
Owns one Codex thread per Hexbot session. Drives `turn/start`, consumes
...
# Hexbot's tools.terminal.security_mode → Codex's permissions profile id.
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Hexbot/OpenAI rich content into app-server text input.
...
class CodexAppServerSession:
    """One Codex thread per Hexbot session, lifetime owned by AIAgent.
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Hexbot's text path is the common case).
        try:
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Hexbot's approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # auto-accept — the user already approved Hexbot's tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        This is protocol-level routing only — it carries NO Hexbot
        approval-mode/timeout logic. The Hexbot-side resolution happens
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        Protocol-level routing only; Hexbot approval-mode/timeout
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Hexbot approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Hexbot returns 'once', 'session', 'always', or 'deny'.
    ...
    NOT in tools/approval.py: the Hexbot approval mode/timeout resolution
    ...
    # "prompt expired"; the Hexbot-side messaging already distinguishes them.
    return "decline"
...
def _get_hermes_version() -> str:
    """Best-effort Hexbot version string for codex's userAgent line."""
    try:
