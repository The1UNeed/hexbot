...
* Authentication is whatever the user's ``op`` CLI already uses — a
  ...
  or a desktop/interactive session (``OP_SESSION_*``).  Hexbot never
...
* Failures NEVER block startup.  A missing ``op`` binary, expired auth, a
  ...
  Hexbot continues with whatever credentials ``.env`` already had.
...
so back-to-back short-lived ``hexbot core`` invocations don't re-shell ``op`` for
...
def _op_child_env(token_value: str) -> Dict[str, str]:
    ...
    # configured Hexbot to source it from, so normalize to that name here.
    if token_value:
    ...
    def remediation(self, kind, cfg: dict) -> str:
        if kind in (ErrorKind.AUTH_FAILED, ErrorKind.AUTH_EXPIRED):
            ...
            if isinstance(cfg, dict):
                token_env = str(cfg.get("service_account_token_env") or token_env)
            return (
                "Run `hexbot core secrets onepassword token` to paste a fresh "
...
def clear_caches(home_path: Optional[Path] = None) -> None:
    ...
    Used after a token rotation (`hexbot core secrets onepassword token`) so
