...
def _fetch_with_timeout(
    ...
    an unbounded hang on every ``hexbot core`` invocation.
    """
...
def _active_profile_name(home_path: Optional[Path]) -> str:
    ...
    A named profile's HERMES_HOME is ``~/.hexbot/profiles/<name>``; the
    default profile (``~/.hexbot``) returns "".
    """
