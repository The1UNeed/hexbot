...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Hexbot starts and finds ``~/.openclaw/``.

    Points users at ``hexbot core claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``hexbot core claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to Hexbot, run "
        "`hexbot core claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `hexbot core claw cleanup` (renames it to ~/.openclaw.pre-migration — "
