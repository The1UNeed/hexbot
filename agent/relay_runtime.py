"""Profile-scoped NeMo Relay runtimes owned by the Hexbot agent core."""
...
def pop_relay_scope(
    ...
    *,
    ...
    'metadata'`` when Hexbot finalization forwards runtime metadata. Filter to
...
@dataclass
class RelaySession:
    """One isolated Relay scope stack owned by a Hexbot session."""
    ...
    def run_in_session(
        ...
        **kwargs: Any,
        ...
        with self._lock:
            ...
            if runtime is None:
                raise RuntimeError("Hexbot Relay operation lease is released")
...
class _ProcessRelayPluginConfiguration:
    ...
    def acquire(
        ...
        with self._lock:
            ...
            if self._active and not self._clear_active():
                logger.warning(
                    "Hexbot Relay plugin cleanup is still pending; refusing to "
            ...
            try:
                ...
                logger.warning(
                    "Hexbot could not determine whether a process-global Relay "
            ...
            if existing_report is not None:
                logger.warning(
                    ...
                    "outside Hexbot native ownership; leaving it unchanged and "
                    "disabling Hexbot-managed Relay middleware for this process"
                )
                return self._remember(
                    owner_id,
            ...
            try:
                ...
                if dynamic_plugins:
                    try:
                        ...
                        raise RuntimeError(
                            "Hexbot Relay dynamic plugin activation failed"
                        ) from exc

                if self._activation is None:
                    # Hexbot only enters Relay's initialization path after an
                ...
                logger.warning(
                    "Hexbot Relay plugin initialization failed: %s",
                    exc,
                    exc_info=True,
                )
                return self._remember(
    ...
    def _clear_active(self) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Hexbot Relay plugin subscriber flush failed",
                exc_info=True,
            )
            return False
        try:
            ...
            logger.warning(
                "Hexbot Relay plugin configuration cleanup failed",
                exc_info=True,
            )
            return False
        self._active = False
...
class RelayRuntime:
    ...
    def managed_execution_enabled(self) -> bool:
        """Return whether a Hexbot-managed consumer needs the Relay pipeline."""
        with self._execution_consumers_lock:
    ...
    def rotate_session_scope(self, session: RelaySession, *, reason: str) -> None:
        ...
        with session.lock:
            ...
            try:
                ...
                logger.warning(
                    "Hexbot Relay segment close failed (session=%s segment=%d); "
            ...
            try:
                ...
                logger.warning(
                    "Hexbot Relay segment open failed (session=%s segment=%d); "
    ...
    def get_session(self, session_id: str) -> RelaySession | None:
        """Return an active Hexbot Relay session without creating one."""
        with self._sessions_lock:
            if self._closing:
                return None
    ...
    def get_session_handle(self, session_id: str) -> Any:
        """Return the Relay parent handle for a Hexbot session, if active."""
        ...
        **kwargs: Any,
        ...
        with session.lock:
            if session.closing and not allow_closing:
                raise RuntimeError("Hexbot Relay session is closing")
            if session.context is None or session.handle is None:
                raise RuntimeError("Hexbot Relay session context is unavailable")
    ...
    def _begin_operation(self) -> None:
        ...
        with self._sessions_lock:
            if self._closing:
                raise RuntimeError("Hexbot Relay runtime is shutting down")
    ...
    def emit_mark(
        ...
        *,
        ...
        """Emit a mark parented to the Hexbot session identified by ``event``."""
    ...
    def apply_tool_request_intercepts(
        ...
        *,
        ...
        """Apply Relay request rewriting before Hexbot authorizes a tool call."""
        ...
        def close_with_drain() -> None:
            ...
            for _ in range(drain_limit):
                ...
                try:
                    ...
                    logger.warning(
                        "Hexbot Relay orphaned scope drain failed",
                        exc_info=True,
                    )
                    break
            ...
            if drained_holder["count"]:
                logger.warning(
                    "Hexbot Relay drained %d orphaned scope(s) before closing %s",
    ...
    def _close_session(self, event: dict[str, Any]) -> None:
        ...
        if failures:
            logger.warning(
                "Hexbot Relay session %s closed with errors: %s",
                session_id,
                "; ".join(failures),
            )
        ...
        if has_active_operations:
            ...
            try:
                ...
                with self._sessions_lock:
                    self._shutdown_started = False
                logger.warning(
                    "Hexbot Relay deferred shutdown could not start",
                    exc_info=True,
                )
            return
    ...
    def _finish_shutdown(self) -> None:
        try:
            ...
            with self._sessions_lock:
                self._shutdown_started = False
            logger.warning("Hexbot Relay shutdown failed", exc_info=True)
            return
        with self._sessions_lock:
    ...
    def _safe(callback: Callable[..., Any], *args: Any, **kwargs: Any) -> Any:
        try:
            ...
            logger.warning("Hexbot Relay runtime operation failed", exc_info=True)
            return None
...
class RelayHostRegistry:
    """Own exactly one Relay host for each canonical Hexbot profile."""
        ...
        *,
        ...
        with self._lock:
            ...
            try:
                ...
                logger.warning(
                    "Hexbot Relay runtime initialization failed", exc_info=True
                )
...
class RelayTurnContext:
    """Runtime-only context for one Hexbot turn or top-level task."""
...
# Set >0 while the native Relay pipeline is mid-dispatch of a Hexbot callback
...
class RelaySessionCoordinator:
    """Own semantic conversation and turn lifetimes for Hexbot core."""
    ...
    def _prepare_session(
        ...
        for name, callback in initializers:
            try:
                ...
                logger.warning(
                    "Hexbot Relay session initializer failed: %s",
                    name,
                    exc_info=True,
                )
        ...
        *,
        ...
        if isinstance(host, RelayRuntime):
            try:
                ...
                logger.warning(
                    "Hexbot Relay conversation initialization failed",
                    exc_info=True,
                )
    ...
    def begin_turn(
        ...
        *,
        ...
        if lease.released:
            raise RuntimeError("Hexbot Relay conversation lease is released")
        ...
        with self._active_turns_lock:
            ...
            if active:
                ...
                # Hexbot turns would create sibling scopes on that stack, but
                # their completion order is not guaranteed to be LIFO.
                turn.relay_enabled = False
                logger.warning(
                    "Skipping Relay instrumentation for concurrent Hexbot turn "
                    "%s in session %s",
                    turn_id,
                    lease.session_id,
                )
        ...
        if (
            ...
            try:
                ...
                logger.warning(
                    "Hexbot Relay segment rotation failed", exc_info=True
                )
            try:
                ...
                logger.warning("Hexbot Relay turn initialization failed", exc_info=True)
            ...
            try:
                if isinstance(lease.host, RelayRuntime) and lease.session is not None:
                    ...
                    if turn.handle is not None:
                        ...
                        if failure:
                            logger.warning(
                                "Hexbot Relay turn finalization failed: %s",
                                failure,
                            )
            finally:
                try:
                ...
                try:
                    ...
                    logger.warning(
                        "Hexbot Relay child conversation finalization failed",
                        exc_info=True,
                    )
                finally:
    ...
    def _consume_deferred_close(self, lease: Any) -> None:
        ...
        try:
            ...
            logger.warning(
                "Hexbot Relay deferred session close failed", exc_info=True
            )
    ...
    def notify_session_compacted(
        ...
        *,
        ...
        try:
            ...
            logger.warning(
                "Hexbot Relay compaction notification failed", exc_info=True
            )
    ...
    def _finish_logical_calls(
        ...
        *,
        ...
        for index in range(len(logical_calls) - 1, -1, -1):
            ...
            logger.warning("Hexbot Relay logical LLM finalization failed: %s", failure)
            break
...
def resolve_execution_context(
    ...
    if _MANAGED_CALLBACK_DEPTH.get() > 0:
        ...
        # a Hexbot tool). Nested managed execution here is structurally
...
def emit_mark(
    ...
    *,
    ...
    """Emit a fail-open Relay mark under a Hexbot session."""
    runtime = get_runtime(create=False)
    if runtime is None:
        return False
    try:
        ...
        logger.warning("Hexbot Relay mark failed: %s", name, exc_info=True)
        return False
...
def apply_tool_request_intercepts(
    *,
    ...
    """Return Relay-rewritten arguments at Hexbot's authorization boundary."""
    if not session_id:
        return args
    runtime = get_runtime(create=False)
    if runtime is None:
...
def ensure_session(*, session_id: str, **context: Any) -> RelaySession | None:
    """Create or return the shared Relay session used by Hexbot core."""
    runtime = get_runtime()
    if runtime is None:
        return None
    try:
        ...
        logger.warning("Hexbot Relay session initialization failed", exc_info=True)
        return None
...
def run_in_session(
    ...
    **kwargs: Any,
    ...
    """Run a scope, LLM, or tool API against a shared Hexbot session."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("Hexbot Relay runtime is unavailable")
    session = runtime.get_session(session_id)
    if session is None:
        session = runtime.ensure_session({"session_id": session_id})
    if session is None:
        raise RuntimeError("Hexbot Relay session is unavailable")
...
async def run_in_session_async(
    ...
    **kwargs: Any,
    ...
    """Await a Relay operation inside a shared Hexbot session context."""
    runtime = get_runtime()
    if runtime is None:
        raise RuntimeError("Hexbot Relay runtime is unavailable")
    session = runtime.get_session(session_id)
...
def get_runtime(
    *,
    ...
    """Return the Relay host for the active Hexbot profile."""
...
def _configured_plugin_inputs(
    ...
    try:
        ...
        if "dynamic_plugins" in config:
            raise ValueError(
                "Hexbot [[dynamic_plugins]] records are unsupported; use Relay "
        ...
        raise _RelayPluginConfigurationLoadError(
            "Hexbot Relay plugin configuration could not be loaded from "
