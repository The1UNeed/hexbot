...
const updateStates = new Set(['idle', 'checking', 'available', 'downloading', 'downloaded', 'none', 'error'])
interface UpdateStatus { state: string; percent?: number; message?: string }
function validUpdateStatus(value: unknown): value is UpdateStatus {
  if (!value || typeof value !== 'object') return false
  const item = value as Record<string, unknown>
  return typeof item.state === 'string' && updateStates.has(item.state)
...
const hexbot = Object.freeze({
  ...
  updater: Object.freeze({
    ...
    onStatus: (callback: (value: UpdateStatus) => void) => listen<unknown>('hexbot:updater:status', value => { if (validUpdateStatus(value)) callback(value) }),
