...
describe('daemon helpers', () => {
  ...
  it('uses the specified restart backoff', () => { expect(backoffSchedule).toEqual([1_000, 2_000, 4_000, 8_000, 16_000]) })
  it('walks ports until the mocked net server can listen', async () => {
    let attempts = 0
    const createServer = (() => {
      const server = new EventEmitter() as EventEmitter & { listen: (port: number, host: string, callback: () => void) => void; close: (callback: () => void) => void }
      server.listen = (_port, _host, callback) => { attempts += 1; if (attempts < 3) queueMicrotask(() => server.emit('error', new Error('busy'))); else queueMicrotask(callback) }
