...
export interface DaemonStatus { state: DaemonState; port?: number; pid?: number; lastError?: string }
...
export function parseReadyLine(line: string): number | undefined {
  ...
  const port = Number(match[1]); return port > 0 && port <= 65_535 ? port : undefined
}

export async function findFreePort(start = 9119, createServer: typeof net.createServer = net.createServer): Promise<number> {
...
function rotateLog(path: string): void {
  try { if (existsSync(path) && statSync(path).size >= 5 * 1024 * 1024) renameSync(path, `${path}.1`) } catch { /* logging must not stop the daemon */ }
}
...
export class DaemonManager extends EventEmitter {
  ...
  constructor(private readonly configuredPort?: number, private readonly serviceInstalled: () => Promise<boolean> = async () => false) {
    super()
    app.on('before-quit', event => {
      if (this.quitting || !this.child) return
      event.preventDefault(); this.quitting = true
      void this.stopForQuit().finally(() => app.exit(0))
    })
  }

  status(): DaemonStatus { return { ...this.current } }
  private setStatus(status: DaemonStatus): void { this.current = status; this.emit('status', this.status()) }
  ...
  private async spawnDaemon(port: number): Promise<void> {
    ...
    const logPath = join(hexbotHome(), 'logs', 'daemon.log'); rotateLog(logPath)
    const log = createWriteStream(logPath, { flags: 'a' })
    this.setStatus({ state: 'starting', port })
    const child = spawn(hexbotExecutable(), ['serve', '--port', String(port)], {
      cwd: activeSourceDir(),
      env: { ...process.env, HEXBOT_HOME: hexbotHome(), PATH: [binDir(), join(venvDir(), 'bin'), process.env.PATH ?? ''].join(delimiter) }
    })
    this.child = child
    ...
    child.once('error', error => this.setStatus({ state: 'crashed', port, lastError: error.message }))
    child.once('exit', (code, signal) => {
      log.end(); if (this.child === child) this.child = undefined
      if (this.intentionalStop) { this.setStatus({ state: 'stopped' }); return }
      const now = Date.now(); this.failures = this.failures.filter(time => now - time < 120_000); this.failures.push(now)
      const retry = this.failures.length - 1
      if (retry >= backoffSchedule.length) { this.setStatus({ state: 'crashed', port, lastError: `Daemon exited (${code ?? signal ?? 'unknown'})` }); return }
      this.setStatus({ state: 'starting', port, lastError: `Daemon exited (${code ?? signal ?? 'unknown'}); restarting` })
      this.restartTimer = setTimeout(() => { void this.spawnDaemon(port) }, backoffSchedule[retry])
    })
  }
  ...
  async stop(): Promise<DaemonStatus> {
    ...
    await new Promise<void>(resolve => {
      ...
      child.once('exit', () => { clearTimeout(force); resolve() }); child.kill('SIGTERM')
    })
    this.setStatus({ state: 'stopped' }); return this.status()
  }

  private async stopForQuit(): Promise<void> { if (!(await this.serviceInstalled())) await this.stop() }
}
