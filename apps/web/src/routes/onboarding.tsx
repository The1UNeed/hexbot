...
function OnboardingPage() {
  ...
  const createBot = async () => {
    ...
    try {
      ...
      await navigate({
        to: '/b/$bot/s/$section',
        params: { bot: result.bot.name, section: result.section.id }
      })
    } catch (reason) {
      setError(String(reason))
    } finally {
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-background p-6 text-foreground">
      <section className="w-full max-w-lg space-y-6">
        <h1 className="text-[length:var(--text-title)] font-semibold">Set up Hexbot</h1>
        {step === 'choice' ? (
          <div className="grid gap-3">
            <Button onClick={() => void navigate({ to: '/connect' })}>
              Connect to a Hexbot daemon
            </Button>
            {isElectron() ? (
              <Button onClick={() => setStep('install')} variant="primary">
                Run Hexbot on this machine
              </Button>
            ) : null}
          </div>
        ) : null}
        {step === 'install' ? (
          <div>
            ...
            <progress
              ...
              max={1}
              value={progress.at(-1)?.fraction ?? undefined}
            />
            ...
            {busy ? null : (
              <Button className="mt-4" onClick={() => setStep('providers')}>
                Continue
              </Button>
            )}
          </div>
        ...
        {step === 'providers' ? (
          <div className="space-y-4">
            <p>Hexbot does not include any model credits. Usage is billed by your providers.</p>
            <Select
              label="Provider"
              onValueChange={setProvider}
              options={(providers.length
                ? providers
                : [{ id: 'openai-api', label: 'OpenAI' } as Provider]
