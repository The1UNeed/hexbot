...
Ids were checked against the Hexbot catalogs in this tree (`agent.models_dev`
...
Hexbot slug — note that OpenAI's direct API is `openai-api` and the ChatGPT /
...
CURATED_MODELS: list[Model] = [
    ...
    # OpenRouter — three popular routes from Hexbot's own OPENROUTER_MODELS list.
    ...
    # Ollama, served from a local endpoint. Hexbot routes local Ollama through
