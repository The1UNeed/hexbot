"""ACP agent server — exposes Hexbot via the Agent Client Protocol."""
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Hexbot running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Hexbot cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Hexbot/OpenAI-compatible user content payload."""
...
class HermesACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Hexbot AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "version",
            "description": "Show Hexbot version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        modes, which coexist with the model picker, so Hexbot maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        The shared Hexbot inventory is also used by ``hexbot core model``, the TUI,
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Hexbot estimates ``used`` from the same
    ...
    async def _send_session_info_update(
        ...
        *,
        ...
        """Send ACP native session metadata after Hexbot changes it.

        When the internal Hexbot head rotated (e.g. compression-driven session
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Hexbot's threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Hexbot setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Hexbot
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Hexbot on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Hexbot's local kawaii waiting/status
        ...
        def _run_agent() -> dict:
            ...
            try:
                ...
                # Without it the prompt advertises the global Hexbot workspace
                # while the tools are rooted at the client's project, so the
                # model emits absolute paths under ~/.hexbot/workspace and the
        ...
        try:
            # Snapshot the internal Hexbot DB session id before the turn so we
        ...
        # Hexbot's local "waiting for model response" interrupt status is metadata,
    ...
    def _handle_slash_command(self, text: str, state: SessionState) -> str | None:
        ...
        # an unpinned handler bakes the Hexbot install tree into the session's
    ...
    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Hexbot v{HERMES_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Hexbot has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
