"""ACP tool-call helpers for mapping hexbot core tools to ACP ToolKind and building content."""
...
def _json_loads_maybe(value: Optional[str]) -> Any:
    ...
    # Some Hexbot tools append a human hint after a JSON payload, e.g.
...
def _tool_result_failed(result: Optional[str], tool_name: str | None = None) -> bool:
    """Return True when a structured Hexbot tool result clearly failed.
    ...
    # Hexbot core/polished tools commonly report tool-level failures as a
...
def _format_read_file_result(result: Optional[str], args: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    # Hexbot read_file output is line-numbered with `|`. If we send it as raw
