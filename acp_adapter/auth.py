"""ACP auth helpers — detect and advertise Hexbot authentication methods."""
...
def detect_provider() -> Optional[str]:
    """Resolve the active Hexbot runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Hexbot can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Hexbot.
    ...
    not have Hexbot provider credentials configured yet, so Hexbot always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Hexbot using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Hexbot provider",
            description=(
                "Open Hexbot's interactive model/provider setup in a terminal. "
                "Use this when Hexbot has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
