"""ACP permission bridging for Hexbot dangerous-command approvals."""
...
# Maps ACP permission option ids to Hexbot approval result strings.
...
def _build_permission_options(
    *, allow_permanent: bool, allow_session: bool = True,
    ...
    """Return ACP options that match Hexbot approval semantics."""
    ...
    # Smart DENY override — the editor must not offer a scope Hexbot
    ...
    if not once_only:
        options.append(PermissionOption(
            ...
            # hint while keeping Hexbot semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_hermes(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into Hexbot approval strings."""
...
def make_approval_callback(
    ...
    """
    Return a Hexbot-compatible approval callback that bridges to ACP.
