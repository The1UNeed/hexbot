"""Windows UTF-8 bootstrap for Hexbot entry points.
...
should be imported at the very top of every Hexbot entry point
(``hexbot core``, ``hermes-agent``, ``hermes-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Hexbot modules.

    Hexbot ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Hexbot source root
