...
This document is the formal interface between the **Hexbot gateway** (Python,
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `hexbot core gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`hexbot core gateway enroll` → connector `/relay/enroll`, or
...
driven by the managed Portal / `hexbot core` CLI.
