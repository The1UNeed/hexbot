...
## 2026-07-13: Scope plugin manager state by Hexbot home/profile (keyed cache)

Status: Accepted

Context:
Hexbot supports multiple profiles via different Hexbot home directories.
...
- Replace the single-slot singleton with a cache keyed on the *resolved*
  Hexbot home path (`_plugin_managers_by_home: Dict[Path, PluginManager]`).
