...
Hexbot gateway daemon, exported over OTLP/HTTP to an operator-configured
...
capture is a separate plane served by Hexbot's native NeMo Relay SDK
...
a plain `hexbot core gateway run`: the exporter lives in the gateway process, so
...
Hexbot-owned Relay transport health remains in scope. A separate gateway
...
```bash
hexbot core monitoring status                 # posture
python scripts/observability/gateway_health_export_probe.py \
  --endpoint http://127.0.0.1:4318/v1/traces \
...
The `hexbot core monitoring` CLI intentionally exposes `status` only. This first
release covers only Hexbot-owned service-health and operational-diagnostic
signals, including Hexbot-owned Relay transport health. Team Gateway's
