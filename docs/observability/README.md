# Hexbot Observer Hooks

Hexbot observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Hexbot's planner, model providers, memory, tool registry,
...
Hexbot also has a first-party NeMo Relay shared-metrics path. It uses these
...
Hook callbacks are fail-open. Hexbot catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Hexbot only builds
...
The native NeMo Relay SDK integration maps Hexbot session, turn, LLM, and tool
