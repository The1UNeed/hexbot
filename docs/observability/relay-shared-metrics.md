...
Hexbot includes NeMo Relay as a normal runtime dependency on platforms for
which Relay publishes a native wheel. The shared-metrics integration is built
into Hexbot and does not require a Hexbot observability plugin. Hexbot remains
importable without Relay on other native targets. Those targets use an
explicit reduced-capability no-op host:
Hexbot execution remains available, while Relay scopes, middleware, plugins,
...
> [!WARNING]
> This removes the Hexbot `observability/nemo_relay` plugin. Existing users
...
> longer activate exporters. Without the new variable, Hexbot does not run
> Relay plugin discovery, configuration layering, middleware, or exporters.

Hexbot requires NeMo Relay 0.7.1 or later within the 0.7 release line. That
...
Hexbot installs the platform-specific `nemo-relay` native wheel from the
...
through that native module in the Hexbot process so configured interceptors can
...
Collection remains off unless Hexbot policy enables it:
...
variable is unset, Hexbot does not invoke Relay's plugin initializer, so Relay
...
from the selected file only. If the selected file cannot be loaded, Hexbot
...
Relay plugin configuration is a process-level deployment choice, not a Hexbot
profile setting. The first hosted profile triggers lazy initialization, and
every additional profile hosted by that Hexbot process shares the resulting
static middleware, dynamic plugins, subscribers, exporters, and guardrail
policy. After initialization succeeds, Hexbot logs:

```text
Relay plugins are active process-wide and apply to all profiles hosted by this Hexbot process.
```
...
profile's Relay scope stack. Native dynamic plugins are loaded into the Hexbot
process and share the same policy boundary.

Run profiles in separate Hexbot processes when they require different trust
...
Hexbot core owns one Relay host and one isolated Relay session scope per Hexbot
...
marks do not require Hexbot plugin registration. Shared-metrics marks must
...
```text
Hexbot turn, API, tool, and approval hooks
  -> Relay session, task, LLM, tool, and mark lifecycle
  -> Hexbot shared-metrics subscriber
  -> SQLite counters
  -> immutable JSON delta package
```

Hexbot sends an empty `LLMRequest` into the metrics-owned lifecycle. This does
...
provider route that Hexbot used for the logical call, such as
...
counter in any rolling 24-hour window. The metric has no dimensions; Hexbot
version, OS family, architecture, and install method remain bounded package
resources. Concurrent Hexbot processes share the SQLite latch, so simultaneous
...
the owning Hexbot session. The start counter contains only bounded execution
...
provider attempts for the same Hexbot API request ID; they do not inflate the
logical model-call count. Tool calls are deduplicated by their Hexbot tool-call
...
cancellations. Active task ownership follows the task ID if Hexbot rotates its
...
outcome, approval outcome, latency, and explicit retry-count buckets. Hexbot
...
tool or plugin names. Hexbot does not infer retries from repeated tool names or
...
state, and a use-count bucket. Hexbot derives reuse and patch-generation
...
are written with atomic replacement. Each package records the Hexbot version,
...
The install identity is scoped to one `HERMES_HOME`. To reset it, stop Hexbot
...
Run a real Hexbot CLI turn against the deterministic local model server:
...
`$HERMES_HOME/telemetry/shared_metrics/outbox/` to the Hexbot telemetry ingest
