...
Both directories are self-contained npm packages, so the install command in each `vercel.json` is a plain `npm install` inside the root directory and never touches the workspace root. Vercel skips the projects a commit did not touch: a change under `apps/site/` only builds the site, a change under `apps/connect/` only builds Connect, and a change outside `apps/` builds both. A project's settings live in the Vercel dashboard; `vercel.json` in each directory holds what can be versioned (headers, install and build commands).
