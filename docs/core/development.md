# Hexbot - Development Guide

Instructions for AI coding assistants and developers working on the Hexbot core
(the Python code at the repository root). Product behaviour belongs in `hexbot/`
and `apps/`; see `AGENTS.md`.

**Never give up on the right solution.**

## What Hexbot Is

Hexbot is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Hexbot ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`hexbot core tools`, `hexbot core setup`, auto-install) rather than bolting on a raw
  env var.
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `hexbot core tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.hexbot/plugins/` (or via a
  pip entry point). This is a coupling-and-maintenance decision, not
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `hexbot core <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `hexbot core webhook`, `hexbot core cron`, `hexbot core tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.hexbot/plugins/` or a pip package, discovered at runtime.
...
- **Ask which identity you actually mean.** `HERMES_DESKTOP=1` legitimately
  ...
  and the embedded terminal pane (`hexbot core --tui` against that same backend) is
...
├── ui-tui/               # Ink (React) terminal UI — `hexbot core --tui`
...
**User config:** `~/.hexbot/config.yaml` (settings), `~/.hexbot/.env` (API keys only).
**Logs:** `~/.hexbot/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_hermes_home()`.
Browse with `hexbot core logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Hexbot: desktop, TUI, website, and future TS packages.
...
- Skill slash commands: `agent/skill_commands.py` scans `~/.hexbot/skills/`, injects as **user message** (not system prompt) to preserve prompt caching
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `hexbot core --tui` or `HERMES_TUI=1`.
...
### TUI in the Dashboard (`hexbot core dashboard` → `/chat`)

The dashboard embeds the real `hexbot core --tui` — **not** a rewrite.  See `hermes_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `hermes_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `hexbot core --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `hexbot core --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@hermes/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `hexbot core serve` backend server (the same gateway `dashboard` serves, minus the browser UI entirely: `serve` sets `headless_backend=True`, so `cmd_dashboard` skips `_build_web_ui` AND exports `HERMES_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists — only the JSON-RPC/WS/API surface is reachable). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.ts` + `backendSupportsServe()` in `electron/main.ts`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `hexbot core` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `hexbot core --tui` — it has its own composer, transcript, and slash-command pipeline. For scoped Desktop architecture, state, resolver, transport, and testing rules, read `apps/desktop/AGENTS.md`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMAND_SPECS` (the built-ins and their Desktop surfaces) plus `NO_DESKTOP_SURFACE` block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Hexbot built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Hexbot core.
Use the plugin route instead: create `~/.hexbot/plugins/<name>/plugin.yaml`
and `~/.hexbot/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Hexbot tool that should ship in the base system.
...
| `load_config()` | `hexbot core tools`, `hexbot core setup`, most CLI subcommands | `hermes_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
~/.hexbot/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Hexbot gold/kawaii (the current look)
...
Users create `~/.hexbot/skins/<name>.yaml`:
...
Hexbot has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.hexbot/plugins/` and pip-installed entry points.

### General plugins (`hermes_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.hexbot/plugins/`, `./.hermes/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `hexbot core` at startup so
  `hexbot core <pluginname> <subcmd>` works with no change to `main.py`
...
it at argparse setup time and wires it into `hexbot core <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `hexbot core --help`.
...
into `~/.hexbot/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `hexbot core memory setup` / `post_setup()` without
...
`~/.hexbot/plugins/` (or via pip entry points). They register through
...
don't own. PRs that add such a directory under
...
The desktop "Bots" experience ships bundled in-tree. Each bot is a Hexbot
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `hexbot core skills install official/<category>/<skill>`. Adapter lives in
...
2. **Tools referenced in SKILL.md prose must be native Hexbot tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Hexbot" is the secondary collaborator. If the
   contributor's commit shows "Hexbot" as author (because they
   used Hexbot to draft the skill), replace it with their actual name
...
Enable/disable per platform via `hexbot core tools` (the curses UI) or the
...
Hexbot returns a delegation id immediately and the result re-enters the
...
go to `~/.hexbot/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `hermes_cli/curator.py` wires `hexbot core curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.hexbot/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `hexbot core cron <verb>`
...
- File lock at `~/.hexbot/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `hexbot core kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `hermes_cli/kanban.py` wires `hexbot core kanban` with verbs
...
## Update Pipeline (`hexbot core update`)
...
- **Plan** (`hermes_cli/update_inventory.py`, `hexbot core update --plan`):
...
- **Report**: every run writes a machine-readable receipt to
  `~/.hexbot/logs/update_receipts/` (`latest.json` pointer; steps,
...
`hexbot core serve` (control plane, desktop-spawned child) dies with the app
...
Hexbot ensures caching remains valid throughout a conversation. **Do NOT implement changes that would:**
...
Hexbot supports **profiles** — multiple fully isolated instances, each with its own
...
1. **Use `get_hermes_home()` for all HERMES_HOME paths.** Import from `hermes_constants`.
   NEVER hardcode `~/.hexbot` or `Path.home() / ".hermes"` in code that reads/writes state.
   ```python
   # GOOD
...
2. **Use `display_hermes_home()` for user-facing messages.** Import from `hermes_constants`.
   This returns `~/.hexbot` for default or `~/.hexbot/profiles/<name>` for profiles.
   ```python
   # GOOD
   ...
   print("Config saved to ~/.hexbot/config.yaml")
   ```
...
6. **Profile operations are HOME-anchored, not HERMES_HOME-anchored** — `_get_profiles_root()`
   ...
   This is intentional — it lets `hexbot core -p coder profile list` see all profiles regardless
...
- Use the canonical matchers: `gateway.status.looks_like_gateway_command_line`
  ...
  (top-level subcommand of any Hexbot argv). Never hand-roll token scans.
...
### DO NOT hardcode `~/.hexbot` paths
Use `get_hermes_home()` from `hermes_constants` for code paths. Use `display_hermes_home()`
for user-facing print/log messages. Hardcoding `~/.hexbot` breaks profiles — each profile
...
### Tests must not write to `~/.hexbot/`
The `_isolate_hermes_home` autouse fixture in `tests/conftest.py` redirects `HERMES_HOME` to a temp dir. Never hardcode `~/.hexbot/` paths in tests.
...
| HOME / `~/.hexbot/` | Your real config+auth.json                  | Temp dir per test                         |
...
Hexbot supports Linux, macOS and native Windows, and plenty of its behaviour
