...
has to be thrown away or summarized. Hexbot has always done this in one batch:
...
Hexbot folds the single oldest un-absorbed exchange into a running summary. The
...
The in-memory splice alone isn't enough. Hexbot's normal session flush is
...
With `micro_compact` unset or `false` Hexbot behaves exactly as it always has:
