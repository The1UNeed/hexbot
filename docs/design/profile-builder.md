...
Supersedes: PR #31781 (prompt_toolkit `hexbot core profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `hexbot core profile wizard` in the terminal.
...
A profile = a full `~/.hexbot/profiles/<name>/` directory with its own:
...
5. spawn `hexbot core -p <profile> skills install <id>` per hub skill (async, returns PIDs)
...
- `pnpm --filter web run build` (no JS test suite in web/).
