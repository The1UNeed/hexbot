...
Hexbot middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Hexbot's
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Hexbot calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Hexbot retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Hexbot guardrails,
...
Hexbot stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Hexbot runs
them as a nested chain in registration order. Middleware failures are fail-open:
Hexbot logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Hexbot applies middleware in this order:
...
For each tool call, Hexbot applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Hexbot pre-execution path against the effective arguments:
...
```bash
hexbot core plugins enable <plugin-name>
```
...
mkdir -p "$HERMES_HOME"
hexbot core plugins enable <plugin-name>
hexbot core chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run hexbot core plugins enable <plugin-name>
uv run hexbot core chat --query 'Reply exactly ok'
```
...
Return the same response shape Hexbot expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Hexbot treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Hexbot preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Hexbot does not convert downstream
