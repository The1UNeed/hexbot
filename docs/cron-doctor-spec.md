...
a provider run fails, or delivery starts failing. `hexbot core cron list` shows some of
...
Add `hexbot core cron doctor` as a read-only diagnostic command that summarizes cron
...
- `hexbot core cron doctor` returns `0` and prints a healthy message when active jobs
