...
export function HermesConsoleModal({ open, onClose }: HermesConsoleModalProps) {
  ...
  const promptRef = useRef("hexbot> ");
  const inputPromptRef = useRef("hexbot> ");
  ...
  const handleFrame = useCallback(
    (frame: ConsoleFrame) => {
      ...
      if (frame.type === "ready") {
        const nextPrompt = frame.prompt || "hexbot> ";
  ...
  useEffect(() => {
    ...
    writeLine(term, "\x1b[2mConnecting to Hexbot Console...\x1b[0m");
