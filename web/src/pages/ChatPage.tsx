/**
 * ChatPage — embeds `hexbot core --tui` inside the dashboard.
 *
...
export default function ChatPage({ isActive = true }: { isActive?: boolean }) {
  ...
  const [banner, setBanner] = useState<string | null>(() =>
    ...
    !window.__HERMES_AUTH_REQUIRED__
      ? "Session token unavailable. Open this page through `hexbot core dashboard`, not directly."
      : null,
  );
  ...
  useEffect(() => {
    ...
    const term = new Terminal({
      ...
      // even when the inner Hexbot TUI has enabled xterm mouse-events
    ...
    if (useWebgl) {
      try {
        ...
        console.warn(
          "[hexbot-chat] WebGL renderer unavailable; falling back to default",
          err,
        );
      }
    }
    ...
    // forwarding them into Hexbot. Keyboard input, paste, and resize still
    // behave normally.
