...
Browser-based dashboard for managing Hexbot configuration, API keys, and monitoring active sessions.
...
`hexbot core dashboard` on port 9119 serves the **built** bundle from `hermes_cli/web_dist/`, not the Vite dev server — changes in `web/src/` will not appear there until you run `pnpm run build` and restart the dashboard (or use `web --no-open` + Vite as above).
...
| Wordmark | `Typography` + size/tracking only | Sidebar/mobile “Hexbot” — mixed case, no Mondwest, no `text-display` |
