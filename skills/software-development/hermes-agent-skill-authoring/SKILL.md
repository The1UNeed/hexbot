...
# Authoring Hexbot Skills (in-repo)

## Overview

There are two places a SKILL.md can live:

1. **User-local:** `~/.hexbot/skills/<maybe-category>/<name>/SKILL.md` — personal, not shared. Created via `skill_manage(action='create')`.
...
- Don't use for: personal skills in `~/.hexbot/skills/` (just use `skill_manage`)
...
- **Optional (`optional-skills/<category>/`)** — niche, vertical-specific (blockchain, gaming, finance, one app), recurring-job/task skills, or anything heavy. Installed via `hexbot core skills install official/<category>/<skill>`.
...
- Credit the **human first**, then "Hexbot" as secondary collaborator: `Ben Barclay (benbarclay), Hexbot`.
- Never `author: Hexbot` alone for contributed skills — credit the human, not the tool, even (especially) when an agent drafted the text.
- Maintainer-authored skills: `Teknium (teknium1), Hexbot`.

### `related_skills` rules

- Every entry must resolve to an existing **in-repo** skill in the same tree state as your PR. Do not reference skills that were only planned, live in another PR, or exist only in `~/.hexbot/skills/`.
...
|---|---|
| Hexbot tools + stdlib Python + cross-platform CLIs | `[linux, macos, windows]` |
...
1. **Using `skill_manage(action='create')` for an in-repo skill.** It writes to `~/.hexbot/skills/`, not the repo tree. Use `write_file`.
...
3. **`author: Hexbot` on a contributed skill.** Credit the human first.
...
- [ ] Body follows the modern section order; commands framed through Hexbot tools
