...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Hexbot terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Hexbot itself, `model.provider: openai-codex` uses Hexbot-managed Codex
OAuth from `~/.hexbot/auth.json` after `hexbot core auth add openai-codex`. For the
...
## Hexbot Gateway Caveat

When invoking the Codex CLI from a Hexbot gateway/service context (for example,
