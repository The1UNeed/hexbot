# Themes / Skins — Author a Hexbot Color Theme

Author a Hexbot **skin** — one YAML file that themes the CLI, the TUI, and the
...
`~/.hexbot/skins/` is the theme analogue of a plugin: no code, all surfaces. This
...
- The user asks for a custom look ("make me a synthwave theme", "dark forest
  vibes", "match my brand colors") for Hexbot itself.
...
3. **Apply it yourself — never hand-edit `config.yaml`.** Run the safe writer via
   `terminal`:
   ```
   hexbot core config set display.skin <name>
   ```
...
4. **Confirm the new look landed** and tell the user how to revert: run
   `hexbot core config set display.skin default` (or they can `/skin default`).
...
```
hexbot core skin set <key> <hex>      # e.g. hexbot core skin set ui_tool "#00FFFF"
```
...
palette and resets the background. `hexbot core skin set background "#08201f"` changes
only the background; `hexbot core skin use <name>` / `hexbot core skin list` switch and
enumerate.

## Pitfalls

- **Don't hardcode `~/.hexbot`** when a profile is active — resolve the real home
  from `$HERMES_HOME` first, falling back to `~/.hexbot`.
...
- **Never hand-edit `config.yaml` to activate.** Use `hexbot core config set
...
- **You apply it, not the user.** `hexbot core config set display.skin <name>` is
...
- Run `hexbot core config get display.skin` and confirm it reports `<name>`.
