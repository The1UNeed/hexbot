...
Config: `~/.hexbot/config.yaml` (settings), `~/.hexbot/.env` (API keys) — both under `$HERMES_HOME` when it is set.
...
- Tests auto-redirect `HERMES_HOME` to temp dirs — never touch real `~/.hexbot/`.
