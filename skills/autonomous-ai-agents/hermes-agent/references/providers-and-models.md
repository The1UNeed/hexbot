...
Set via `hexbot core model` (picker) or `hexbot core setup`. 35+ provider profiles ship as
...
| nous | OAuth device code | `hexbot core auth add nous` (or `NOUS_API_KEY`) |
| openai-codex | OAuth | `hexbot core auth add openai-codex` |
| qwen-oauth | OAuth | `hexbot core auth add qwen-oauth` |
| minimax-oauth | OAuth | `hexbot core auth add minimax-oauth` |
...
Multiple credentials per provider pool and rotate automatically (`hexbot core auth`).
Fallback chain when the primary fails: `hexbot core fallback add|remove|list`.
...
#   hexbot core config set model.aliases.fav openrouter/anthropic/claude-sonnet-4.6
model:
  aliases:
