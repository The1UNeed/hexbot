...
Create dynamic webhook subscriptions so external services (GitHub, GitLab, Stripe, CI/CD, IoT sensors, monitoring tools) can trigger Hexbot agent runs by POSTing events to a URL.

## Setup (Required First)

The webhook platform must be enabled before subscriptions can be created. Check with:
```bash
hexbot core webhook list
```
...
```bash
hexbot core gateway setup
```
Follow the prompts to enable webhooks, set the port, and set a global HMAC secret.

### Option 2: Manual config
Add to `~/.hexbot/config.yaml`:
```yaml
platforms:
  webhook:
    enabled: true
...
All management is via the `hexbot core webhook` CLI command:

### Create a subscription
```bash
hexbot core webhook subscribe <name> \
...
- **Route scripts** (`--script` on subscribe, or `script:` on a config route): a script under `~/.hexbot/scripts/` receives the payload as JSON on stdin. JSON stdout replaces the payload before prompt templating; empty stdout, `[SILENT]`, or a nonzero exit ignores the webhook. `.sh`/`.bash` run with bash, everything else with Python. Scripts cannot live outside `~/.hexbot/scripts/` (path traversal is blocked).

```bash
hexbot core webhook subscribe todoist-hermes \
...
```bash
hexbot core webhook remove <name>
```

### Test a subscription
```bash
hexbot core webhook test <name>
hexbot core webhook test <name> --payload '{"key": "value"}'
```
...
### GitHub: new issues
```bash
hexbot core webhook subscribe github-issues \
  --events "issues" \
...
### GitHub: PR reviews
```bash
hexbot core webhook subscribe github-prs \
  --events "pull_request" \
...
```bash
hexbot core webhook subscribe stripe-payments \
...
```bash
hexbot core webhook subscribe ci-builds \
  --events "pipeline" \
...
```bash
hexbot core webhook subscribe alerts \
...
```bash
hexbot core webhook subscribe antenna-matches \
  --deliver telegram \
...
- Subscriptions persist to `~/.hexbot/webhook_subscriptions.json`

## How It Works

1. `hexbot core webhook subscribe` writes to `~/.hexbot/webhook_subscriptions.json`
...
3. **Check gateway logs:** `grep webhook ~/.hexbot/logs/gateway.log | tail -20`
4. **Signature mismatch?** Verify the secret in your service matches the one from `hexbot core webhook list`. GitHub sends `X-Hub-Signature-256`, GitLab sends `X-Gitlab-Token`.
5. **Firewall/NAT?** The webhook URL must be reachable from the service. For local development, use a tunnel (ngrok, cloudflared).
6. **Wrong event type?** Check `--events` filter matches what the service sends. Use `hexbot core webhook test <name>` to verify the route works.
