...
the `cronjob` tool, the `hexbot core cron` CLI (`list`, `add`, `edit`,
...
- **CLI:** `hexbot core curator <verb>` — `status`, `usage`, `run`, `pause`,
...
- **Cost:** the deterministic inactivity/prune sweep runs for free. The
  ...
  **off by default** — opt in with `curator.consolidate: true` or
  `hexbot core curator run --consolidate`. Routine background curation costs
  zero tokens.
- **Telemetry:** sidecar at `~/.hexbot/skills/.usage.json` holds
...
Users drive it via `hexbot core kanban <verb>`; dispatcher-spawned workers
