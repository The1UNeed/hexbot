...
Edit with `hexbot core config edit` or `hexbot core config set section.key value`.
...
`hexbot core config check` reports sections missing from an older config.

### Toolsets

Enable/disable via `hexbot core tools` (interactive) or `hexbot core tools enable/disable NAME`.
