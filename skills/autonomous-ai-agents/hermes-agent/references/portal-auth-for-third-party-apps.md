...
## Layer 1 — Is this thing a Hexbot plugin, or a separate app?
...
|---|---|---|
| **OpenViking memory plugin** (`plugins/memory/openviking/`) | Code that runs **inside the Hexbot process**. Its LLM calls go through Hexbot's already-configured provider. | Already uses Portal if user's Hexbot is configured for Portal. Nothing extra needed. `OPENVIKING_API_KEY` is the OpenViking *server's* own auth, not LLM auth. |
...
plugin that already runs inside Hexbot. That LLM call is already authenticated
via Hexbot's provider config. The plugin's own server auth (e.g.
...
What Hexbot has that *feels* like OAuth — `hexbot core login --provider nous`
opening a browser, user signs in, token lands in `~/.hexbot/auth.json` — is a
**Hexbot-specific browser flow**. Under the hood it produces a credential
Hexbot uses as a bearer. It is not a public OAuth provider that Karakeep et al.
...
1. Read Hexbot's existing Portal credential out of `~/.hexbot/auth.json`.
...
rides on the credential Hexbot already holds.

Where this could live in Hexbot:
...
- **Per-key rate limits** so a noisy app can be capped without eating the
  user's headroom for Hexbot itself.
...
1. First decide: Hexbot plugin (runs inside Hexbot) or separate app? If plugin,
   it already uses Portal via Hexbot's provider config — done.
...
4. Mixed setup ("Portal for some things, OpenRouter/Ollama Cloud for the
   Hexbot agent itself") is fully supported. Hexbot treats agent
...
pitch in the Tool Gateway announcement is specifically about Hexbot's
*tools* (web search, browser, image gen, TTS) flowing through the Portal
subscription when Hexbot is configured to use Portal as its provider. It is
