...
Author widget apps for the Hexbot TUI (`hexbot core --tui`): glanceable ambient
...
- The TUI must be in use (`hexbot core --tui`). Widgets do not render in the
...
- A thrown `register()` is logged and skipped; check
  `~/.hexbot/logs/tui_gateway_crash.log` if a widget never appears.
