#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for Hexbot.
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same Hexbot profile to refresh consent."
    )
...
def install_deps():
    ...
    # Fallback: the interpreter has no pip (the Hexbot Docker image's venv is
    ...
    print(
        "On environments without pip (e.g. Nix, or the Hexbot Docker image's "
        "uv-managed venv), install the optional extra instead:"
    )
    print("  hexbot core setup")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to Hexbot home."""
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for Hexbot")
