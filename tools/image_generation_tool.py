...
selectable via ``hexbot core tools`` → Image Generation; the active model is
...
def _resolve_managed_fal_gateway():
    """Resolve the FAL route from the stored `hexbot core tools` selection.
    ...
    try:
        ...
        if status is not None and 400 <= status < 500:
            ...
            raise ValueError(
                ...
                f"  • Pick a different model via `hexbot core tools` → Image Generation."
                f"{gateway_message}"
            ) from exc
        raise
...
def _agent_cache_base_for_env(env: Any) -> str | None:
    ...
    # Hexbot cache roots can be translated without side effects. SSH can still
    ...
    try:
        ...
        if source_images and not edit_endpoint:
            raise ValueError(
                ...
                f"via `hexbot core tools` → Image Generation."
            )
...
def check_fal_api_key() -> bool:
    """True if the FAL backend selected via `hexbot core tools` (or, on a
...
def _build_no_backend_setup_message() -> str:
    ...
    if managed_nous_tools_enabled():
        lines.append(
            ...
            "gateway enabled (`hexbot core setup`)"
        )
    lines.append(
        "  3. Configure a different image_gen provider via `hexbot core tools` "
        "→ Image Generation (run `hexbot core plugins list` to see installed "
        "backends)"
    )
    return "\n".join(lines)
    ...
    if provider is None:
        return json.dumps({
            ...
            "error": (
                ...
                f"registered that name. Run `hexbot core plugins list` to see "
        ...
        if "image_url" in kwargs or "reference_image_urls" in kwargs:
            ...
            return json.dumps({
                ...
                "error": (
                    ...
                    f"supports editing via `hexbot core tools` → Image Generation."
                ),
...
# model/provider via `hexbot core tools` or `/skills`.
