...
DEFAULT_EXCLUDES = [
    ...
    ".hg/",
    ".svn/",
    # Worktrees (Hexbot convention — don't recursively snapshot siblings)
    ".worktrees/",
...
def _load_ledger(store: Path, dir_hash: str) -> Dict[str, Dict]:
    ...
    ``write_file`` / ``patch`` produced, so restores can tell "Hexbot wrote
...
def _git_env(
    ...
    The shared store is internal Hexbot infrastructure — it must NOT inherit
...
def _migrate_legacy_store(base: Path) -> Optional[Path]:
    ...
    cleared with ``hexbot core checkpoints clear-legacy``.
    ...
    if legacy_root is not None:
        logger.info(
            ...
            "Clear with `hexbot core checkpoints clear-legacy` when safe.",
            legacy_root,
        )
    return legacy_root
...
class CheckpointManager:
    ...
    def record_agent_write(self, file_path: str) -> None:
        """Record the content hash of a file Hexbot just successfully wrote.
        ...
        recorded hash was hand-edited by the user after Hexbot last touched
    ...
    def safe_restore_plan(self, working_dir: str, commit_hash: str) -> Dict:
        ...
        still matches what Hexbot last wrote (per the agent-write ledger)
        and ``skipped`` lists files the user hand-edited after Hexbot's
        last write or that Hexbot never wrote at all.
        """
        ...
        ledger = _load_ledger(store, dir_hash)
        if not ledger:
            # No agent-write ledger yet (pre-existing store, or Hexbot has
        ...
        for rel in names_out.splitlines():
            ...
            if recorded is None:
                # Hexbot never wrote this file (or the ledger predates it) —
                # do not touch it in safe mode.
                skipped.append(rel)
                continue
            current = _hash_file(abs_path)
            if current is None:
                # File deleted since Hexbot wrote it: restoring it back is
                # safe — its last content was Hexbot-authored.
                restore.append(rel)
    ...
    def session_diff(self, working_dir: str) -> Dict:
        ...
        This powers ``/diff session``.  It answers "what has Hexbot changed
        ...
        *retained* checkpoint may predate the current session (or, after
        ...
        Hexbot changed", not an exact per-session ledger.
    ...
    def restore(
        ...
        hand-edited after Hexbot's last write — per the agent-write ledger —
        are left untouched, and only Hexbot-authored changes are reverted.
        ...
        if restore_paths is not None:
            ...
            # Hexbot-created files absent from it (delete to restore state).
            ...
            for rel in restore_paths:
                ...
                if ok_in_commit:
                    ...
                    # Hexbot created it. Deleting it would not restore a prior
...
# ---------------------------------------------------------------------------
# Public helpers for `hexbot core checkpoints` CLI
# ---------------------------------------------------------------------------
