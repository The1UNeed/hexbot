...
# All skills live in ~/.hexbot/skills/ (seeded from bundled skills/ on install).
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.hexbot/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.hexbot/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``hexbot core skills`` config UI). Default False
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: hexbot core plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    try:
                ...
                return json.dumps(
                    {
                        ...
                        "hint": (
                            ...
                            "the repo with `hexbot core skills untrust`."
                        ),
                    },
                    ensure_ascii=False,
                )
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.hexbot/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `hexbot core skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
            ...
            try:
                ...
                if is_org_mirror_path(skill_dir, active_skills_dir):
                    ...
                    header = (
                        ...
                        "them back with\n"
                        "> `hexbot core sync propose` (or automatically, if your "
                        "org enables it).\n\n"
                    )
