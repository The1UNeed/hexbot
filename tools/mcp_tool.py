...
Configuration is read from ~/.hexbot/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.hexbot/logs/mcp-stderr.log), tagged with the
...
def sdk_httpx():
    ...
    side by side with Hexbot's own pinned ``httpx``. Every object that crosses
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Hexbot's logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
def _build_safe_env(user_env: Optional[dict]) -> dict:
    ...
    Hexbot explicitly tagged during dotenv loading, plus any variables
    ...
    an exception: users configured that backend specifically so Hexbot and
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            candidates = [
                ...
                # image (which the Hexbot Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Hexbot MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Hexbot gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Hexbot document cache; returns a marker with
...
def _resolve_identity_header(server_name: str, config: dict):
    ...
    mode resolves the value to the active Hexbot profile name once at
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Hexbot's existing approval
...
class MCPServerTask:
    ...
    def _make_logging_callback(self):
        ...
        server into Hexbot's logging (agent.log via hermes_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _ensure_mcp_sdk():
            raise ImportError(
                ...
                "it is not installed. Run `hexbot core setup` to install MCP support, "
                "then retry."
            )
        ...
        # ungraceful exit of this Hexbot process (kill -9, crash, force-quit)
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Hexbot's approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Hexbot is fully
                ...
                if not self._ever_connected:
                    if failure_class == "permanent":
                        ...
                        # re-authenticated with ``hexbot core mcp login``. Parking
...
def _http_status_error_types() -> tuple:
    ...
    mcp >= 2.0) or by Hexbot's pinned ``httpx``, and the two define unrelated
...
def _handle_auth_error_and_retry(
    ...
    return tool_error(
        ...
        f"Run `hexbot core mcp login {server_name}` (or delete the tokens "
        f"file under ~/.hexbot/mcp-tokens/ and restart). Do NOT retry "
...
# ---------------------------------------------------------------------------
# Advisory file lock that prevents N concurrent Hexbot processes (e.g.
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Hexbot config file.
    ...
    ``os.environ`` (which includes ``~/.hexbot/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Hexbot's MEDIA: tag convention and out to messaging
            ...
            # Hexbot's MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Hexbot's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Hexbot uses the ``mcp__<server>__<tool>``
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Hexbot registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``hexbot core tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Hexbot registry.
