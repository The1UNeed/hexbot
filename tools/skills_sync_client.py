...
409), and is driven by:
  ...
  * the ``hexbot core sync status|pull|push|now`` CLI.
...
is toggled via ``hexbot core sync enable/disable`` (a ``sync`` flag on the skill's
...
user-authored skills under ``~/.hexbot/skills/`` are eligible; bundled and
...
# only as the local *intent* the user toggles via ``hexbot core sync enable`` — it is
...
def sync_org_auto_propose() -> bool:
    ...
    runs ``hexbot core sync propose <skill>``. The skill keeps working with the
...
def sync_default_opt_in() -> bool:
    ...
    ``hexbot core sync enable`` (or a plane manifest that opted it in). True: opt-OUT
...
#
# Only agent-created + user-authored skills under ~/.hexbot/skills/ sync.
...
def is_sync_eligible(skill_name: str) -> bool:
    ...
    Eligible = present locally under ~/.hexbot/skills/, NOT bundled, NOT
...
def _all_local_skill_names() -> List[str]:
    ...
    opt-out policy). A skill is any directory under ~/.hexbot/skills/ containing
...
def stable_device_id() -> str:
    ...
    Use ``set_device_name()`` / ``hexbot core sync device --name`` to set an explicit
    label."""
    ...
    # is then persisted, so a later `hexbot core sync device --name` (or editing the
...
def set_device_name(name: str) -> str:
    ...
    Writes the (trimmed) name to ~/.hexbot/skills/.sync_device_id, overwriting
...
# ~/.hexbot/skills/.sync_state as JSON.
#
...
# path under ~/.hexbot/skills/ (the sync contract: "the profile root is a tree
...
def _skill_rel_path(skill_name: str) -> Optional[PurePosixPath]:
    """Return the skill's path relative to ~/.hexbot/skills/ (posix), or None."""
    try:
...
def _check_version(caps: Dict[str, Any]) -> None:
    ...
    if major != WIRE_VERSION:
        raise SyncError(
            f"this server speaks sync version {ver!r}, but this Hexbot speaks "
            f"{WIRE_VERSION} — update Hexbot to sync with it"
        )
    ...
    if overlaps:
        ...
        return {
            ...
            "message": (
                ...
                f"{conflict_ref}. Resolve out-of-band (hexbot core sync / NAS UI)."
            ),
        }
...
def pull_skills(
    ...
    *,
    ...
    ~/.hexbot/skills/. Only paths the user has opted into (``sync: true``) are
...
def maybe_push_skills(*, message: str = "hexbot skill sync") -> Optional[Dict[str, Any]]:
...
def sync_status() -> Dict[str, Any]:
    """Return a status snapshot for ``hexbot core sync status``. Never raises."""
    status: Dict[str, Any] = {
        "nous_admin": False,
        "logged_in": False,
...
#
# Org skills live under a DISTINCT local namespace, ~/.hexbot/skills/_org/
...
#
# `hexbot core sync propose` is the org sharing surface; proposal is
...
def pull_org_skills(
    ...
    *,
    ...
    """Pull the org canonical set into ``~/.hexbot/skills/_org/<org_id>/``.
