...
<url>``) is spawned as a direct child of the Hexbot process. Hexbot's own
...
Hexbot process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Hexbot startup's opt-in
