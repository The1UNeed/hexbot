...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.hexbot/config.yaml``. Hexbot
...
Configuration is loaded from ~/.hexbot/config.yaml under the 'tts:' key.
...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    ...
    credential pool populated by ``hexbot core auth add <provider_id>``).
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.hexbot/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.hexbot/config.yaml.
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``hexbot core tools``); otherwise the historical Edge backend remains active.
...
# custom voice-cloning scripts, etc.) into Hexbot without any Python code
...
#           output_format: wav
#
# Hexbot writes the input text to a temp UTF-8 file, runs the command with
...
def _command_provider_env_passthrough(config: Dict[str, Any]) -> list:
    ...
    template (curl one-liners). The child env is scrubbed of Hexbot secrets by
...
def _run_command_tts(
    ...
    Child env is scrubbed of Hexbot secrets (salvage of #56332) while still
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `hexbot core setup` to configure, "
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `hexbot core model` or set XAI_API_KEY.")
    ...
    # output_format when Hexbot actually needs a non-default format/override.
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        ...
        # Include Hexbot client context following Gemini's partner
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Hexbot caches Piper voice models.

    Resolves to ``~/.hexbot/cache/piper-voices/`` under the active
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run `hexbot core setup` to install Mistral support."
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run hexbot core setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'hexbot core setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'hexbot core tools' and select Piper under TTS, or install manually: "
