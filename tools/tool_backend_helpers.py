...
def nous_tool_gateway_unavailable_message(
    ...
    *,
    ...
    try:
        ...
        pass
    return (
        f"{capability} is unavailable. Run `hexbot core model` to refresh your "
...
def resolve_provider_secret(
    ...
    Resolution order (fixes #68003 — keys added via ``hexbot core auth add
    ...
    2. The environment / ``~/.hexbot/.env``. Under a multiplexed gateway turn
    ...
    3. The credential pool / auth store for ``provider_id`` (``hexbot core auth
    ...
    try:
        ...
        # `hexbot core auth add <provider>` keys a registry provider by its plain
...
def resolve_openai_audio_api_key() -> str:
    ...
    the credential pool (``hexbot core auth add openai-api``) via
...
def read_selection(section: str) -> str | None:
    """Return the stored `hexbot core tools` provider string for a config section.
...
def selection_error(section: str, selection_name: str, failure: str) -> str:
    ...
    if note:
        failure = note
    return (
        f"{section} is configured to use {selection_name} (set via hexbot "
        f"core tools), but {failure}. Run 'hexbot core tools' to change it."
    )
...
def fal_key_is_configured() -> bool:
    ...
    Consults both ``os.environ`` and ``~/.hexbot/.env`` (via
