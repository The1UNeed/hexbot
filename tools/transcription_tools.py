...
def _try_lazy_install_stt() -> bool:
    ...
    try:
        ...
        logger.warning(
            ...
            "This is often a permission issue: the Hexbot process user cannot "
...
def _is_local_stt_provider(provider: str, stt_config: Dict[str, Any]) -> bool:
    """Return whether *provider* is exempt from Hexbot's remote upload cap."""
...
def _unregistered_stt_provider_error(provider: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "error": (
            ...
            "provider registered that name. Run `hexbot core plugins list` to see "
    ...
    *,
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="hermes-local-stt-") as output_dir:
            ...
            # Scrub Hexbot secrets from the child env (sibling path to #56332 /
    ...
    *,
    ...
    if not api_key:
        return {
            "success": False,
            "transcript": "",
            "error": "No xAI credentials found. Configure xAI OAuth in `hexbot core model` or set XAI_API_KEY",
        }
...
def _dispatch_stt_provider(
    ...
    # unavailable). Surface it — with its `hexbot core tools` remediation —
