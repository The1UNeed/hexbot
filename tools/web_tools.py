...
Backend is selected during ``hexbot core tools`` setup (web.backend in config.yaml).
When available, Hexbot can route Firecrawl calls through a Nous-hosted tool-gateway
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Hexbot config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Hexbot's config/.env layer (``hexbot core config set``, ``hexbot core tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.hexbot/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``hexbot core tools``). A
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `hexbot core tools` repaint.
        try:
            ...
            if provider is None and backend and selection_exists("web"):
                ...
                if disabled_key:
                    ...
                    error_text = (
                        ...
                        f"Re-enable it with `hexbot core plugins enable {disabled_key}` "
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        "Run `hexbot core tools` to set one up."
                    ),
                }
        else:
            logger.info(
                ...
                if backend and selection_exists("web"):
                    ...
                    if disabled_key:
                        ...
                        error_text = (
                            ...
                            f"config. Re-enable it with `hexbot core plugins "
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`hexbot core plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
def _provider_is_ready(provider) -> bool:
    ...
    require a true availability probe — otherwise ``hexbot core doctor`` paints a
