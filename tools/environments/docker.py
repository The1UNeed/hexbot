...
def _load_hermes_env_vars() -> dict[str, str]:
    """Load ~/.hexbot/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Hexbot profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    ...
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Hexbot process whose reuse path will pick them
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Hexbot
    process leaves its container behind permanently because there's no
    subsequent Hexbot process scheduled to reuse that exact (task, profile)
    pair.
    """
    ...
    * ``host_args`` — extra ``--add-host`` flags so the container can reach the
    ...
    if not status.configured:
        msg = (
            ...
            "Run `hexbot core egress setup` to mint tokens and write proxy.yaml."
        )
        if enforce:
    ...
    if not (status.pid and status.listening):
        msg = (
            ...
            "Start it with `hexbot core egress start`."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if status.ca_cert_path is None or not status.ca_cert_path.exists():
        ...
        msg = (
            ...
            "Re-run `hexbot core egress setup` to regenerate it."
        )
        if enforce:
            raise RuntimeError(msg)
        logger.warning("%s — continuing without proxy (enforce_on_docker=false).", msg)
    ...
    if not mappings:
        msg = (
            ...
            "corrupt.  Re-run `hexbot core egress setup` to mint provider "
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.hexbot/sandboxes/). Non-persistent
        ...
        # container shared across sessions").  If a prior Hexbot process
    ...
    def _resolve_passthrough_env(self) -> tuple[dict[str, str], set[str]]:
        ...
        # win over the generic Hexbot secret blocklist. Only implicit passthrough
        # keys are filtered. Also strip Hexbot-internal dynamic secrets
    ...
    def _find_reusable_container(
        ...
        try:
            ...
            if egress_label != "off":
                ...
                # reused after the operator runs "hexbot core egress disable",
        ...
        # container) but can happen if a previous Hexbot process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Hexbot exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Hexbot process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Hexbot startup. That covers the
        ...
        # outlives Hexbot processes, processes inside it stay alive, and
