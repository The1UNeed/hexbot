...
Then set ``CAMOFOX_URL=http://localhost:9377`` in ``~/.hexbot/.env``.
...
def is_camofox_mode() -> bool:
    ...
    ``hexbot core tools``). ``CAMOFOX_URL`` is the server ADDRESS only — its
...
def _managed_persistence_enabled() -> bool:
    """Return whether Hexbot-managed persistence is enabled for Camofox.
...
def _camofox_identity_override(task_id: Optional[str], camofox_cfg: Dict[str, Any]) -> Optional[Dict[str, str]]:
    ...
    so Hexbot operates in the same browser profile instead of creating a
...
def _adopt_existing_tab_enabled(camofox_cfg: Dict[str, Any]) -> bool:
    """Return whether Hexbot should recover an existing Camofox tab ID."""
...
def _loopback_rewrite_enabled(camofox_cfg: Dict[str, Any]) -> bool:
    ...
    ``http://127.0.0.1:9377``.  That is correct for Hexbot talking to the
...
def _adopt_existing_tab(session: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Some integrations own the visible Camofox tab outside Hexbot. Gateway
...
def _get_session(task_id: Optional[str]) -> Dict[str, Any]:
    ...
    derived from the Hexbot profile so the Camofox server can map it
