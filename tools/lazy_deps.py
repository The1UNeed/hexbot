"""
Lazy dependency installer for opt-in Hexbot backends.

Many Hexbot features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
remediation hint pointing at ``hexbot core tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  ...
  installed package cannot brick Hexbot, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # Dashboard (`hexbot core dashboard`)
    "tool.dashboard": (
        "fastapi==0.133.1",
    ...
    "tool.computer_use": (
        "mcp==2.0.0",
        "httpx2==2.12.0",  # mcp 2.x HTTP stack — keep in sync with pyproject [computer-use]
    ...
    # presence, the `hexbot core update` lazy-refresh pass re-asserts THIS pin on
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``hexbot core update`` lazy-refresh pass.
    """
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Hexbot under WSL to use Matrix on Windows."
        )
    return None
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``hexbot core update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    try:
        ...
        # which() misses the uv Hexbot installed and falls through to the
...
def ensure(feature: str, *, prompt: bool = True) -> None:
    ...
    # Package-manager installs (NixOS, and any other distro that ships Hexbot
    ...
    if _lazy_install_target() is None:
        ...
        if managed_by:
            raise FeatureUnavailable(
                ...
                f"packages come from {managed_by}, so Hexbot cannot install "
                f"them at runtime. Add the dependencies for {feature!r} via "
                f"{managed_by} (or run a pip/uv install of Hexbot instead)."
            )
...
def active_features() -> list[str]:
    ...
    Used by ``hexbot core update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``hexbot core update``. Never raises; lazy-install failures
