#!/usr/bin/env python3
"""Read which OS window sits directly underneath the Hexbot desktop window.
...
def read_window_below_tool(callback: Optional[Callable] = None) -> str:
    """Return the window underneath the Hexbot window as a JSON string."""
    if callback is None:
        return tool_error(
            "read_window_below is only available in the Hexbot desktop app."
        )
...
READ_WINDOW_BELOW_SCHEMA = {
    ...
    "description": (
        "Identify the app window directly behind the Hexbot desktop window "
