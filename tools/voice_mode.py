...
def _voice_capture_install_hint() -> str:
    ...
    # If we're running inside a venv (e.g. the bundled Hexbot venv at
    # ~/.hexbot/profiles/<name>/hermes-agent/venv/), `pip install` on the
...
def _get_beep_volume() -> float:
    ...
    system can't be imported (e.g. broken ~/.hexbot/config.yaml during a
...
# playback-phase barge transcript is treated as a self-capture of Hexbot's
...
def is_tts_echo(
    ...
    Hexbot just spoke using a character-level similarity ratio, which works
    across languages without word-tokenization. A genuine user interjection
    is very unlikely to closely match Hexbot's own words, so a high ratio is
...
def _play_audio_file_impl(file_path: str) -> bool:
    ...
    # A unique suffix prevents concurrent Hexbot TTS calls from colliding on
