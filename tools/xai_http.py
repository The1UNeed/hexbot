...
def has_xai_credentials() -> bool:
    ...
    hot-paint paths (``hexbot core tools`` repaint, tool-registration scans,
    ...
    2. ``~/.hexbot/auth.json`` has a non-empty ``providers.xai-oauth.tokens.access_token``
       (single file read, no expiry check, no refresh).
    3. ``credential_pool.xai-oauth`` has any entry with a non-empty
       ``access_token`` (covers multi-account ``hexbot core auth add xai-oauth``
...
def get_env_value(name: str, default=None):
    """Read ``name`` from ``~/.hexbot/.env`` first, then ``os.environ``.
...
def hermes_xai_user_agent() -> str:
    """Return a stable Hexbot-specific User-Agent for xAI HTTP calls."""
    try:
...
def hermes_xai_default_headers() -> Dict[str, str]:
    ...
    so chat/completions and Responses traffic is attributed as Hexbot,
...
def _load_config_section(section_name: str) -> Dict[str, Any]:
    """Return a top-level Hexbot config section as a dict, or empty."""
    try:
...
def maybe_mark_xai_storage_notice_seen(section_name: str) -> Optional[str]:
    """Return the storage notice once per Hexbot home, then mark it seen."""
...
def resolve_xai_http_credentials(
    *,
    ...
    Prefers Hexbot-managed xAI OAuth credentials when available, then falls back
    to ``XAI_API_KEY`` resolved via ``hermes_cli.config.get_env_value`` so keys
    stored in ``~/.hexbot/.env`` (the standard Hexbot location) are honored —
