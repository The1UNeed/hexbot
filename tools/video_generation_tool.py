...
provider** — turn on a backend by enabling a plugin (``hexbot core plugins
enable video_gen/<name>``) and selecting it in ``hexbot core tools`` → Video
Generation.
...
def _missing_provider_error(configured: Optional[str]) -> str:
    if configured:
        msg = (
            ...
            f"installed video gen backends, or `hexbot core tools` → Video "
    ...
    msg = (
        "No video generation backend is configured. Run `hexbot core tools` → "
...
# `hexbot core tools` or `/skills`, the schema rebuilds automatically.
...
_GENERIC_DESCRIPTION = (
    ...
    "`hexbot core tools` → Video Generation; the agent does not pick them. "
...
def _build_dynamic_video_schema() -> Dict[str, Any]:
    ...
    if provider is None:
        parts.append(
            ...
            "until the user picks one via `hexbot core tools` → Video Generation."
        )
        return {
            "description": "\n".join(parts),
            "parameters": {
