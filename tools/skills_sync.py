...
Copies bundled skills from the repo's skills/ directory into ~/.hexbot/skills/
...
The manifest lives at ~/.hexbot/skills/.bundled_manifest.
"""
...
# Marker file written by `hexbot core profile create --no-skills` (named profiles)
# and by the installer's `--no-skills` flag (the default ~/.hexbot profile).
# When present in HERMES_HOME, sync_skills() is a no-op so neither the
# installer, `hexbot core update`, nor a direct sync re-injects bundled skills.
...
def _read_suppressed_names() -> set:
    ...
    to reading ``~/.hexbot/skills/.curator_suppressed`` directly if that import
...
def _compute_relative_dest(skill_dir: Path, bundled_dir: Path) -> Path:
    ...
    e.g., bundled/skills/mlops/axolotl -> ~/.hexbot/skills/mlops/axolotl
    """
...
def _read_hub_install_paths() -> Set[str]:
    ...
    Hub-installed skills are owned by the hub (``hexbot core skills uninstall``),
        ...
        if _dir_hash(candidate) != origin_hash:
            ...
            if not quiet:
                print(
                    ...
                    f"updates. Run `hexbot core skills reset {skill_name} --restore` "
...
def sync_skills(quiet: bool = False) -> dict:
    """
    Sync bundled skills into ~/.hexbot/skills/ using the manifest.

    Returns:
        dict with keys: copied (list), updated (list), skipped (int),
                        user_modified (list), cleaned (list), total_bundled (int)
    """
    # Opt-out: a profile (named or the default ~/.hexbot) that wrote the
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        # (~/.hexbot/skills/.curator_suppressed) is written when the curator
        # archives a bundled skill with curator.prune_builtins enabled. Without
        # this skip, every `hexbot core update` would resurrect a skill the user
                ...
                if dest.exists():
                    ...
                    if _dir_hash(dest) == bundled_hash:
                        ...
                        print(
                            ...
                            f"was kept. Run `hexbot core skills reset {skill_name}` "
...
def _rmtree_writable(path: Path) -> None:
    ...
    # ``~/.hexbot/`` (``.env``, ``MEMORY.md``, ``kanban.db``, custom
    ...
    # ``shutil.rmtree(~/.hexbot)`` into a loud, recoverable ``ValueError``
...
def reset_bundled_skill(name: str, restore: bool = False) -> dict:
    ...
    if not in_manifest and not is_bundled:
        return {
            ...
            "message": (
                ...
                f"(Hub-installed skills use `hexbot core skills uninstall`.)"
            ),
            "synced": None,
        }
    ...
    if restore and deleted_user_copy:
        ...
        message = (
            f"Cleared manifest entry for '{name}'. Future `hexbot core update` runs "
...
def _is_tracked_user_modification(origin_hash: str, user_hash: str) -> bool:
    """Whether an on-disk skill counts as a user modification ``hexbot core update`` keeps.
...
def list_user_modified_bundled_skills() -> List[dict]:
    """Return the bundled skills that ``hexbot core update`` keeps because the user
    edited them locally.
...
def diff_bundled_skill(name: str) -> dict:
    ...
    edits or ``hexbot core skills reset`` back to upstream.
    ...
    if bundled_src is None:
        return {
            ...
            "message": (
                ...
                f"diff against). Hub-installed skills use `hexbot core skills inspect`."
            ),
        }
...
def set_bundled_skills_opt_out(enabled: bool) -> dict:
    ...
    installer, ``hexbot core update``, and any direct sync stop seeding bundled
    skills. When False, removes the marker so seeding resumes on the next
    sync. This is the on-disk-state half of ``hexbot core skills opt-out`` /
    ...
    try:
        if enabled:
            ...
            changed = existed
            message = (
                "Opted back in. The next `hexbot core update` (or `hexbot core skills "
...
if __name__ == "__main__":
    print("Syncing bundled skills into ~/.hexbot/skills/ ...")
