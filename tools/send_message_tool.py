...
def _media_caption_split(text, media_files, *, max_caption_len):
    ...
    across every standalone sender. ``hexbot core send`` (and the send_message
    ...
    ``hexbot core send --to whatsapp "MEDIA:/x.png This Caption"`` into two parts.
    ...
    if not pconfig or not pconfig.enabled:
        ...
        if platform_name == "weixin":
            ...
            if wx_token and wx_account:
                ...
                return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.hexbot/config.yaml or environment variables.")
        else:
            return tool_error(f"Platform '{platform_name}' is not configured. Set up credentials in ~/.hexbot/config.yaml or environment variables.")
    ...
    if not chat_id:
        ...
        if home:
            ...
            return tool_error(
                ...
                f"or set a home channel via: hexbot core config set {home_env} <channel_id>"
            )
...
async def _send_matrix_via_adapter(pconfig, chat_id, message, media_files=None, thread_id=None):
    ...
    is running (standalone cron, ``hexbot core send`` CLI).
    """
    media_files = media_files or []
...
#   - the ``hexbot core send`` CLI command (hermes_cli/send_cmd.py)
