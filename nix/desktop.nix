# nix/desktop.nix — Hexbot Desktop (Electron) app build + wrapper
#
...
# uses our fully wrapped binary at step 4 ("existing Hexbot CLI").
...
stdenv.mkDerivation {
  ...
  installPhase = ''
    ...
    # binary so the desktop's resolver step 4 ("existing Hexbot CLI on
  ...
  meta = with lib; {
    description = "Native Electron desktop shell for Hexbot";
