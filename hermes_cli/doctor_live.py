"""``hexbot core doctor --live`` — opt-in bounded real-call tool-backend probes.
...
- **Opt-in only.** These probes make real (cheap, metadata/read-only) network
  ...
  passes ``hexbot core doctor --live``.
...
def _probe_mcp_server(name: str, config: dict, timeout: float):
    ...
    Reuses the exact machinery behind ``hexbot core mcp test``.
    """
