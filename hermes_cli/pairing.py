...
Usage:
    hexbot core pairing list              # Show all pending + approved users
    hexbot core pairing approve <platform> <request-id|code>  # Approve a pairing request
    hexbot core pairing revoke <platform> <user_id> # Revoke user access
    hexbot core pairing clear-pending     # Clear all expired/pending codes
"""

def pairing_command(args):
    """Handle hexbot core pairing subcommands."""
    ...
    if action == "list":
        ...
        print("Usage: hexbot core pairing {list|approve|revoke|clear-pending}")
        print("Run 'hexbot core pairing --help' for details.")
...
def _cmd_list(store):
    ...
    if pending:
        ...
        print("\n  Approve with: hexbot core pairing approve <platform> <request-id>")
...
def _cmd_approve(store, platform: str, code: str):
    ...
    if result:
        ...
        print(
            ...
            "~/.hexbot/platforms/pairing/_rate_limits.json\n".format(platform)
        )
    else:
        print(f"\n  Pairing request or code '{code}' not found or expired for platform '{platform}'.")
        print("  Run 'hexbot core pairing list' to see pending requests.\n")
