...
The Hexbot docs site hosts a JSON manifest of curated models for providers
...
1. ``get_catalog()`` — returns a parsed manifest dict.
   ...
   - Reads disk cache at ``~/.hexbot/cache/model_catalog.json``.
...
def get_default_model_from_cache(provider: str) -> str | None:
    ...
    ``"default": true``; that entry is the model Hexbot silently lands on when
    ...
    network-free. The cache is kept fresh by the picker/`hexbot core update` paths;
...
def seed_cache_from_checkout(project_root: "Path | str") -> bool:
    ...
    ``hexbot core update`` pulls the latest repo, so the freshly-pulled
    ...
    to ``~/.hexbot/cache/model_catalog.json`` via the same atomic writer the
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``hexbot core model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
