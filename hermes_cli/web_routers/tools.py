...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    # same background install `hexbot core tools` runs interactively. Without this,
    ...
    # dead-end that forced users to discover `hexbot core computer-use install`
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``hexbot core tools`` picker shows
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `hexbot core tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``hexbot core tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.hexbot/.env`` via ``save_env_value`` —
    the same store ``hexbot core tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``hexbot core tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``hexbot core -p <profile> tools post-setup``.
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``hexbot core computer-use permissions grant`` as a background action.
