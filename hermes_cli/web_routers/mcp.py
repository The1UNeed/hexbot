...
async def mcp_oauth_callback(
    ...
    if flow is None:
        return HTMLResponse("<h1>OAuth flow expired</h1><p>Return to Hexbot and try again.</p>", status_code=404)
    try:
    ...
    if error:
        return HTMLResponse("<h1>Authorization failed</h1><p>Return to Hexbot for details.</p>", status_code=400)
    return HTMLResponse("<h1>Authorization received</h1><p>You can close this tab and return to Hexbot.</p>")
...
async def list_mcp_catalog(profile: Optional[str] = None):
    ...
    `hexbot core mcp catalog` / `hexbot core mcp install` read.  ``profile`` scopes
