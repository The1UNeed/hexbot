"""``hexbot core slack ...`` CLI subcommands.

Today only ``hexbot core slack manifest`` is implemented — it generates the
...
Typical workflow::
    ...
    $ hexbot core slack manifest > slack-manifest.json
    # or:
    $ hexbot core slack manifest --write
...
def _build_full_manifest(
    ...
    it stays in sync with the rest of Hexbot. Other manifest sections
    (display info, OAuth scopes, socket mode) are set to sensible defaults
    for a Hexbot deployment — users can tweak them in the Slack UI after
    pasting.

    By default, this keeps Hexbot on Slack's older Assistant messaging
    ...
    if messaging_experience == "assistant":
        features["assistant_view"] = {
            "assistant_description": "Chat with Hexbot in threads and DMs.",
        }
        ...
        features["agent_view"] = {
            "agent_description": "Chat with Hexbot in Slack Messages.",
        }
        bot_scopes.append("assistant:write")
    ...
    display_information = {
        "name": bot_name[:35],
        "description": (bot_description or "Your Hexbot agent on Slack")[:140],
...
def slack_manifest_command(args) -> int:
    ...
    Flags (all parsed in ``hermes_cli/main.py``):
      ...
      --name NAME     Override the bot display name (default: "Hexbot")
    ...
    """
    name = getattr(args, "name", None) or "Hexbot"
    description = getattr(args, "description", None) or "Your Hexbot agent on Slack"
    ...
    if getattr(args, "slashes_only", False) and (
        ...
        print(
            "hexbot core slack manifest: long description options cannot be used "
            "with --slashes-only",
            file=sys.stderr,
        )
        return 2
    if long_description_file is not None:
        ...
        try:
            ...
            print(
                f"hexbot core slack manifest: cannot read long description from "
                f"{source_arg}: {exc}",
                file=sys.stderr,
            )
            return 2
    if (
        ...
        print(
            "hexbot core slack manifest: long description must be at least "
    ...
    if (
        ...
        print(
            "hexbot core slack manifest: long description must be at most "
    ...
    if write_target is not None:
        ...
        print(
            "\nNext steps:\n"
            "  1. Open https://api.slack.com/apps and pick your Hexbot app\n"
