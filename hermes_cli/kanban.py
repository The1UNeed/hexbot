"""CLI for the Hexbot Kanban board — ``hexbot core kanban …`` subcommand.
...
def _check_dispatcher_presence(
    ...
    Used by ``hexbot core kanban create`` (and callers) to warn when a task
    ...
    if pid and not dispatch_on:
        return (
            ...
            "standalone daemon (`hexbot core kanban daemon --force`)."
        )
    return (
        False,
        "No gateway is running — the task will sit in 'ready' until you "
        "start it. Run:\n"
        "    hexbot core gateway start\n"
...
def build_parser(parent_subparsers: argparse._SubParsersAction) -> argparse.ArgumentParser:
    ...
    kanban_parser = parent_subparsers.add_parser(
        ...
        description=(
            "Durable SQLite-backed task board shared across Hexbot profiles. "
    ...
    kanban_parser.add_argument(
        ...
        help=(
            ...
            "(set via `hexbot core kanban boards switch <slug>` or the "
            "HERMES_KANBAN_BOARD env var). Use `hexbot core kanban boards list` "
            "to see all boards."
        ),
    )
    ...
    b_import = boards_sub.add_parser(
        ...
        description=(
            "Import a .tar.gz produced by `hexbot core kanban boards export`. "
    ...
    p_create.add_argument("--project", default=None,
                          help="Link to a project (id or slug). Anchors the task's "
                               ...
                               "deterministic branch. See `hexbot core project list`.")
    ...
    p_daemon = sub.add_parser(
        "daemon",
        help="DEPRECATED — dispatcher now runs in the gateway. Use `hexbot core gateway start`.",
    )
    ...
    p_asg = sub.add_parser(
        ...
        help="List known profiles + per-profile task counts "
             "(union of ~/.hexbot/profiles/ and current assignees on the board)",
    )
...
def kanban_command(args: argparse.Namespace) -> int:
    """Entry point from ``hexbot core kanban …`` argparse dispatch.
    ...
    if not action:
        ...
        if parser is not None:
            ...
            print(
                "usage: hexbot core kanban <action> [options]\n"
                "Run 'hexbot core kanban --help' for the full list of actions.",
                file=sys.stderr,
            )
        return 0
    ...
    if board_override:
        ...
        if normed != kb.DEFAULT_BOARD and not kb.board_exists(normed):
            print(
                ...
                f"Create it with `hexbot core kanban boards create {normed}`.",
                file=sys.stderr,
            )
            return 1
    ...
    with board_scope:
        ...
        # every `hexbot core kanban repair` into "could not initialize database"
...
# ---------------------------------------------------------------------------
# Boards management (hexbot core kanban boards …)
# ---------------------------------------------------------------------------

def _dispatch_boards(args: argparse.Namespace) -> int:
    """Handle ``hexbot core kanban boards <action>``.
...
def _cmd_boards_list(args: argparse.Namespace) -> int:
    ...
    if not boards:
        print("(no boards — create one with `hexbot core kanban boards create <slug>`)")
        return 0
    ...
    if len(boards) > 1:
        print("Switch boards with `hexbot core kanban boards switch <slug>`.")
    return 0
...
def _cmd_boards_create(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "switch", False):
        ...
        print(f"  Use `hexbot core kanban boards switch {meta['slug']}` to make it current.")
    return 0


def _cmd_boards_rm(args: argparse.Namespace) -> int:
    # When the user runs `hexbot core kanban boards delete <slug>` (alias), the
...
def _cmd_boards_export(args: argparse.Namespace) -> int:
    ...
    print("Import it with `hexbot core kanban boards import <archive>`.")
    return 0
...
def _cmd_boards_import(args: argparse.Namespace) -> int:
    ...
    if res["activated"]:
        ...
        print(f"  Switch to it with `hexbot core kanban boards switch {res['board']}`.")
    return 0
...
def _cmd_init(args: argparse.Namespace) -> int:
    ...
    if profiles:
        ...
        print("No profiles found under ~/.hexbot/profiles/.")
        print("Create one with `hexbot core -p <name> setup` before assigning tasks.")
    print()
    print("Next step: start the gateway so ready tasks actually get picked up.")
    print("  hexbot core gateway start")
    print()
    print(
...
def _cmd_assignees(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        return 0
    if not data:
        print("(no assignees — create a profile with `hexbot core -p <name> setup`)")
        return 0
    # Header
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    if len(all_boards) > 1:
        ...
        print(
            ...
            f"`hexbot core kanban boards list`)\n"
        )
    if not tasks:
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    if task.result:
        ...
        # Surface it at top-level so a glance at ``hexbot core kanban show <id>``
...
def _cmd_daemon(args: argparse.Namespace) -> int:
    ...
    if not getattr(args, "force", False):
        print(
            "hexbot core kanban daemon: DEPRECATED — the dispatcher now runs\n"
            "inside the gateway. To use kanban:\n"
            "\n"
            "    hexbot core gateway start       # starts the gateway + embedded dispatcher\n"
            "\n"
        ...
        if health_state["bad_ticks"] >= HEALTH_WINDOW:
            ...
            if now - health_state["last_warn_at"] >= 300:
                print(
                    ...
                    f"`hexbot core kanban list --status ready` / "
                    f"`hexbot core kanban list --status blocked` for recent "
    ...
    def _ready_queue_nonempty() -> bool:
        ...
        task whose assignee maps to a real Hexbot profile (i.e. one the
