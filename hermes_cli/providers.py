"""
Single source of truth for provider identity in Hexbot.
...
2. **Hexbot overlays** — transport type, auth patterns, aggregator flags,
...
# -- Hexbot overlay ----------------------------------------------------------
# Hexbot-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class HermesOverlay:
    """Hexbot-specific provider metadata layered on top of models.dev."""
...
def get_provider(name: str, *, allow_network: bool = True) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. Hexbot overlays (for providers not in models.dev: nous, openai-codex, etc.)
      2. models.dev catalog + Hexbot overlay
    ...
    if overlay is not None:
        # Hexbot-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "", model: str = "") -> str:
    ...
    *model* is optional but required for dual-wire providers (Nous) whose
    ...
    # chat_completions. The Hexbot overlay still advertises openai_chat
...
def resolve_provider_full(
    ...
    # 0.5 Exact Hexbot provider IDs must win over LOSSY alias collapsing.
