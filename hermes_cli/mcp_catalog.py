...
entries via ``hexbot core mcp catalog`` or the interactive ``hexbot core mcp picker``,
and install them with ``hexbot core mcp install <name>`` (or by toggling in the
...
- Manifests pin transport details (commands, args, refs). Pins follow the
  ...
  auto-updated; users explicitly re-run ``hexbot core mcp install <name>`` to
  pull a new manifest version after a repo update.
- Secrets prompted at install time go to ``~/.hexbot/.env`` (the
...
class TransportSpec:
    ...
    # auth.env so they are prompted for and land in ~/.hexbot/.env.
...
def _catalog_root() -> Path:
    """Return the optional-mcps/ directory shipped with this Hexbot install."""
...
def _parse_manifest(path: Path) -> CatalogEntry:
    ...
    if mv != _MANIFEST_VERSION:
        raise CatalogError(
            ...
            f"(this Hexbot understands version {_MANIFEST_VERSION})"
        )
...
def list_catalog() -> List[CatalogEntry]:
    ...
    UIs can tell the user their Hexbot is out of date.
    """
    root = _catalog_root()
    if not root.exists():
        return []
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      - ``future_manifest`` — manifest_version is newer than this Hexbot
        understands. Update Hexbot to install this entry.
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.hexbot/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.hexbot/.env via save_env_value()."""
...
def _apply_tool_selection(
    ...
    *,
    ...
    Probe-fail path:
      ...
      - Either way, point the user at ``hexbot core mcp configure <name>``.
    """
    print()
    ...
    if entry.tools.default_excluded and prior_selection is None:
        if prior_exclude is not None:
            ...
            print(color(
                ...
                "config.yaml or run "
                f"`hexbot core mcp configure {entry.name}` to change.",
                Colors.GREEN,
            ))
            return
    ...
    if probed is None:
        ...
        if prior_selection is not None:
            ...
            print(color(
                ...
                f"Run `hexbot core mcp configure {entry.name}` after the server "
            ...
            print(color(
                ...
                f"Run `hexbot core mcp configure {entry.name}` after first "
                "connect to prune.",
                Colors.YELLOW,
            ))
        return
    ...
    if not chosen_indices:
        ...
        print(color(
            f"  No tools selected. Run `hexbot core mcp configure {entry.name}` "
            "to change.",
            Colors.YELLOW,
        ))
        return
    ...
    if len(chosen_indices) == len(probed):
        ...
        # the user can re-run `hexbot core mcp configure <name>` and unselect a
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `hexbot core auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`hexbot core auth {entry.auth.provider}` if you have not "
    ...
    print(color(
        ...
        f"Start a new Hexbot session to load its tools.",
        Colors.GREEN,
    ))
