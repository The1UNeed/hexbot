...
1. **Update receipt** — a machine-readable JSON record of what one
   ``hexbot core update`` run discovered, did, skipped (and why), written to
...
# Module-level current receipt. ``hexbot core update`` is a single-threaded CLI
...
class UpdateReceipt:
    """Collects the observable facts of one ``hexbot core update`` run."""
    ...
    def gateway_restart_result(
        ...
        *,
        ...
        if fresh_recovery is not None:
            ...
            # Serve/dashboard coverage (#92145). ``hexbot core serve`` hosts
...
def finalize_pending_update_receipt(
    ...
    ``hexbot core update`` has many early-termination paths (Windows
