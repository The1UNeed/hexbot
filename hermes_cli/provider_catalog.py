...
The provider list shown by ``hexbot core model`` (CLI/TUI) and the desktop Settings
...
the same universe ``hexbot core model`` renders (``CANONICAL_PROVIDERS``), joining:
...
``CANONICAL_PROVIDERS`` universe, i.e. exactly what ``hexbot core model`` shows.
"""
...
class ProviderDescriptor:
    ...
    order: int                     # CANONICAL_PROVIDERS index — mirrors `hexbot core model`
...
def provider_catalog() -> list[ProviderDescriptor]:
    """Return one descriptor per provider in the ``hexbot core model`` universe.
    ...
    # Hexbot overlays carry auth_type for providers that have no registry/profile
    ...
    for order, entry in enumerate(CANONICAL_PROVIDERS):
        ...
        # Hexbot overlay (e.g. moa → "virtual"), then api_key.
        auth_type = (
