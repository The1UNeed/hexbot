"""MCP picker — interactive `hexbot core mcp picker` (also the default `hexbot core mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``hexbot core mcp add``, lets them pick one, and routes to install / enable /
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Hexbot session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `hexbot core mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: hexbot core mcp install <name>    Picker: hexbot core mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Hexbot is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Hexbot — run `hexbot core update` "
...
def show_catalog() -> None:
    """`hexbot core mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`hexbot core mcp picker` (and default `hexbot core mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`hexbot core mcp install <name>` — non-interactive entry-point.
    ...
    if entry is None:
        print(color(
            ...
            "Run `hexbot core mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
