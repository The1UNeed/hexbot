"""
Interactive setup wizard for Hexbot.
...
# display_hermes_home imported lazily at call sites (stale-module safety during hexbot core update)
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ Hexbot Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Hexbot using environment variables or config commands:")
    print_info("  hexbot core config set model.provider custom")
    print_info("  hexbot core config set model.base_url http://localhost:8080/v1")
    print_info("  hexbot core config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'hexbot core setup' in an interactive terminal to use the full wizard.")
    print()
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'hexbot core setup')")
...
def _print_setup_summary(config: dict, hermes_home):
    ...
    if not _provider_ready:
        print()
        print_warning("No inference provider is configured — Hexbot cannot chat yet.")
        print_info("  Finish this one step with either of:")
        print_info("    hexbot core model            (pick any provider/model)")
        print_info("    hexbot core setup --portal   (Nous Portal OAuth, no API key)")
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'hexbot core setup' to configure"))
    ...
    # Video generation — opt-in via `hexbot core tools` → Video Generation.
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'hexbot core setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'hexbot core setup tts'"))
    else:
    ...
    if _stt_feature is not None and _stt_feature.managed_by_nous:
        ...
        if fw_ok:
            ...
            tool_status.append(
                ("Speech-to-Text (Local Whisper — not installed)", False, "run 'hexbot core tools' → Speech-to-Text")
            )
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'hexbot core setup terminal'"))
    ...
    # Spotify (OAuth via hexbot core auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'hexbot core setup tools' to configure them,"
        )
    ...
    print()
    print(f"   {color('hexbot core setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('hexbot core setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('hexbot core setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('hexbot core setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('hexbot core setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('hexbot core config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('hexbot core config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('hexbot core config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('hexbot core', Colors.GREEN)}              Start chatting")
    print(f"   {color('hexbot core gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('hexbot core doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `hexbot core tools` and `hexbot core setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``hexbot core model``)
    ...
    provider added to ``hexbot core model`` is automatically available here.
    ...
    # Delegate to the shared hexbot core model flow — handles provider picker,
    ...
    try:
        ...
        print_info("You can try again later with: hexbot core model")
    ...
    # on demand via `hexbot core auth add`, `hexbot core setup` vision, and
    # `hexbot core setup tts`. This keeps both quick and full setup thin.
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``hexbot core model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.hexbot/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Hexbot auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via hexbot core setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'hexbot core setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Hexbot runs shell commands and code.")
    ...
    # under ~/.hexbot/plugins/). Fail-soft: a broken plugin must not take the
    # setup wizard down.
    ...
    if selected_backend == "local":
        ...
        # configurable later via `hexbot core setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `hexbot core setup terminal`.
        ...
        if prompt_yes_no("  Enable egress firewall for Docker sandboxes?", False):
            ...
            print_info(
                "Run `hexbot core egress setup` then `hexbot core egress start` to mint "
                "tokens and launch the proxy."
            )
        else:
            print_info(
                "Skipping egress firewall. You can enable it later with `hexbot core egress setup`."
            )
        ...
        try:
            ...
            # which() misses the uv Hexbot installed. Bootstrapping one is
...
def _apply_default_agent_settings(config: dict):
    ...
    print_info("  Run `hexbot core setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.hexbot/logs/tool_calls.log (gateway only)")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Hexbot to iMessage via BlueBubbles — a free, open-source")
...
def _setup_webhooks():
    ...
    print()
    print_info("   Open config in your editor:  hexbot core config edit")
    print_info("   Open config in your editor:  hexbot core config edit")
...
def setup_gateway(config: dict):
    ...
    print_info("Connect to messaging platforms to chat with Hexbot from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'hexbot core setup gateway' later to configure.")
    else:
        for idx in selected:
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     hexbot core config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
    ...
    # via `hexbot core import` or `hexbot core setup gateway`). Gating this on
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `hexbot core setup tools` and `hexbot core tools` use the same flow:
...
# Config values may have different semantics between OpenClaw and Hexbot.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Hexbot to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point Hexbot at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point Hexbot at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point Hexbot at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Hexbot at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to Hexbot equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Hexbot config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in Hexbot.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ Hexbot's yolo mode.", Colors.YELLOW))
...
def _offer_openclaw_migration(hermes_home: Path) -> bool:
    ...
    print_info("Hexbot can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: hexbot core claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: hexbot core claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Hexbot configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Hexbot config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Hexbot (use hexbot core claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``hexbot core setup --portal`` and ``hexbot core portal``. This is the
    ...
    Hexbot session — model selected, provider set, and web/image/tts/browser
    tools routed via their Portal sub — without being told to run
    ``hexbot core setup`` and hunt for the quick-setup option.
    ...
    (``_run_first_time_quick_setup``) and the same one ``hexbot core model`` runs
    when you pick Nous. Routing through it (instead of hand-rolling the auth +
    provider write here) means ``hexbot core portal`` always offers a model picker,
    ...
    print(color("│     ⚕ Hexbot Setup — Nous Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    # setup calls, so `hexbot core portal` == quick setup's Nous step.
    try:
        ...
        print_info("  You can retry later with `hexbot core portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_nous error during `hexbot core portal`: %s", exc)
        print()
    ...
    print_info("  Run `hexbot core portal info` to inspect routing.")
    print_info("  Run `hexbot core` to start chatting.")
...
def run_setup_action_with_navigation(
    ...
    *,
    ...
    Shared commands such as ``hexbot core model`` use the same provider/model
...
def _run_setup_wizard_impl(args):
    ...
    Supports full, quick, and section-specific setup:
      hexbot core setup           — full or quick (auto-detected)
      hexbot core setup model     — just model/provider
      hexbot core setup tts       — just text-to-speech
      hexbot core setup terminal  — just terminal backend
      hexbot core setup gateway   — just messaging platforms
      hexbot core setup tools     — just tool configuration
      hexbot core setup telemetry — just local shared metrics
      hexbot core setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ Hexbot Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│                ⚕ Hexbot Setup Wizard                   │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Hexbot installation.             │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        print_success("You already have Hexbot configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'hexbot core setup model|terminal|")
        ...
        setup_mode = prompt_choice(
            "How would you like to set up Hexbot?",
            [
    ...
    print()
    print_info("You can edit these files directly or use 'hexbot core config edit'")
    ...
    # Tune later with `hexbot core setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``hexbot core setup <section>``
    or switch providers with ``hexbot core model``.
    """
    from hermes_cli.config import load_config
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'hexbot core setup gateway'",
        ],
        0,
    )
    ...
    if gateway_choice == 0:
        ...
        # (e.g. via `hexbot core import` from another machine).
        from hermes_cli.gateway import ensure_gateway_service
        ensure_gateway_service(context="setup")
    ...
    print()
    print_info("  Configure all settings:    hexbot core setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  hexbot core setup gateway")
...
def _print_macos_fda_tip() -> None:
    ...
    print_info("  The grant is permanent — it survives every Hexbot update.")
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    configuring, and troubleshooting Hexbot) is always seeded — without
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``hexbot core tools`` (which
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``hexbot core setup agent`` /
    ``hexbot core config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, hermes_home, is_existing: bool):
    ...
    print_info("can help you drive and configure Hexbot itself.")
    print()
    ...
    if path == 0:
        ...
        # `hexbot core update` runs don't re-inject them. Essential skills (the
        ...
        print_info("  Enable tools:        hexbot core tools")
        print_info("  Seed skills:         hexbot core skills opt-in --sync")
        print_info("  Add MCP servers:     hexbot core mcp add")
        print_info("  Enable plugins:      hexbot core plugins")
        print_info("  Tune agent settings: hexbot core setup agent")
        print()
        _print_setup_summary(config, hermes_home)
        return
...
def _blank_slate_walkthrough(config: dict, hermes_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`hexbot core update` runs from re-injecting them. Opt back in any")
            print_info("time with `hexbot core skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `hexbot core tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `hexbot core plugins list` / `hexbot core plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `hexbot core plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `hexbot core mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `hexbot core mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   hexbot core tools")
    print_info("  Seed skills:         hexbot core skills opt-in --sync")
    print_info("  Add MCP servers:     hexbot core mcp add")
    print_info("  Tune agent settings: hexbot core setup agent")
    print()
...
def _run_quick_setup(config: dict, hermes_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'hexbot core setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Hexbot to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'hexbot core setup gateway'.")
