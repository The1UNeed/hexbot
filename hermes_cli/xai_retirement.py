...
Pure logic: walks a Hexbot config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `hexbot core doctor` and a future `hexbot core migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Hexbot config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Hexbot config and return retirement issues.
...
def apply_migration(
    ...
    # None``, so the next `hexbot core migrate xai` reports nothing to migrate
