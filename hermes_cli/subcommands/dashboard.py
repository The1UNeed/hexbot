"""``hexbot core dashboard`` / ``hexbot core serve`` subcommand parsers.
...
def _add_server_runtime_args(parser) -> None:
    ...
    # `hexbot core dashboard` / `hexbot core serve` cmdlines and SIGTERM them directly —
    # the same path `hexbot core update` uses to clean up stale servers.
    parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running Hexbot web server processes and exit",
    )
    parser.add_argument(
        "--status",
        action="store_true",
        help="List running Hexbot web server processes and exit",
    )
...
def build_dashboard_parser(
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Hexbot web dashboard for managing config, API keys, and sessions",
    )
    _add_server_runtime_args(dashboard_parser)
    dashboard_parser.add_argument(
        "--no-open", action="store_true", help="Don't open browser automatically"
    )
    # Backward-compat shim: older Hexbot desktop app shells (<= 0.15.x) spawn the
    # backend as `hexbot core dashboard --no-open --tui --host ... --port ...`. The
    ...
    # showed "Hexbot couldn't start" with no actionable cause. Accept and silently
    ...
    # browser. It exists so the Hexbot Desktop app (and headless remote
    ...
    serve_parser = subparsers.add_parser(
        "serve",
        help="Start the Hexbot backend server (headless; powers the desktop app and remote backends)",
        description=(
            "Run the Hexbot backend server — the JSON-RPC/WebSocket gateway the "
    ...
    # `hexbot core dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.hexbot/.env.
    # Nested subparser so bare `hexbot core dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "HERMES_DASHBOARD_OAUTH_CLIENT_ID into ~/.hexbot/.env, and prints "
            "how to engage the login gate. Requires being logged in (hexbot core setup)."
        ),
    )
