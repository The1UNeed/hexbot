"""``hexbot core sync`` subcommand parser — Skill Sync.
...
Skill Sync covers two surfaces, both under this one command for launch:
  ...
  Personal — your own skills, across your own devices:
    hexbot core sync status                 show gate/opt-in/head state
    hexbot core sync pull                   pull and materialize opted-in skills
    hexbot core sync push                   push opted-in skills
    hexbot core sync now                    reconcile: pull then push
    hexbot core sync enable <skill>         opt a skill into sync
    hexbot core sync disable <skill>        opt a skill out of sync
    hexbot core sync device [--name]        show or set this device's label

  Organisation — skills shared with your team:
    hexbot core sync propose <skill>        share a skill with your organisation
...
def build_sync_parser(subparsers, *, cmd_sync: Callable) -> None:
    ...
    sync_parser = subparsers.add_parser(
        ...
        epilog=(
            "Examples:\n"
            "  hexbot core sync status            what is synced, and from where\n"
            "  hexbot core sync enable my-skill   include a skill in your sync\n"
            "  hexbot core sync now               pull, then push\n"
            "  hexbot core sync propose my-skill  share a skill with your team\n"
        ),
