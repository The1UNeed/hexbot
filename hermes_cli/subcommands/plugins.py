"""``hexbot core plugins`` subcommand parser.
...
def build_plugins_parser(subparsers, *, cmd_plugins: Callable) -> None:
    ...
    plugins_parser = subparsers.add_parser(
        ...
        description=(
            "Install, update, remove, list, or validate native Hexbot plugins "
    ...
    plugins_install.add_argument(
        ...
        help=(
            ...
            "(see `hexbot core plugins search`)"
        ),
    )
    plugins_install.add_argument(
        "--force",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `hexbot core plugins enable <name>`",
    )
