"""``hexbot core debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Hexbot. Use 'hexbot core debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    hexbot core debug share              Upload debug report (asks for confirmation)
    hexbot core debug share --yes        Skip confirmation (for scripts/CI)
    hexbot core debug share --lines 500  Include more log lines
    hexbot core debug share --expire 30  Keep paste for 30 days
    hexbot core debug share --local      Print report locally (no upload)
    hexbot core debug share --no-redact  Disable upload-time secret redaction
    hexbot core debug share --nous       Upload to Nous-internal storage (private)
    hexbot core debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'hexbot core debug share'",
    )
