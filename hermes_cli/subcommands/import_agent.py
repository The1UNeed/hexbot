"""``hexbot core import-agent`` subcommand parser.

Follows the ``hexbot core claw`` pattern (see ``hermes_cli/subcommands/claw.py``):
...
def build_import_agent_parser(subparsers, *, cmd_import_agent: Callable) -> None:
    ...
    parser = subparsers.add_parser(
        "import-agent",
        help="Import a Claude Code or Codex CLI setup into Hexbot",
        description=(
            "One-command import of another coding agent's setup into Hexbot. "
            "Maps CLAUDE.md/AGENTS.md instructions, permission allowlists, MCP "
            "servers, skills, and memories into their Hexbot equivalents. "
            "Always shows a preview before making changes. API keys and "
            "credentials are never imported — run 'hexbot core setup' for those."
        ),
    )
    parser.add_argument(
        "agent",
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrite existing Hexbot items on name conflicts (default: skip)",
    )
    parser.add_argument(
