"""``hexbot core login`` subcommand parser.
...
def build_login_parser(subparsers, *, cmd_login: Callable) -> None:
    ...
    ``hexbot core login`` was removed in favor of ``hexbot core auth`` / ``hexbot core model``
    ...
    old scripts/aliases invoking ``hexbot core login [--flags]`` still receive the
    ...
    - The subparser is registered WITHOUT a ``help=`` kwarg so the row is
      omitted from ``hexbot core --help`` (argparse only lists subcommands that
    ...
    - ``--provider`` accepts ANY value (no ``choices=``) so that, e.g.,
      ``hexbot core login --provider anthropic`` reaches the deprecation handler and
      gets pointed at ``hexbot core model`` instead of crashing in argparse with
      ``invalid choice: 'anthropic'`` before the handler can run.
    """
    login_parser = subparsers.add_parser(
        "login",
        description=(
            "Deprecated. Use `hexbot core auth` to manage credentials, "
            "`hexbot core model` to select a provider, or `hexbot core setup` for full setup."
        ),
    )
    ...
    login_parser.add_argument(
        "--provider",
        default=None,
        help="(deprecated) Provider name; ignored — see `hexbot core model`",
    )
    login_parser.add_argument(
