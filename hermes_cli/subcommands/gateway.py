"""``hexbot core gateway`` and ``hexbot core proxy`` subcommand parsers.
...
def build_gateway_parser(
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.hexbot/.env. "
            "Requires being logged in (hexbot core setup). Not available in managed installs."
        ),
    )
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.hexbot/.env and forwarded at provision. "
    ...
    proxy_start.add_argument(
        "--provider",
        default="nous",
        help="Upstream provider: nous or xai (default: nous). See `hexbot core proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
