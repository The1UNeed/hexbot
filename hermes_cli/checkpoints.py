"""`hexbot core checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.hexbot/checkpoints/``.  Actions:

    hexbot core checkpoints               # same as `status`
    hexbot core checkpoints status        # total size, project count, breakdown
    hexbot core checkpoints list          # per-project checkpoint counts + workdir
    hexbot core checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    hexbot core checkpoints clear [-f]    # nuke the entire base (asks first)
    hexbot core checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    hexbot core checkpoints
    hexbot core checkpoints prune --retention-days 3 --max-size-mb 200
    hexbot core checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: hexbot core checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``hexbot core checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `hexbot core checkpoints` → status
