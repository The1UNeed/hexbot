"""Cross-process mutual exclusion for in-flight Hexbot updates.

Three different surfaces can start an update of the same install tree:

* ``hexbot core update`` from a terminal,
...
once — so a dashboard-spawned ``hexbot core update`` and an installer-driven
...
then spawns ``hexbot core update`` as a child stage. Without a handoff the child
sees its own parent's live marker and refuses — the GUI update deadlocks
against itself on every attempt ("Hexbot is still running", retry forever).
...
* A live holder that is a *process ancestor* of ours is likewise our own
  ...
  ``hermes-setup`` binary under ``~/.hexbot`` is only refreshed by a full
  ...
  ancestry check those users get exit 2 ("Hexbot is still running") on every
  GUI update forever, with no Hexbot process actually running.
"""
...
# its own pid before spawning `hexbot core update` as a child stage. The parent
...
# to show "Hexbot is still running" instead of a generic failure. Naming it
...
def update_marker_path() -> Path:
    ...
    Uses the *process* Hexbot home (never the context-local profile override):
...
def _is_ancestor_pid(pid: int) -> bool:
    ...
    The orchestrating updater spawns ``hexbot core update`` as a (grand)child, so a
...
def describe_holder(holder: UpdateHolder) -> str:
    ...
    return (
        f"✗ Another Hexbot update is already running (PID {holder.pid}, "
...
class UpdateLock:
    ...
    def acquire(self) -> bool:
        ...
        updater spawning `hexbot core update` as a stage): we run under ITS claim
