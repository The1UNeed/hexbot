...
def _resolve_discovery_timeout(
    ...
    When ``single_query`` is True (``hexbot core -z "..."`` / ``-q``), the larger
...
def mcp_discovery_in_flight() -> bool:
    ...
    ``hexbot core dashboard``).  Those processes populate THIS module's
...
def ensure_mcp_discovery_before_agent_build(
    *,
    ...
    Non-interactive first turns (``chat -q``, ``hexbot core -z``) can construct
