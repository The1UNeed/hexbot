"""``hexbot core dashboard register`` — register a self-hosted dashboard OAuth client.
...
resulting ``agent:{id}`` OAuth client ID, and paste it into ``~/.hexbot/.env``
as ``HERMES_DASHBOARD_OAUTH_CLIENT_ID``.

This command:
  1. Resolves a fresh Nous Portal access token from the existing login
     (``~/.hexbot/auth.json``), refreshing it if needed. Fails fast with a
     "run `hexbot core setup`" hint when the user isn't logged in.
  ...
  3. Writes ``HERMES_DASHBOARD_OAUTH_CLIENT_ID`` and (if absent)
     ``HERMES_DASHBOARD_PORTAL_URL`` into ``~/.hexbot/.env`` idempotently.
...
def _register_self_hosted_client(
    *,
    ...
    makes re-running ``hexbot core dashboard register`` idempotent. The portal
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                ...
                "Try `hexbot core auth add nous` to re-authenticate."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def _print_post_register_hint(
    *,
    ...
    print(
        ...
        "  `hexbot core dashboard` (localhost) leaves the gate off and serves locally\n"
    ...
    if custom_redirect_uri:
        ...
        print("    hexbot core dashboard --host 0.0.0.0")
...
def cmd_dashboard_register(args) -> None:
    ...
    if is_managed():
        print(
            "✗ `hexbot core dashboard register` is not available in a managed/hosted "
            "install.\n"
    ...
    try:
        ...
        if getattr(exc, "relogin_required", False):
            ...
            print("  Run `hexbot core setup` (or `hexbot core auth add nous`) first, then retry.")
        else:
