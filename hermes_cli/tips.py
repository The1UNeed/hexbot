...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or hexbot core -c.",
    ...
    "Ctrl+Z suspends Hexbot to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "hexbot core -c resumes your most recent CLI session. hexbot core -c \"project name\" resumes by title.",
    "hexbot core -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "hexbot core -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "hexbot core chat -t web,terminal enables only specific toolsets for a focused session.",
    "hexbot core chat -s github-pr-workflow preloads a skill at launch.",
    "hexbot core chat -q \"query\" runs a single non-interactive query and exits.",
    "hexbot core chat --max-turns 1000 overrides the default 500-iteration limit per turn.",
    "hexbot core chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "hexbot core --yolo bypasses all dangerous command approval prompts for the entire session.",
    "hexbot core chat --source telegram tags the session for filtering in hexbot core sessions list.",
    "hexbot core -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "hexbot core doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "hexbot core dump outputs a compact setup summary — great for bug reports.",
    "hexbot core config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "hexbot core config edit opens config.yaml in your default editor.",
    "hexbot core config check scans for missing or stale configuration options.",
    "hexbot core sessions browse opens an interactive session picker with search.",
    "hexbot core sessions stats shows session counts by platform and database size.",
    "hexbot core sessions prune --older-than 30 cleans up old sessions.",
    "hexbot core skills search react --source skills-sh searches the skills.sh public directory.",
    "hexbot core skills check scans installed hub skills for upstream updates.",
    "hexbot core skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "hexbot core skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "hexbot core mcp add github --command npx adds MCP servers from the command line.",
    "hexbot core mcp serve runs Hexbot itself as an MCP server for other agents.",
    "hexbot core auth add lets you add multiple API keys for credential pool rotation.",
    "hexbot core completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "hexbot core logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "hexbot core backup creates a zip backup of your entire Hexbot home directory.",
    "hexbot core profile create coder creates an isolated profile that becomes its own command.",
    "hexbot core profile create work --clone copies your current config and keys to a new profile.",
    "hexbot core update syncs new bundled skills to ALL profiles automatically.",
    "hexbot core gateway install sets up Hexbot as a system service (systemd/launchd).",
    "hexbot core memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "hexbot core webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: hexbot core tools disables unused tools, hexbot core skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "hexbot core model → Configure auxiliary models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as hexbot core -w).",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Hexbot tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'hexbot core profile create coder' creates the 'coder' command.",
    "hexbot core profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Hexbot prints a resume command with session ID and stats.",
    "hexbot core sessions export backup.jsonl exports all sessions for backup or analysis.",
    "hexbot core -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "hexbot core skills install official/security/1password installs optional skills from the repo.",
    ...
    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: hexbot core cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Hexbot runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "hexbot core gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Hexbot via a local macOS server.",
    ...
    "Hexbot loads project context from .hermes.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "hexbot core mcp opens an interactive picker of Nous-approved MCPs you can install in one keystroke.",
    "hexbot core mcp catalog lists Nous-approved MCP servers shipped with the repo.",
    "hexbot core mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — hexbot core tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.hexbot/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "hexbot core chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "hexbot core plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "hexbot core acp runs Hexbot as an ACP server for VS Code, Zed, and JetBrains integration.",
    ...
    "hexbot core auth add nous or hexbot core auth add openai-codex sets up OAuth-based providers.",
    ...
    "hexbot core status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.hexbot/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Hexbot your own.",
    ...
    "Before a gateway session reset, Hexbot auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.hexbot/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Profiles scope Hexbot state via HERMES_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    ...
    "Every interrupt during an agent run is logged to ~/.hexbot/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'hexbot core profile create ls' would be rejected.",
    "hexbot core profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "hexbot core claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via hexbot core tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.hexbot/.env.",
    "Skills from trusted repos (NousResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.hexbot/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Hexbot auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Hexbot config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Hexbot is working.',
    ...
    '/reload reloads ~/.hexbot/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.hexbot/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.hexbot/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'hexbot core curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "hexbot core curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'hexbot core curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'hexbot core auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'hexbot core dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'hexbot core dashboard embeds the full Hexbot TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.hexbot/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.hexbot/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.hexbot/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.hexbot/skills/ so drop-in skills appear without restarting the session.',
    ...
    'hexbot core -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'hexbot core chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'hexbot core chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'hexbot core chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "hexbot core chat --source tool tags programmatic chats so they don't clutter hexbot core sessions list.",
    'hexbot core dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'hexbot core sessions rename <ID> "new title" renames any past session; hexbot core sessions delete <ID> removes one.',
    'hexbot core import restores a full Hexbot backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'hexbot core fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'hexbot core pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'hexbot core setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'hexbot core status --deep runs the full health sweep across every component; plain hexbot core status is the quick view.',
    ...
    # --- Security ---
    'security.tirith_fail_open: false makes Hexbot block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from hexbot core sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.hexbot/dashboard-plugins/.',
]
...
# any project or none — Hexbot is not a coding-only agent.
# ---------------------------------------------------------------------------
