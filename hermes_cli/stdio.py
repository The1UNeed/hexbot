...
on Japanese locales, etc.).  Hexbot's banners, tool output feed, and slash
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Hexbot is running
...
def configure_windows_stdio() -> bool:
    ...
    # Augment PATH with the Hexbot-managed Git install directories so
    ...
    # install without Hexbot-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Hexbot's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Hexbot's
    ``hexbot core config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Hexbot (User env var in Windows Settings, or
    export in a PowerShell profile) and Hexbot picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Hexbot-managed tool directories to os.environ['PATH'].
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``hexbot core`` from right after install) retain their old PATH.

    Any subprocess Hexbot spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Hexbot-managed tool directories to our
    ...
    candidate_dirs = [
        ...
        # Hexbot venv Scripts directory — host of the hermes.exe shim itself,
