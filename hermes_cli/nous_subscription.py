...
# `hexbot core tools` picker scope its entitlement gate to the selected backend, so a
...
def _selected_provider(section: object, name_key: str = "provider") -> Optional[str]:
    ...
    with the runtime resolver is what stops ``hexbot core status`` from lying.
    """
    if not isinstance(section, dict):
        return None
...
def _has_agent_browser() -> bool:
    ...
    try:
        ...
        # Hexbot-managed Node dirs (Windows installer / POSIX $HERMES_HOME/node)
...
def get_nous_subscription_features(
    ...
    *,
    ...
    # Keyless Tavily is opt-in: selecting it in `hexbot core tools` / setup writes
...
# ---------------------------------------------------------------------------
# Inline Nous Portal login for the Tool Gateway picker (`hexbot core tools`)
# ---------------------------------------------------------------------------
...
def ensure_nous_portal_access(
    *,
    ...
    Used by ``hexbot core tools`` when a user selects a Nous-managed Tool Gateway
    backend (e.g. "Firecrawl (Nous Portal)").  Unlike ``hexbot core model``'s Nous
    login, this:
