"""``hexbot core debug`` debug tools for Hexbot.

Currently supports:
    hexbot core debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.hexbot/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[hexbot core debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.hexbot/pastes/pending.json``.
    ...
    ran ``hexbot core debug share`` repeatedly.

    Deletion is now driven by the gateway's cron ticker
    (``gateway/run.py::_start_cron_ticker``) which calls
    ``_sweep_expired_pastes`` once per hour.  ``hexbot core debug share`` also
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``hexbot core debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``hexbot core debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `hexbot core debug share` "
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``hexbot core debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.hexbot/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``hexbot core debug share`` also runs an opportunistic sweep as a fallback
...
# desktop app talks to a remote/docker/SSH backend, `hexbot core debug share` runs
...
_CLIENT_SIDE_LOGS = {
    "desktop": (
        "written by Hexbot Desktop on the machine running the app, not by this "
...
def _capture_log_snapshot(
    ...
    *,
    ...
    ``hexbot core debug share --no-redact``).
    """
...
def _capture_dump() -> str:
    """Run ``hexbot core dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``hexbot core dump`` is run
        internally.
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``hexbot core debug share`` (CLI) and the
    ...
    if redact:
        logger.info(
            "hexbot core debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    try:
        ...
        print("\nRun `hexbot core debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  hexbot core debug delete <url>")

    print("\nShare these links with the Hexbot team for support.")
...
def _run_debug_share_nous(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``hexbot core debug share --nous``: upload the bundle to Nous-S3.
    ...
    if redact:
        logger.info(
            "hexbot core debug share --nous: applied force-mode redaction before upload"
        )
    ...
    try:
        ...
        print(
            ...
            "provisioned.\n"
            "Run `hexbot core debug share --local` to print the report instead, "
            "or `hexbot core debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: hexbot core debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'hexbot core debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``hexbot core debug`` call.
    ...
    # best-effort — any failure is swallowed so ``hexbot core debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: hexbot core debug <command>")
        print()
        print("Commands:")
