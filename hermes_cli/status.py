"""
Status command for the Hexbot core CLI.

Shows the status of all Hexbot components.
"""
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``hexbot core config``'s
...
def _estop_status_line():
    """One-line pause banner for `hexbot core status`, or None when not paused.
    ...
    return f"⏸️  PAUSED (global emergency stop{suffix}; `hexbot core resume` to lift)"


def show_status(args):
    """Show status of all Hexbot components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                    ⚕ Hexbot Status                     │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    try:
        ...
        # Read-only display: use the refresh-free snapshot so `hexbot core status`
    ...
    if nous_logged_in:
        ...
        nous_label = "not logged in (run: hexbot core portal)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: hexbot core model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: hexbot core auth add minimax-oauth)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: hexbot core auth add xai-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: hexbot core model)"
    ...
    if terminal_env == "ssh":
        ...
        # provider's doctor rows (fail-soft — never break `hexbot core status`).
        try:
    ...
    try:
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   hexbot core gateway")
    ...
    print(color("  Run 'hexbot core doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'hexbot core setup' to configure", Colors.DIM))
    print()
