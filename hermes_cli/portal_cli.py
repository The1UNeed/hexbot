"""``hexbot core portal`` — the human-readable entry point for Nous Portal.

Running ``hexbot core portal`` with no subcommand performs the one-shot Portal
...
``hexbot core auth add nous --type oauth`` (which still works), is identical to
``hexbot core setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``hexbot core auth`` or ``hexbot core tools``. It's the onboarding + discovery
...
def _cmd_status(args) -> int:
    ...
    if logged_in:
        ...
        print("  Login:   hexbot core portal")
    ...
    if provider == "nous":
        ...
        print(f"  Model:   currently {provider} (switch with `hexbot core model`)")
...
def _cmd_tools(args) -> int:
    ...
    if not features.nous_auth_present:
        print(color("  Not logged into Nous Portal — sign in with `hexbot core portal`.", Colors.YELLOW))
        print()
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `hexbot core auth add nous --type
    oauth`. It reuses the exact wiring behind `hexbot core setup --portal` (which in
...
def portal_command(args) -> int:
    """Top-level dispatch for `hexbot core portal <subcommand>`."""
    sub = getattr(args, "portal_command", None)
    if sub in {None, "", "login"}:
        # Default to the one-shot onboarding — `hexbot core portal` is the
        # human-readable alias for `hexbot core auth add nous --type oauth` /
        # `hexbot core setup --portal`.
    ...
    print("Run `hexbot core portal -h` for usage.", file=sys.stderr)
    return 1


def add_parser(subparsers) -> None:
    """Register `hexbot core portal` on the given argparse subparsers object."""
    portal_parser = subparsers.add_parser(
        "portal",
        help="Set up Nous Portal (login, model pick, Tool Gateway); see also `portal info`",
        description=(
            "Run `hexbot core portal` with no subcommand to log in to Nous Portal "
            "and set it up — pick a model, set Nous as your provider, and offer "
            "the Tool Gateway (the human-readable alias for `hexbot core auth add "
            "nous --type oauth`, identical to `hexbot core setup --portal`). "
