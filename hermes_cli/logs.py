"""``hexbot core logs`` — view and filter Hexbot log files.
...
Usage examples::
    ...
    hexbot core logs                    # last 50 lines of agent.log
    hexbot core logs -f                 # follow agent.log in real time
    hexbot core logs errors             # last 50 lines of errors.log
    hexbot core logs gateway -n 100    # last 100 lines of gateway.log
    hexbot core logs gui -f            # follow gui.log (dashboard/pty/ws)
    hexbot core logs desktop -f        # follow desktop.log (Electron app boot/backend)
    hexbot core logs --level WARNING    # only WARNING+ lines
    hexbot core logs --session abc123   # filter by session ID substring
    hexbot core logs --component tools  # only tool-related lines
    hexbot core logs --since 1h         # lines from the last hour
    hexbot core logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if not log_path.exists():
        ...
        print("(Logs are created when Hexbot runs — try 'hexbot core chat' first)")
        sys.exit(1)
...
def list_logs() -> None:
    ...
    if not found:
        print("  (no log files yet — run 'hexbot core chat' to generate logs)")
