...
def auth_add_command(args) -> None:
    ...
    if provider == "nous":
        ...
        # full device-code flow. This makes `hexbot core --profile <name>
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``hexbot core auth add openai-codex`` overwrote
    ...
    if provider == "xai-oauth":
        ...
        # The singleton round-trip collapsed every added account into the
        # latest login: a second ``hexbot core auth add xai-oauth`` overwrote the
    ...
    raise SystemExit(f"`hexbot core auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Hexbot reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `hexbot core auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `hexbot core auth` is called bare."""
