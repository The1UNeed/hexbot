"""Pydantic request/response models for the Hexbot dashboard web server.
...
class ModelAssignment(BaseModel):
    ...
    # ``hexbot core model`` custom flow collects.
    api_key: str = ""
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `hexbot core import`. The spawned action runs with
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`hexbot core -p <name> skills install`)
