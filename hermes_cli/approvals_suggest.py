"""``hexbot core approvals suggest`` — mine approval history into allowlist proposals.

Hexbot has no dedicated approval-decision ledger: ``always`` answers land in
...
is the session DB (``~/.hexbot/state.db``): every assistant ``terminal`` tool
...
* **Destructive / privilege / credential / obfuscation classes are never
  ...
  restarts, Hexbot self-management, …) are eligible.
...
def _render_text(proposals: list[Proposal], days: int) -> None:
    ...
    print(
        ...
        "  hexbot core approvals suggest --apply 1,3\n"
        "Entries are merged into command_allowlist in ~/.hexbot/config.yaml."
    )


def suggest_command(args) -> int:
    """Entry point for ``hexbot core approvals suggest``."""
...
def approvals_command(args) -> int:
    """Dispatch ``hexbot core approvals <subcommand>``."""
    ...
    print(
        "usage: hexbot core approvals <subcommand>\n"
        "\n"
        "subcommands:\n"
        ...
        "\n"
        "Run `hexbot core approvals <subcommand> -h` for details."
    )
    return 1
