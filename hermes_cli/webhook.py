"""hexbot core webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    hexbot core webhook subscribe <name> [options]
    hexbot core webhook list
    hexbot core webhook remove <name>
    hexbot core webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.hexbot/webhook_subscriptions.json and are
...
def _setup_hint() -> str:
  ...
  Then start the gateway: hexbot core gateway run
"""
...
def webhook_command(args):
    """Entry point for 'hexbot core webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: hexbot core webhook {subscribe|list|remove|test}")
        print("Run 'hexbot core webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (hexbot core gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: hexbot core webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from hexbot core webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (hexbot core gateway run)")
