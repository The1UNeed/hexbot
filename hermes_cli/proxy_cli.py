"""CLI handlers for ``hexbot core egress ...``.
...
The top-level command is ``hexbot core egress``.  Note that the inbound OAuth
reverse-proxy command (``hexbot core proxy``) lives elsewhere in
...
def register_cli(parent_parser: argparse.ArgumentParser) -> None:
    ...
    ``hexbot core egress`` parser.
    """

    # dest='egress_command' — keeps this subparser tree disjoint from the
    # inbound OAuth ``hexbot core proxy`` subparser (which uses dest='proxy_command').
    ...
    setup.add_argument(
        ...
        help="Do not restart a running daemon after setup; you'll need to run "
             "`hexbot core egress restart` yourself for changes to take effect.",
    )
...
def cmd_setup(args: argparse.Namespace) -> int:
    ...
    if args.from_bitwarden:
        ...
        if not bw_cfg.get("enabled"):
            ...
            console.print(
                "  Run `hexbot core secrets bitwarden setup` first, or omit "
                "--from-bitwarden."
            )
            return 1
        try:
        ...
        # provider keys only in ~/.hexbot/.env (loaded automatically when the
        # agent runs, but NOT exported into an interactive shell).  Fall back
        # to loading that file so `hexbot core egress setup` finds the same keys the
        ...
        if loaded:
            console.print(
                ...
                f"~/.hexbot/.env for discovery.[/dim]"
            )
    ...
    if rotate and existing:
        ...
        if _sys.stdin.isatty():
            console.print(
                ...
                "tokens in every running Hexbot sandbox.  They will start "
    ...
    # `hexbot core egress reload` apply future ruleset changes without a
    ...
    # rotation), running `hexbot core egress setup` again WITHOUT
    ...
    if do_restart:
        try:
            ...
            console.print(
                "  Run [cyan]hexbot core egress start[/cyan] manually before "
        ...
        console.print(
            ...
            "changed.  Run [cyan]hexbot core egress restart[/cyan] (or "
    ...
    console.print(
        "  Start:   [cyan]hexbot core egress start[/cyan]\n"
        "  Restart: [cyan]hexbot core egress restart[/cyan]  (after any re-setup)\n"
        "  Reload:  [cyan]hexbot core egress reload[/cyan]   (apply ruleset edits "
        "in-place, no restart)\n"
        "  Status:  [cyan]hexbot core egress status[/cyan]\n"
        "  Stop:    [cyan]hexbot core egress stop[/cyan]\n"
        "  Disable: [cyan]hexbot core egress disable[/cyan]"
    )
    return 0
...
def cmd_start(args: argparse.Namespace) -> int:
    ...
    if not proxy_cfg.get("enabled"):
        console.print(
            "[yellow]proxy.enabled is false — run `hexbot core egress setup` "
            "first.[/yellow]"
        )
        return 1
    ...
    if credential_source == "bitwarden" and not refresh_bw:
        if bool(proxy_cfg.get("allow_env_fallback", False)):
            ...
            console.print(
                ...
                "back to env credentials with `hexbot core egress setup "
    ...
    if refresh_bw:
        ...
        if not os.environ.get(bw_access_env, "").strip():
            ...
            console.print(
                ...
                "`hexbot core egress setup --no-bitwarden` to switch back to "
        ...
        if not (bw_cfg or {}).get("project_id"):
            ...
            console.print(
                "  Run `hexbot core secrets bitwarden setup` to configure the "
                "project, or switch back via `hexbot core egress setup "
                "--no-bitwarden`."
            )
            return 1
...
def cmd_reload(args: argparse.Namespace) -> int:
    ...
    ``hexbot core egress restart`` instead: the daemon reads real credentials
    ...
    console.print(
        ...
        "still need `hexbot core egress restart` — the daemon reads real "
...
def format_status_text(*, show_tokens: bool = False) -> str:
    ...
    if bool(proxy_cfg.get("enabled")) and not status.configured:
        lines.extend(["", "Next: run `hexbot core egress setup` to mint tokens and write proxy.yaml."])
    elif bool(proxy_cfg.get("enabled")) and not (status.pid and status.listening):
        lines.extend(["", "Next: run `hexbot core egress start` before launching Docker sandboxes."])
...
def cmd_disable(args: argparse.Namespace) -> int:
    ...
    if ip.get_status().pid is not None:
        console.print(
            ...
            "[cyan]hexbot core egress stop[/cyan] if you want it down too."
        )
    return 0
...
def cmd_config(args: argparse.Namespace) -> int:
    ...
    if status.config_path is None:
        console.print(
            "[yellow](no config generated — run `hexbot core egress setup`)[/yellow]"
        )
        return 1
...
def _load_env_file_into_environ() -> int:
    """Backfill provider keys from ``~/.hexbot/.env`` into ``os.environ``.

    ``hexbot core egress setup`` discovers providers by reading ``os.environ``, but
    many operators keep their keys ONLY in ``~/.hexbot/.env`` (which the agent
