...
this. Capabilities govern the *host API surfaces* Hexbot hands out (which
...
until the user re-consents (``hexbot core plugins update`` surfaces the diff).
...
def parse_declared_capabilities(raw: Any, plugin_name: str = "?") -> List[str]:
    ...
    for a newer Hexbot may declare ids this build doesn't know; they can
