...
Outputs a compact, plain-text summary of the user's Hexbot setup
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.hexbot/.env.
    ...
    exports. ``hexbot core debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _redact(value: str) -> str:
    ...
    ``hexbot core dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def run_dump(args):
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.hexbot/.env: a managed
        ...
        # A credential added via `hexbot core auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `hexbot core auth list` shows it (#42130).
