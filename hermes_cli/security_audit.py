"""On-demand supply-chain audit for Hexbot installs.

Scans three surfaces a Hexbot user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Hexbot venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.hexbot/plugins``
...
def _discover_plugins(hermes_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.hexbot/plugins``.
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `hexbot core security audit`."""
