...
``<root>`` is the **shared Hexbot root** (the parent of any active
...
``hexbot core -p <profile>`` joins the same board as the dispatcher that
...
the profile's Hexbot home but are otherwise isolated: a worker spawned
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``hexbot core kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.hexbot``. In Docker / custom
deployments where ``HERMES_HOME`` points outside ``~/.hexbot`` (e.g.
...
def kanban_home() -> Path:
    """Return the shared Hexbot root that anchors the kanban board.
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``hexbot core kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    2. ``<root>/kanban/current`` on disk (set by ``hexbot core kanban boards
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``hexbot core kanban boards switch <typo>`` returns an error
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``hexbot core kanban log`` unambiguous even when multiple
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``hexbot core gateway run --replace`` /
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    **Narrow auto-repair:** when the integrity failure consists *only* of
    ...
    all sources Hexbot treats as user-controlled-but-trusted on the
...
def repair_db(
    ...
    *,
    ...
    :class:`RepairResult` so ``hexbot core kanban repair`` can report and choose
    its own exit code.
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
    ...
    # holder (e.g. an external `hexbot core kanban list` mid-integrity-probe) block
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``hexbot core kanban init`` and the
...
def create_task(
    ...
    *,
    ...
    each name to ``hexbot core --skills ...``. Use this to pin a task to a
    ...
    # `hexbot core --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``hexbot core kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `hexbot core kanban recompute` (issue #22459).
...
def store_attachment_bytes(
    ...
    *,
    ...
    (``hexbot core kanban attach``) so name-sanitisation, the size cap, and the
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``hexbot core kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``hexbot core kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``hexbot core kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``hexbot core kanban complete <id>``) works without requiring
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Hexbot's own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Hexbot-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``hexbot core kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Hexbot checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Hexbot treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Hexbot creates/reuses that linked worktree. With
      no ``workspace_path``, Hexbot anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Hexbot uses
      ``wt/<task-id>``.
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Hexbot
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``hexbot core kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``hexbot core kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Hexbot profile.
...
def review_dispatch_enabled() -> bool:
    ...
    The default is true because Hexbot ships the ``sdlc-review`` skill and the
...
# Assumed per-worker memory footprint for the derived default cap. Hexbot
...
def resolve_max_in_progress(configured: Optional[int]) -> Optional[int]:
    ...
    Callers that parse config (gateway dispatcher, ``hexbot core kanban dispatch``)
...
def _dispatch_once_locked(
    ...
    *,
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Hexbot profile.
        # `_default_spawn` invokes ``hexbot core -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Hexbot
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `hexbot core kanban tail` — without
    ...
    # Auto-dispatch is enabled by default because Hexbot bundles the
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound Hexbot CLI invocation."""
    # ``hermes_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``hexbot core`` package — there is no
    # ``hexbot core`` package to import.
    return [sys.executable, "-m", "hermes_cli.main"]


def _absolute_hermes_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Hexbot shim."""
...
def _hermes_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Hexbot executable path.
...
def _resolve_hermes_argv() -> list[str]:
    """Resolve the ``hexbot core`` invocation as argv parts for ``Popen``.
    ...
    3. ``sys.executable -m hermes_cli.main`` — fallback for setups where
       Hexbot is launched from a venv and the ``hexbot core`` shim is not on
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``hexbot core -p <profile> chat -q ...`` subprocess.
    ...
    # env, and when the child process starts `hexbot core -p <name>` the
    ...
    # read on the board and in `hexbot core kanban log` — it is not a conversation
    ...
    # that even when the worker activates a profile (`hexbot core -p <name>`
    ...
    # `hexbot core -p <assignee>` activates the profile, but the env var is
    ...
    # `hexbot core kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`hexbot core` executable not found on PATH. "
            "Install Hexbot or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``hexbot core kanban daemon`` is systemd-friendly.
    ...
    dispatcher and ``hexbot core kanban dispatch`` — the standalone daemon must
    ...
    while not stop_event.is_set():
        try:
            ...
            # dispatcher and `hexbot core kanban dispatch` do (OOF-30): explicit
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Hexbot root exists
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``hexbot core kanban assignees`` for the terminal.
