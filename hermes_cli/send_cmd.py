"""CLI subcommand: ``hexbot core send`` — pipe text from shell scripts to any
...
def _read_message_body(
    ...
    if file_path:
        ...
        try:
            ...
            print(
                f"hexbot core send: {file_path} is not a text file. --file reads the "
                ...
                f'  hexbot core send --to telegram "MEDIA:{file_path}"\n'
                f'  hexbot core send --to telegram "optional caption MEDIA:{file_path}"\n'
                "Add [[as_document]] to deliver an image as an uncompressed file:\n"
                f'  hexbot core send --to telegram "[[as_document]] MEDIA:{file_path}"',
                file=sys.stderr,
            )
            sys.exit(_USAGE_EXIT)
        except OSError as exc:
            print(f"hexbot core send: cannot read {file_path}: {exc}", file=sys.stderr)
            sys.exit(_USAGE_EXIT)
    ...
    *,
    ...
    if json_mode:
        ...
        if payload.get("error"):
            print(f"hexbot core send: {payload['error']}", file=sys.stderr)
...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    try:
        ...
        print(f"hexbot core send: failed to load channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT

    try:
        raw = load_directory()
    except Exception as exc:
        print(f"hexbot core send: failed to read channel directory: {exc}", file=sys.stderr)
        return _FAILURE_EXIT
    ...
    # setup used only for outbound `hexbot core send`) would otherwise be
    ...
    if platform_filter:
        ...
        if not filtered:
            print(
                f"hexbot core send: no targets found for platform '{platform_filter}'. "
    ...
    if not platforms:
        ...
        print("Set one up with `hexbot core gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.hexbot/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hermes_env() -> None:
    """Populate ``os.environ`` from ``~/.hexbot/.env`` AND bridge top-level
    ...
    startup that ``hexbot core send`` must replicate when invoked standalone:

    1. ``load_dotenv(~/.hexbot/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.hexbot/config.yaml`` into
       ...
       via ``hexbot core config set``.

    See ``gateway/run.py`` for the canonical version of this bridge — we
    intentionally reimplement the minimum needed here so ``hexbot core send``
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.hexbot/.env and ~/.hexbot/config.yaml into os.environ so the
    ...
    if not target:
        print(
            "hexbot core send: --to PLATFORM[:channel[:thread]] is required\n"
            "Examples:\n"
            "  hexbot core send --to telegram \"hello\"\n"
            "  hexbot core send --to discord:#ops --file report.md\n"
            "  hexbot core send --list      # list available targets",
            file=sys.stderr,
        )
        sys.exit(_USAGE_EXIT)
    ...
    if message is None or not message.strip():
        print(
            "hexbot core send: no message provided. Pass text as a positional "
    ...
    # Import lazily so `hexbot core send --help` stays fast and does not pull in
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            "Pipe text from any shell script to any messaging platform Hexbot "
            "is already configured for. Reuses the gateway's platform "
            "credentials (~/.hexbot/.env + ~/.hexbot/config.yaml) — no LLM, "
            "no agent loop, no running gateway required for bot-token "
            "platforms like Telegram/Discord/Slack/Signal."
        ),
        epilog=(
            "Examples:\n"
            "  hexbot core send --to telegram \"deploy finished\"\n"
            "  echo \"RAM 92%\" | hexbot core send --to telegram:-1001234567890\n"
            "  hexbot core send --to discord:#ops --file /tmp/report.md\n"
            "  hexbot core send --to slack:#eng --subject \"[CI]\" --file build.log\n"
            "  hexbot core send --to telegram \"MEDIA:/tmp/chart.png\"   # send a media attachment\n"
            "  hexbot core send --list                  # all platforms\n"
            "  hexbot core send --list telegram         # filter by platform\n"
            "\n"
    ...
    parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="List available targets. Optional positional filter: `hexbot core send --list telegram`.",
    )
