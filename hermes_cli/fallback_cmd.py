"""
hexbot core fallback — manage the fallback provider chain.
...
Subcommands:
  hexbot core fallback [list]   Show the current fallback chain (default when no subcommand)
  hexbot core fallback add      Pick provider + model via the same picker as `hexbot core model`,
                           then append the selection to the chain
  hexbot core fallback remove   Pick an entry to delete from the chain
  hexbot core fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.hexbot/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        ...
        # `hexbot core model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  hexbot core fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `hexbot core model`, then append the selection to the chain."""
    ...
    print("  `hexbot core model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `hexbot core fallback list` to view, or `hexbot core fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``hexbot core fallback [subcommand]``."""
