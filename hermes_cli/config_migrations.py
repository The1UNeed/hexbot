...
def support_floor_message() -> str:
    ...
    return (
        ...
        f"{display_hermes_home()}/config.yaml and run `hexbot core setup` to "
        ...
        if not quiet:
            if grandfathered:
                ...
                print(
                    ...
                    "Use `hexbot core plugins enable <name>` to activate."
                )
...
def _migrate_to_23(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # settings in their `config.yaml`, and `hexbot core curator status` has no
    ...
    #   3. Creates `~/.hexbot/logs/curator/` if missing (belt-and-suspenders
        ...
        if added_curator:
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_curator)}) — edit via `hexbot core config set`"
                )
        if added_aux:
            results["config_added"].append(
            ...
            if not quiet:
                print(
                    ...
                    f"({', '.join(added_aux)}) — edit via `hexbot core config set`"
                )
...
# behavior set it to true explicitly via `hexbot core config set`.
...
def _migrate_to_39(results: Dict[str, Any], quiet: bool) -> None:
    ...
    # provider surface (`video_generate`, `hexbot core tools` → Video Generation).
    ...
    if changed:
        ...
        if not quiet:
            print(
                ...
                "lists — video generation now lives under `hexbot core tools` → "
