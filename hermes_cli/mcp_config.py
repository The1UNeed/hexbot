"""
MCP Server Management CLI — ``hexbot core mcp`` subcommand.

Implements ``hexbot core mcp add/remove/list/test/configure`` for interactive
MCP server lifecycle management (issue #690 Phase 2).

Relies on tools/mcp_tool.py for connection/discovery and keeps
configuration in ~/.hexbot/config.yaml under the ``mcp_servers`` key.
"""
...
def _resolve_mcp_server_config(config: dict) -> dict:
    ...
    ``~/.hexbot/.env`` into ``os.environ`` and recursively interpolate any
...
def _oauth_tokens_present(name: str) -> bool:
    ...
    Used after ``hexbot core mcp login`` to distinguish a genuine authentication
...
def cmd_mcp_add(args):
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  hexbot core mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  hexbot core mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  hexbot core mcp add myserver --preset mypreset')
        return
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            if _save_mcp_server(name, server_config):
                ...
                _info("Fix the issue, then: hexbot core mcp test " + name)
        return
...
def cmd_mcp_remove(args):
    ...
    # earlier `hexbot core mcp test` in the same session) is evicted too.
    try:
        from tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  hexbot core mcp add <name> --url <endpoint>')
        _info('  hexbot core mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def _reauth_oauth_server(name: str, server_config: dict) -> bool:
    ...
    landed before reporting success. Shared by ``hexbot core mcp login`` and
    ``hexbot core mcp reauth`` so both behave identically for a single server.
    """
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `hexbot core mcp remove` + `hexbot core mcp add` to reconfigure auth.")
        return False
    ...
    #
    # force_interactive_oauth: `hexbot core mcp login` is *explicitly* user-
    # initiated even when stdin isn't a TTY (Hexbot desktop / agent-
    ...
    try:
        ...
        if not _oauth_tokens_present(name):
            ...
            print()
            _info("Then re-run `hexbot core mcp login " + name + "`.")
            return False
        if tools:
...
def cmd_mcp_reauth(args):
    ...
    ``hexbot core mcp reauth <name>`` re-auths a single server (same as ``login``).
    ``hexbot core mcp reauth --all`` discovers every ``auth: oauth`` server in
    ...
    if not name:
        ...
        _info("Usage: hexbot core mcp reauth <name>   |   hexbot core mcp reauth --all")
        return
    if name not in servers:
        _error(f"Server '{name}' not found in config.")
        if servers:
...
# ─── hexbot core mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'hexbot core mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``hexbot core mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("hexbot core mcp                                    Open the catalog picker (default)")
        _info("hexbot core mcp catalog                            List Nous-approved MCPs")
        _info("hexbot core mcp install <name>                     Install a catalog MCP")
        _info("hexbot core mcp serve                              Run as MCP server")
        _info("hexbot core mcp add <name> --url <endpoint>        Add a custom MCP server")
        _info("hexbot core mcp add <name> --command <cmd>         Add a stdio server")
        _info("hexbot core mcp add <name> --preset <preset>       Add from a known preset")
        _info("hexbot core mcp remove <name>                      Remove a server")
        _info("hexbot core mcp list                               List configured servers")
        _info("hexbot core mcp test <name>                        Test connection")
        _info("hexbot core mcp configure <name>                   Toggle tools")
        _info("hexbot core mcp login <name>                       Re-authenticate OAuth")
        _info("hexbot core mcp reauth <name> | --all              Re-auth one or all OAuth servers")
        print()
