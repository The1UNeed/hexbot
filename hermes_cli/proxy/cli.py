"""CLI handlers for the ``hexbot core proxy`` subcommand."""
...
def _print_aiohttp_missing() -> None:
    print(
        "hexbot core proxy requires aiohttp. Run `hexbot core setup` to install it.",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"hexbot core auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Hexbot proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Hexbot proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: hexbot core proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``hexbot core proxy <subcommand>``."""
    ...
    print(
        "hexbot core proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  hexbot core proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  hexbot core proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  hexbot core proxy providers\n"
