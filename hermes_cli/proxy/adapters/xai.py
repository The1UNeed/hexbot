...
# xAI's public API is OpenAI-compatible for the endpoints Hexbot commonly
# uses. The Responses endpoint is included because Hexbot's native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Hexbot-managed OAuth credentials."""

    auth_hint = "hexbot core auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`hexbot core auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`hexbot core auth reset xai-oauth` or re-authenticate with "
                    "`hexbot core auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `hexbot core auth add xai-oauth --type oauth`."
            )
