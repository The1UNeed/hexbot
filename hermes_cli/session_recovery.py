"""Offline, non-destructive recovery for a damaged Hexbot session database.
    ...
    try:
        ...
        if before != after:
            raise SessionRecoverySafetyError(
                ...
                "Stop every Hexbot process using this profile and retry. "
                "This includes the interactive `hexbot core` CLI session this "
                ...
                "Hexbot process even after the gateway is stopped. Run the "
                "recovery from a fresh shell with no `hexbot core` session open, "
