"""Helpers for loading Hexbot .env files consistently across entrypoints."""
...
# Used by setup / `hexbot core model` flows to label detected credentials so
...
def _known_hermes_env_keys() -> set[str]:
    """Return the combined set of known Hexbot env-var keys.
...
# Behavioral routing keys a parent Hexbot process injects into child env and
...
def _clear_known_keys_missing_from_dotenv(path: Path) -> None:
    """Remove inherited profile-managed Hexbot keys absent from ``.env``.
    ...
    parent Hexbot process injects and that silently change *which provider
...
def _sanitize_loaded_credentials() -> None:
    ...
    for key, value in list(os.environ.items()):
        ...
        print(
            ...
            "  provider's dashboard and run `hexbot core setup` (or edit the "
...
def load_hermes_dotenv(
    *,
    ...
    """Load Hexbot environment files with user config taking precedence.

    Behavior:
    - `~/.hexbot/.env` overrides stale shell-exported values when present.
    ...
    if user_env.exists():
        ...
        # Mirror reload_env() known-key cleanup so inherited Hexbot keys
    ...
    # 2. A fresh ``hexbot core update`` retry just completed a deferred dependency
    ...
    # TERMINAL_ENV=docker left in ~/.hexbot/.env (e.g. written by an older
    # `hexbot core setup` before the user switched terminal.backend in config.yaml)
...
def _apply_external_secret_sources(home_path: Path) -> None:
    ...
    to locate bootstrap tokens) but BEFORE the rest of Hexbot reads
    ...
    if report.applied_any:
        ...
        _sanitize_loaded_credentials()
        # Remember where each var came from so setup / `hexbot core model`
...
def _load_secrets_config(home_path: Path) -> dict:
    ...
    # first config.yaml read in a normal `hexbot core` startup, so populating the
