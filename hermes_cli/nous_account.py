...
def format_nous_portal_entitlement_message(
    ...
    *,
    ...
    if account_info is None:
        return (
            f"Hexbot could not verify your Nous Portal entitlement, so {capability} "
            f"is unavailable. Run `hexbot core model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"Nous inference credentials are configured, but Hexbot cannot verify "
                f"your Nous Portal paid access for {capability}. Log in with "
                f"`hexbot core model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Nous Portal to use {capability}: run `hexbot core model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Hexbot could not verify your Nous Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `hexbot core model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Hexbot could not find a Nous Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `hexbot core model` to "
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `hexbot core model` to refresh Hexbot."
        return message
...
def _no_paid_access_message(
    ...
    if access and access.member_spend_cap_exceeded:
        ...
        return (
            ...
            f" member spend cap at {billing_url}, then run `hexbot core model`"
            f" to refresh."
        )
...
def nous_policy_notice(*, removed: bool) -> str:
    ...
    A blocked model is omitted rather than marked, which reads as "Hexbot does
