...
class PlatformEntry:
    ...
    # status displays (``hexbot core setup``, ``hexbot core status``, the dashboard
    ...
    # Env vars this platform needs (for ``hexbot core setup`` display).
    ...
    # built-ins).  Used by ``hexbot core gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `hexbot core`
        # invocation -- including plain `hexbot core chat`, which never touches any
        ...
        # `hexbot core setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``hexbot core setup``/``gateway status``, channel
