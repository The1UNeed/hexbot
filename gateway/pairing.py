...
# ``hexbot core pairing`` CLI) re-imports this module later with the final
...
class PairingStore:
    ...
    as ``hexbot core -p <name> pairing ...``. This keeps multiplex gateways and
    ...
    def __init__(self, profile: Optional[str] = None):
        ...
        if profile:
            ...
            # ``hexbot core -p <profile> pairing ...`` process does. Merge the
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `hexbot core` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> hexbot core pairing approve ...` as root, "
