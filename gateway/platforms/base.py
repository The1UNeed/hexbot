...
# Audio file extensions Hexbot recognizes for native audio delivery.
# Keep Telegram's narrower attachment/voice sets below separate: formats such
# as MPEG-2 Layer II are audio to Hexbot but unsupported by sendAudio/sendVoice.
_AUDIO_MIME_TYPES = {
    ".ogg": "audio/ogg",
    ".opus": "audio/opus",
    ".mp3": "audio/mpeg",
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Hexbot's DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Hexbot-created Telegram private-chat
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.hexbot/.env manually."
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Hexbot root.
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Hexbot profile and shared Hexbot root both contain control
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.hexbot
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Hexbot
    secrets (``~/.hexbot/.env``, ``auth.json``) are *separate, more-specific*
...
def _cache_dir_container_mounts() -> List[Tuple[Path, Path]]:
    """(host, container) pairs for the auto-mounted Hexbot cache dirs.
...
def _translate_docker_container_media_path(candidate: Path, session_key: str = "") -> Optional[Path]:
    ...
    auto-mounted Hexbot cache dirs (``/root/.hermes/...``), the session's
    ...
    # In-process gateways (Desktop backend, `hexbot core serve`) may not have
...
def validate_media_delivery_path(path: str, session_key: str = "") -> Optional[str]:
    ...
    Hexbot-managed cache, under an operator-allowlisted root
    ...
    # credential/secret stores under the Hexbot root (~/.hexbot/.env,
    ...
    # ``MEDIA:~/.hexbot/google_token.json``) remain rejected.
...
class BasePlatformAdapter(ABC):
    ...
    # Hexbot commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Hexbot sends
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        if owner_profile:
            ...
            remedy = (
                ...
                f"(hexbot core --profile {owner_profile} gateway stop)."
            )
        else:
            holder = f" (PID {owner_pid})" if owner_pid else ""
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Hexbot home layout.
        """
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Hexbot home layout.
        """
