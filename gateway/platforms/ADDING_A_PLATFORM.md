...
There are two ways to add a platform to the Hexbot gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.hexbot/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Hexbot code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `hexbot core gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
Checklist for integrating a platform directly into the Hexbot core.
