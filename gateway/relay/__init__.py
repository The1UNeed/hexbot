"""Relay/connector support package for the Hexbot gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``hexbot core gateway enroll`` writes them to
    ``~/.hexbot/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``hexbot core gateway enroll``).
...
def relay_display_name() -> Optional[str]:
    ...
    if not value:
        ...
        # multi-agent scope it would prefix every reply "**Hexbot:**" —
        ...
        if value in ("Hermes Agent", "Hexbot"):
            value = ""
...
def _resolve_relay_identity_token() -> str:
    ...
    ``hexbot core gateway enroll`` CLI. Three modes, in precedence order:
...
def self_provision_relay() -> bool:
    ...
    up. The creds live ONLY in process memory — never written to ``~/.hexbot/.env``.
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``hexbot core gateway enroll``: has a PINNED
