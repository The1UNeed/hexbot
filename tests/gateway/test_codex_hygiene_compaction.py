...
* ``compression.codex_app_server_auto`` semantics hold: only ``hexbot core``
  lets Hexbot's threshold start a compaction; ``native``/``off`` skip
...
def test_force_without_live_thread_does_not_run_local_compressor(mode):
    # The #73715 branch let force=True fall through to the local Hexbot
