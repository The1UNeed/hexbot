...
AlexKucera reported on Discord (2026-04-16) that after ``hexbot core update`` pulled
...
Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.hexbot/.env).
"""
...
# Load ~/.hexbot/.env so live runs pick up OPENROUTER_API_KEY without
