...
Under a gateway (Telegram, Discord, Slack, ...) Hexbot passes the
...
class TestCrossPlatformMemoryUnification:
    """The same physical user talking to Hexbot via Telegram AND Discord
...
class TestProfilePeerUniqueness:
    """Each Hexbot profile can pin to its own unique peerName.

    Profile cloning copies host blocks, but operators routinely diverge them
    afterwards (e.g. `hexbot core -p partner` pinned to a different person's peer).
