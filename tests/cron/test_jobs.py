...
class TestParseSchedule:
    ...
    def test_naive_iso_anchors_to_configured_tz_not_server_local(self, monkeypatch):
        """A naive ISO timestamp must be interpreted in the CONFIGURED Hexbot
...
class TestNaiveScheduleTimezoneDivergence:
    ...
    become due even when the configured Hexbot timezone differs from the
...
class TestJobsJsonIdKeyedMap:
    ...
    or hand edit — Hexbot's own save_jobs() only ever writes a list) made
    ...
    required`` and took down ``hexbot core cron list``, the ``cronjob(action=
    ...
    def test_list_jobs_survives_id_keyed_map(self, tmp_cron_dir):
        """The reported traceback path (hexbot core cron list / cronjob list tool)."""
        import json
