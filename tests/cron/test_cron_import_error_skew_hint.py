...
Field incident: an interrupted `hexbot core update` (pull done, restart never ran)
...
had no way to know the fix was one `hexbot core gateway restart`. The failure
...
def test_import_error_with_skew_names_shas_and_the_restart_command(monkeypatch):
    ...
    assert "hexbot core gateway restart" in msg
...
def test_import_error_without_skew_stays_a_plain_import_message(monkeypatch):
    ...
    assert "hexbot core gateway restart" not in msg
