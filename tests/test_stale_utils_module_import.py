...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.hexbot/hermes-agent/utils.py)
...
2. ``hexbot core update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``hermes_cli/main.py`` (the ``hexbot core update`` flow, ~line 9326) already
