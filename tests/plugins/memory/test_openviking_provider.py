...
def test_start_local_openviking_server_strips_pythonpath_from_child_env(monkeypatch):
    """The spawned server must not inherit Hexbot's PYTHONPATH (#78153).

    Inheriting it makes openviking-server import packages from the Hexbot
    venv instead of its own, and on Windows locks Hexbot venv DLLs so the
    venv cannot be rebuilt during `hexbot core update`.
    """
    popen_calls = []
...
def test_runtime_does_not_autostart_when_local_server_reports_unhealthy(monkeypatch):
    ...
    assert warnings == [
        ...
        "OpenViking memory is temporarily unavailable; Hexbot will retry on a later access "
...
class TestOpenVikingEnvWriter:
    ...
    def test_bom_prefixed_env_updates_in_place(self, tmp_path):
        ...
        # The stale value must be gone, not left as a duplicate. Hexbot and
