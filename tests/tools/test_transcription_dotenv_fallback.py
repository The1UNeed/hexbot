...
``~/.hexbot/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.hexbot/.env`` would be told
...
class TestEndToEndRegressionGuard:
    ...
    ``~/.hexbot/.env`` carrying the key while ``os.environ`` does not.
