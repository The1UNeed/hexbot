...
class TestSendToPlatformChunking:
    ...
    def test_signal_long_message_is_chunked(self, monkeypatch):
        ...
        The standalone path (hexbot core send / cron / MCP) speaks raw JSON-RPC via
...
class TestSendViaAdapterStandaloneFallback:
    ...
    When the gateway runner is not in this process (e.g. ``hexbot core cron``
    runs separately from ``hexbot core gateway``), ``_send_via_adapter`` should
