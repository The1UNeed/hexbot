...
def _skill_dir(tmp_path):
    ...
    only the temp directory — not the real ~/.hexbot/skills/."""
...
class TestExternalSkillMutations:
    ...
    caused agents to create duplicate copies in ~/.hexbot/skills/ as a
    workaround.
    """
...
# come up. The user unpins via `hexbot core curator unpin <name>` to delete.
# ---------------------------------------------------------------------------
...
class TestPinnedGuard:
    ...
    def test_delete_refuses_pinned(self, tmp_path):
        ...
        assert "hexbot core curator unpin my-skill" in result["error"]
        # Skill still exists
        assert (tmp_path / "my-skill" / "SKILL.md").exists()
