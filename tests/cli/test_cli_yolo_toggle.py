...
prompt. Only ``hexbot core --yolo`` (process-start env), ``HERMES_YOLO_MODE=1``,
and ``hexbot core config set approvals.mode off`` actually bypassed.
...
def _clear_approval_state(monkeypatch):
    ...
    # Hexbot-driven test runs may inherit HERMES_YOLO_MODE=1 from the parent
...
class TestIsSessionYoloActiveHelper:
    ...
    def test_helper_honors_frozen_yolo_mode(self):
        """``hexbot core --yolo`` sets ``HERMES_YOLO_MODE`` before tool imports, so
