...
file to be discoverable via ``hexbot core sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.hexbot/sessions/saved/`` and prints
...
def test_save_conversation_writes_under_hermes_home(hermes_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.hexbot/sessions/saved/, not CWD."""
    ...
    # File MUST be under ~/.hexbot/sessions/saved/
    ...
    assert "hexbot core --resume 20260101_120000_abc123" in out, out
