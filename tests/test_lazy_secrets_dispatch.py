...
1. `hexbot core secrets bitwarden setup --help` works (dispatch path)
2. `hexbot core update --check` works (update path)
3. `hexbot core secrets bitwarden disable` works (handler execution)
4. `hexbot core secrets onepassword status` works (lazy backend loads on demand)
...
def _run_hermes(args: list[str], timeout: int = 30) -> subprocess.CompletedProcess[str]:
    ...
    connectivity — but ``hexbot core update --check`` really does ``git fetch``
...
class TestSecretsDispatchE2E:
    ...
    def test_bitwarden_setup_help(self) -> None:
        """`hexbot core secrets bitwarden setup --help` must exit 0 and print usage.
    ...
    def test_bitwarden_status(self) -> None:
        """`hexbot core secrets bitwarden status` must exit 0 (runs lazy backend)."""
    ...
    def test_bitwarden_disable(self) -> None:
        """`hexbot core secrets bitwarden disable` must exit 0."""
    ...
    def test_onepassword_status(self) -> None:
        """`hexbot core secrets onepassword status` must exit 0 (1Password lazy backend)."""
    ...
    def test_onepassword_setup_help(self) -> None:
        """`hexbot core secrets onepassword setup --help` must exit 0."""
...
class TestUpdatePathE2E:
    ...
    These tests invoke the real `hexbot core update --check` path as a subprocess.
    ...
    def test_update_check_clean(self) -> None:
        """`hexbot core update --check` must not load cryptography._rust."""
