"""Regression test: `hexbot core dashboard --tui` must not hard-crash.

Older Hexbot desktop app shells (<= 0.15.x) spawn the backend as::

    hexbot core dashboard --no-open --tui --host 127.0.0.1 --port <PORT>
...
died before it became ready and the desktop GUI showed only "Hexbot couldn't
