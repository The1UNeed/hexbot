...
class TestListNavigation:
    """hexbot core config set must preserve YAML list fields when using numeric
...
class TestCronModelDriftConfigWarning:
    ...
    def test_warning_names_the_user_owned_cli_pin_path(
        ...
        assert "hexbot core cron edit <job_id> --provider <provider> --model <model>" in warning
...
class TestSchemaValidation:
    """#34067: ``hexbot core config set`` must not report bare success for
    ...
    a post-write notice warns that Hexbot may never read it and suggests the
...
class TestDisplaySkinTouch:
    ...
    (`hexbot core config set display.skin X` while it is already X — the recovery
...
class TestMappingGuard:
    """``hexbot core config set <section> <scalar>`` must not silently destroy an
    ...
    def test_bare_model_shorthand_preserves_siblings(self, _isolated_hermes_home):
        """hexbot core config set model <id> → model.default, siblings survive."""
        self._write_config(_isolated_hermes_home, {
            "model": {
                "default": "gpt-4o",
                "provider": "openai-api",
    ...
    def test_non_model_mapping_is_refused(self, _isolated_hermes_home):
        """hexbot core config set terminal bash → refuse, terminal has sub-keys."""
        self._write_config(_isolated_hermes_home, {
            "terminal": {
                "backend": "docker",
    ...
    def test_non_model_mapping_force_overwrites(self, _isolated_hermes_home):
        """hexbot core config set --force terminal bash → proceed, section wiped."""
        self._write_config(_isolated_hermes_home, {
            "terminal": {
                "backend": "docker",
                "shell": "bash",
    ...
    def test_model_force_overwrites_entire_section(self, _isolated_hermes_home):
        """hexbot core config set --force model <id> → overwrite entire section."""
        self._write_config(_isolated_hermes_home, {
            "model": {
                "default": "gpt-4o",
                "provider": "openai-api",
...
class TestLiteralDotKeyEscaping:
    """``hexbot core config set/unset/get`` must not split a key segment on a
