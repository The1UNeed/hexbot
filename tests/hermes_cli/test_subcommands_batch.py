...
def test_config_get_unset_subcommands_parse():
    """`hexbot core config get/unset` parse key args (and --json for get)."""
...
# ── deprecated `hexbot core login` fails gracefully, not with argparse error ────
#
# `hexbot core login` is a removed command; its handler (`login_command` in
# `hermes_cli/auth.py`) prints a deprecation notice pointing at `hexbot core auth` /
# `hexbot core model` and exits 0.  Two behavior contracts guard the UX:
...
def test_login_subparser_help_is_suppressed():
    """The deprecated `login` row must not appear in `hexbot core --help`.
