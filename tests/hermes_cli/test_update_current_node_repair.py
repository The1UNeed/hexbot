...
A previous ``hexbot core update`` whose pnpm install failed printed "Fix pnpm and
re-run `hexbot core update`" — but re-running hit the "Already up to date!" early
...
def test_current_checkout_repairs_failed_node_deps(capsys):
    """A recorded failure surfaces the fix-pnpm hint, not 'Already up to date!'."""
    ...
    assert "Fix pnpm and re-run `hexbot core update`." in out
