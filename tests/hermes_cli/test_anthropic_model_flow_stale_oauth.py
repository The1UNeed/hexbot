"""Tests for Bug #12905 fix — stale OAuth token detection in hexbot core model flow.

Bug 3: `hexbot core model` with `provider=anthropic` skips OAuth re-authentication
when a stale ANTHROPIC_TOKEN exists in ~/.hexbot/.env but no valid
