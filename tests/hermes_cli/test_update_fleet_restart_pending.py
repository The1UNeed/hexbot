...
A ``hexbot core update`` killed after git pull advanced HEAD but before the
...
- next ``hexbot core update`` with git already up to date still runs the
...
def test_startup_warn_prints_when_marker_present(capsys):
    ...
    assert "hexbot core gateway restart" in err
