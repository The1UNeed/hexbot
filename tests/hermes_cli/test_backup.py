"""Tests for hexbot core backup and import commands."""
...
def _make_hermes_tree(root: Path) -> None:
    """Create a realistic ~/.hexbot directory structure for testing."""
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        """A zip containing state.db is accepted as a valid Hexbot backup."""
...
class TestImport:
    ...
    def test_import_survives_service_layer_import_failure(self, tmp_path, monkeypatch, capsys):
        ...
        assert "Done. Your Hexbot configuration has been restored." in out
...
class TestImportAtomicWrites:
    """`hexbot core import` must never leave a user's file truncated.
    ...
    def test_restore_preserves_existing_file_owner(self, tmp_path, monkeypatch):
        ...
        ``sudo hexbot core import`` onto a user-owned (or Docker/NAS volume-owned)
    ...
    def test_restore_does_not_carry_setuid_onto_archive_content(
        ...
        re-serialize content this process produced; ``hexbot core import`` is the
...
# ---------------------------------------------------------------------------
# Pre-update backup (hexbot core update safety net)
# ---------------------------------------------------------------------------
...
class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``hexbot core update``
    ...
    def test_backup_contents_match_full_backup(self, hermes_home):
        ...
        ``hexbot core backup`` would, and should exclude the same directories."""
    ...
    def test_pre_update_zip_does_not_nest_the_pre_update_snapshot(self, hermes_home):
        """``hexbot core update`` in ``full`` mode takes the quick snapshot *before*
...
# ---------------------------------------------------------------------------
# Pre-migration backup (hexbot core claw migrate safety net)
# ---------------------------------------------------------------------------

class TestPreMigrationBackup:
    """Tests for create_pre_migration_backup — the auto-backup
    ``hexbot core claw migrate`` runs before mutating ~/.hexbot/."""
    ...
    def test_restorable_with_hermes_import(self, hermes_home, tmp_path):
        """The zip produced by pre-migration backup must be a valid Hexbot
        backup — `hexbot core import` should accept it."""
...
class TestRestoreCronJobsIfEmptied:
    """`hexbot core update` config migration can leave cron/jobs.json valid-but-empty,
...
class TestImportHonorsHermesHomeOverride:
    """`hexbot core import` must restore into the home the command runs under.
