"""Tests for xAI Grok OAuth — tokens stored in Hexbot auth store (~/.hexbot/auth.json)."""
...
def _setup_hermes_auth(
    ...
    *,
    ...
    """Write xAI OAuth tokens into the Hexbot auth store at the given root."""
...
def test_refresh_xai_oauth_pure_403_marked_tier_denied_not_relogin(monkeypatch):
    ...
    being active. Re-running ``hexbot core model`` won't help in that
...
def test_format_auth_error_tier_denied_does_not_suggest_relogin():
    ...
    Regression for #26847: telling a tier-gated user to ``hexbot core model``
...
def test_refresh_xai_oauth_pure_rejects_non_https_token_endpoint(monkeypatch):
    ...
    Hexbot that didn't validate) must not be silently honored on the refresh
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    """After `hexbot core model` -> xai-oauth, the singleton holds tokens.  load_pool
    must surface that as a pool entry so `hexbot core auth list` reflects truth and
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``hexbot core auth remove xai-oauth 1`` for a
    ...
    # Act: the user runs `hexbot core auth remove xai-oauth 1`.
...
def test_login_xai_oauth_relogin_clears_suppression_and_reseeds(tmp_path, monkeypatch):
    """remove -> ``hexbot core model`` re-login (``_login_xai_oauth``) must clear the
    ...
    only ``auth_add_command`` cleared it — the ``hexbot core model`` re-login path did
    not. So after remove -> re-login the seed kept skipping and ``hexbot core auth
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`hexbot core auth add xai-oauth` entries (source='manual:xai_pkce') are
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    provider (visible to ``hexbot core auth status``, ``hexbot core setup``, and the
    ``hexbot core`` no-arg dispatcher).  Pin the ``set_active=False`` contract so
