...
quarantine retry / reboot-deferred fallback during `hexbot core update` on Windows.
...
def test_classify_concurrent_instance_recognises_gateway_runtimes(monkeypatch):
    ...
    gateway/run.py, bare `hexbot core gateway` which defaults to run)."""
    cases = [
        ...
        ["hermes.exe", "gateway"],  # bare `hexbot core gateway` defaults to run
