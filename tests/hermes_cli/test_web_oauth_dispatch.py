...
Hexbot must not mint subscription OAuth tokens from an unattended HTTP
...
def test_anthropic_dashboard_oauth_is_removed_and_external():
    ...
    assert providers["anthropic"]["cli_command"] == "hexbot core auth add anthropic"
...
def test_accounts_offers_every_oauth_provider_from_catalog():
    ...
    `hexbot core model` universe) must be offered by /api/providers/oauth. This keeps
...
def test_oauth_catalog_marks_external_providers_not_disconnectable():
    """External CLI credentials are visible in Accounts but cannot be removed by Hexbot."""
    resp = client.get("/api/providers/oauth", headers=HEADERS)
    assert resp.status_code == 200, resp.text
    providers = {p["id"]: p for p in resp.json()["providers"]}
...
def test_xai_dashboard_poller_seeds_single_entry_and_clears_suppression(tmp_path, monkeypatch):
    ...
    ``device_code`` suppression left by a prior ``hexbot core auth remove
    xai-oauth``.
    """
    from hermes_cli import auth as auth_mod
    from hermes_cli import web_server as ws
    ...
    # Prior `hexbot core auth remove xai-oauth` left the source suppressed.
