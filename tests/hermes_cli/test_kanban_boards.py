"""Tests for the multi-board kanban layer (``hexbot core kanban boards …``).
...
* CLI surface: ``hexbot core kanban boards list/create/switch/rm``.
...
def _cli(args: list[str], env_extra: dict | None = None) -> subprocess.CompletedProcess:
    """Run ``hexbot core kanban …`` with PYTHONPATH pinned to the worktree."""
    env = dict(os.environ)
