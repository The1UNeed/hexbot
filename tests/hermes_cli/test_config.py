...
class TestLoadConfigParseFailure:
    ...
    their config.yaml was being ignored. The helper must:
      * log at WARNING (so ``hexbot core logs`` surfaces it)
...
class TestOptionalEnvVarsRegistry:
    ...
    def test_max_iterations_not_offered_as_env_var(self):
        ...
        Offering it as an editable env var (dashboard, `hexbot core setup`) lets a
...
class TestMemoryProviderEnvVarsRegistry:
    ...
    and `hexbot core setup` surface it (previously only Honcho was listed, leaving
...
class TestConfigSupportFloor:
    ...
    def test_v11_config_is_refused_and_untouched(self, tmp_path, capsys):
        ...
        assert "run `hexbot core setup` to regenerate" in captured.out
...
class TestEnvWriteDenylist:
    ...
    ``PATH``, ``EDITOR``, etc. — or selected Hexbot runtime/security controls.
    ...
    ``LD_PRELOAD=/tmp/evil.so`` in ``.env`` and own the next Hexbot
...
class TestConfigCommandFailClosedSurface:
    """`hexbot core config set/unset` must exit cleanly (no traceback) when the
