...
1. The venv-repair import probes (early recovery + `hexbot core update`) must
...
2. ``hexbot core doctor`` must fail the certificate check in that state, and
   ``hexbot core doctor --fix`` must repair by force-reinstalling certifi and
   re-verifying.
"""
...
class TestUpdateProbeScriptChecksBundle:
    """The subprocess probe used by `hexbot core update`'s venv repair must apply
...
# =========================================================================
# 2. hexbot core doctor: detection and --fix repair
# =========================================================================
...
class TestSslGuardRepairHint:
    def test_missing_bundle_error_mentions_doctor_fix(self, monkeypatch, tmp_path):
        ...
        assert "hexbot core doctor --fix" in message
