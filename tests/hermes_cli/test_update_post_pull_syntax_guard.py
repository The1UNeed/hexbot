"""Tests for the post-pull syntax guard in ``hexbot core update``.
...
becomes unbootable — every ``hexbot core`` invocation imports those files at
...
markers in ``hermes_cli/config.py``; users who ran ``hexbot core update`` in
the 7-minute window before #28458 landed could not run any ``hexbot core``
command afterward.
"""
...
# release; if a future ``hexbot core update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
