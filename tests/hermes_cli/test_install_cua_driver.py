...
* Be supported-platform-only — no-op silently elsewhere so ``hexbot core update``
...
class TestInstallCuaDriverUpgrade:
    ...
    def test_install_target_writability_is_probed_for_real_on_macos(self):
        ...
        ``macos_only``: ``/Applications`` is the only install target Hexbot
...
class TestRequireConfirmedUpdate:
    """`hexbot core update` passes require_confirmed_update=True: the full
    ...
    Explicit `hexbot core computer-use install --upgrade` keeps the old
    ...
    def test_explicit_upgrade_still_falls_through_on_indeterminate(self):
        # `hexbot core computer-use install --upgrade` (default flag): the old
    ...
    def test_incompatible_driver_repairs_on_posix_despite_indeterminate_check(self):
        """Hexbot's own version floor is the confirmation. When the installed
        driver fails the runtime contract, the `hexbot core update` refresh must
...
class TestUpdateCheckTimeoutDefaults:
    ...
    full reinstall fall-through during `hexbot core update`.
    """
...
class TestInstallerTimeoutDrainIsBounded:
    ...
    def test_drain_that_times_out_still_surfaces_the_run_timeout(self):
        ...
        so the existing manual re-run hint prints and ``hexbot core update``
...
class TestCuaVersionSummary:
    """`hexbot core computer-use status` prints one line, whatever the binary says.
