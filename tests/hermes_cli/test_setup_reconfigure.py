...
- Bare `hexbot core setup` drops straight into the full reconfigure wizard
  (every prompt shows the current value as its default).
- `hexbot core setup --quick` runs the narrower "fill in missing items" flow.
- `hexbot core setup --reconfigure` is a backwards-compat alias for the
  bare-setup default.
...
class TestExistingInstallDefault:
    """Bare `hexbot core setup` on an existing install = full reconfigure wizard."""
