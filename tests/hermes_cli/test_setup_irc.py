"""Tests for IRC gateway configuration via `hexbot core setup gateway` UI.

Covers the full plugin-platform discovery → status → configure flow so that
a fresh Hexbot install (no state, no env vars) can set up IRC through the
...
class TestIRCFreshInstallDiscovery:
    """IRC appears in the setup menu on a brand-new Hexbot install."""
...
class TestIRCGatewaySetupFreshInstall:
    """Simulate the full `hexbot core setup gateway` experience with IRC present."""
