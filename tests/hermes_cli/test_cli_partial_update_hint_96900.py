...
bare ``hexbot core`` and ``hexbot core chat`` (including the fast-chat launch path)
all go through it.
"""
...
def test_emit_hint_for_missing_resolve_turn_limit():
    ...
    assert "hexbot core update" in text
...
def test_emit_hint_for_missing_split_model_config_default():
    ...
    buf = io.StringIO()
    assert emit_partial_update_hint(exc, file=buf) is True
    assert "hexbot core update" in buf.getvalue()
...
def test_cmd_chat_prints_update_hint_when_config_helper_is_missing(
    ...
    assert name in err
    assert "hexbot core update" in err
