"""Tests for --ignore-user-config and --ignore-rules flags on `hexbot core chat`.
...
files. In Hexbot the equivalent isolation is:

* ``--ignore-user-config`` → skip ``~/.hexbot/config.yaml`` in ``load_cli_config()``
