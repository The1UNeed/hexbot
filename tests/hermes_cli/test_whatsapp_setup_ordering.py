...
Before the fix, ``hexbot core whatsapp`` wrote ``WHATSAPP_ENABLED=true`` at
...
``hexbot core gateway`` then paid a 30s bridge-bootstrap timeout and queued
...
def test_existing_pairing_skip_branch_enables_whatsapp(isolated_home, monkeypatch):
    """User runs ``hexbot core whatsapp`` with an existing paired session and
