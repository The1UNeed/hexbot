"""Tests for `hexbot core memory setup [provider]` routing.
...
``hexbot core memory setup honcho``) without the interactive picker — which matters
...
class TestMemorySetupProviderRouting:
    ...
    def test_unknown_provider_reports_and_returns_early(self, capsys):
        ...
        assert "hexbot core memory setup" in out
