...
``hexbot core profile delete`` removes the tree. That empty shell then
reappears in ``hexbot core profile list`` and Desktop Bot Mode. These tests
...
class TestNamedProfileHome:
    def test_logs_under_named_profile_resolve_to_profile_home(self, tmp_path):
        # tmp_path acts as a real Hexbot home (Docker/custom layout): it
    ...
    def test_unrelated_profiles_dir_is_not_named(self, tmp_path):
        ...
        # its parent is not a Hexbot home, so logging must keep mkdir-ing.
    ...
    def test_tombstone_dir_marks_profiles_root(self, tmp_path):
        ...
        # `hexbot core profile delete` — its presence alone anchors recognition,
