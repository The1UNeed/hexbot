...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``hexbot core update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    # Exact flags used by hexbot core update (hermes_cli/main.py).
...
def test_restore_rejects_invalid_python_and_keeps_clean_updated_tree(
    ...
    assert "made the Hexbot agent unexecutable" in output
