"""``hexbot core config set`` must parse list/mapping literals, not store them as strings.

Before this fix, ``hexbot core config set platform_toolsets.discord '["file","web"]'``
