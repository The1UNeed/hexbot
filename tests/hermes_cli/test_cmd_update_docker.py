"""Tests for ``hexbot core update`` / ``--check`` inside the Docker container.
...
fix, ``hexbot core update`` would fall through to ``"✗ Not a git repository.
Please reinstall: curl ... install.sh"`` — that script installs a *new*
host-side Hexbot, not an update to the running container, so the message
...
def test_cmd_update_in_docker_prints_guidance_and_exits(
    ...
    """``hexbot core update`` inside Docker → friendly message + exit 2, no git calls.
