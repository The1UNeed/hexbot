...
The bootstrap module is imported at the top of every Hexbot entry point
...
Key invariants covered by these tests:
  ...
  5. Load order: every Hexbot entry point imports hermes_bootstrap as its
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Hexbot CLI
...
class TestEntryPointsImportBootstrap:
    """Every Hexbot entry point must import hermes_bootstrap as its
    ...
    # Entry points that invoke Hexbot as a process.  Each one must
    ...
    def test_entry_point_imports_bootstrap(self, path):
        ...
        half-finished ``hexbot core update`` (git-reset landed new code but
...
class TestHardenImportPath:
    ...
    directory from shadowing Hexbot's own top-level modules — covering both
