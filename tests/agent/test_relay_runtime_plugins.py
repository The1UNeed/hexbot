...
def test_foreign_active_plugin_configuration_is_left_unchanged(
    ...
    try:
        ...
        assert relay.events == []
        assert "already active outside Hexbot native ownership" in caplog.text
...
def test_initialization_failure_is_fail_open(explicit_static_config, caplog):
    ...
    try:
        ...
        assert "Hexbot Relay plugin initialization failed" in caplog.text
    finally:
        host.shutdown()
...
""".strip(),
    ...
    try:
        ...
        assert relay.events == []
        assert "Hexbot [[dynamic_plugins]] records are unsupported" in caplog.text
