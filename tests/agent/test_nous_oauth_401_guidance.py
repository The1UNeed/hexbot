...
to a generic "Your API key was rejected... run hexbot core setup" message, which is
...
def test_nous_401_guidance_strings_present():
    ...
    # Must give a concrete re-auth command, not a generic "hexbot core setup".
    assert "hexbot core portal" in source
