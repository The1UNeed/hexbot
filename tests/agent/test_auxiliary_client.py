...
class TestResolveXaiOAuthForAux:
    def test_uses_pool_backed_credentials_without_singleton(self, tmp_path, monkeypatch):
        ...
        ``hexbot core auth status`` already reports these as logged in; compression
...
class TestResolveProviderClientUniversalModelFallback:
    ...
    provider was picked via ``hexbot core model``, no per-task override is
...
class TestAuxiliaryFallbackLayering:
    ...
    def test_fallback_entry_openai_codex_uses_oauth_pool_without_inline_key(self):
        """Configured Codex fallback resolves through Hexbot auth / credential pool."""
...
class TestAuxClientNoSdkRetries:
    ...
    disabled so Hexbot owns the retry/timeout budget (issue #54465). The SDK
...
class TestAnthropicAuxiliaryReasoningTranslation:
    """Native Anthropic aux adapters must receive normalized Hexbot reasoning.

    MoA slot reasoning is carried through call_llm as a Hexbot
