...
class TestEmbeddedTuiPaneClarifier:
    """When ``HERMES_DESKTOP_TERMINAL=1``, a standalone ``hexbot core --tui`` is
...
class TestContradictionGone:
    ...
    contained both ``You are running in the Hexbot terminal UI (TUI).`` and
    ``Runtime surface: you're running inside the Hexbot desktop GUI app.``.
