"""The ACP text bridge is what makes Hexbot's own tools reachable on an ACP provider.
...
def test_allowlist_forwards_only_the_named_tools():
    ...
    make Hexbot re-run finished work, so those clients forward an allowlist."""
...
def test_response_level_extras_survive_the_stream_conversion():
    """Hexbot reads provider extras off the returned object; a plain list would
...
def test_copilot_prompt_still_carries_the_contract_and_the_tools():
    ...
    assert "Hexbot requested model hint: gpt-5" in prompt
    assert "hi" in prompt
