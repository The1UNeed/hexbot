...
conversation-affinity hint Hexbot sends — ``prompt_cache_key`` on both
...
bucket it warmed. Hexbot cannot infer the logical conversation from the id's
...
class TestDeclaredConversationScope:
    ...
    def test_scope_never_carries_the_raw_key(self, db):
        ...
        A session id is a Hexbot-internal token; a session KEY embeds the
