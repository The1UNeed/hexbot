"""POSIX advisory locks must survive Hexbot's own database inspection.
...
Hexbot used to byte-probe live databases in several places (kanban's
post-commit page-count check, the zeroed-state.db detector run on every
SessionDB construction, backup header verification). Under `hexbot core sessions
...
out across Hexbot's inspection calls.
"""
