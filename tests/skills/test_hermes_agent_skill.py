"""The `hermes-agent` skill is what a running Hexbot knows about itself.

`website/` is never packaged, so an installed Hexbot has no local copy of the
...
shipped feature is invisible and the agent answers "Hexbot can't do that."
"""
